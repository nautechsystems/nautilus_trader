"""
Expose the canonical equities-maker runtime param surface and defaults.
"""
...
_LOCAL_OWNERSHIP_PARAM_NAMES = frozenset(
    {
        "des_qty_local",
        "max_qty_local",
        "max_skew_bps_local",
...
_EQUITIES_MAKER_DEFAULT_OVERRIDES: dict[str, bool | int | float] = {
    "qty": 1.0,
...
EQUITIES_MAKER_RUNTIME_PARAM_REGISTRY = RuntimeParamRegistry(
    param_set=EQUITIES_MAKER_PARAM_SET,
    specs=_clone_runtime_specs(),
)
EQUITIES_MAKER_RUNTIME_PARAM_SCHEMA: dict[str, dict[str, Any]] = {
    name: dict(spec) for name, spec in EQUITIES_MAKER_RUNTIME_PARAM_REGISTRY.schema.items()
}
EQUITIES_MAKER_RUNTIME_PARAM_DEFAULTS: dict[str, Any] = dict(
    EQUITIES_MAKER_RUNTIME_PARAM_REGISTRY.defaults,
)
PARAM_SET = EQUITIES_MAKER_PARAM_SET
RUNTIME_PARAM_SCHEMA = EQUITIES_MAKER_RUNTIME_PARAM_SCHEMA
RUNTIME_PARAM_DEFAULTS = EQUITIES_MAKER_RUNTIME_PARAM_DEFAULTS


def profile_key() -> str:
    return EQUITIES_MAKER_PROFILE_KEY
...
def params_manager_factory(
    *,
    ...
    runtime_defaults = dict(EQUITIES_MAKER_RUNTIME_PARAM_DEFAULTS)
    if defaults:
    ...
    def _factory(strategy: Any) -> FluxParamsManager:
        ...
        resolved_defaults = {
            ...
            for name in EQUITIES_MAKER_RUNTIME_PARAM_REGISTRY.names
        }
        return FluxParamsManager(
            ...
            schema=EQUITIES_MAKER_RUNTIME_PARAM_SCHEMA,
            defaults=resolved_defaults,
            param_set=EQUITIES_MAKER_RUNTIME_PARAM_REGISTRY.param_set,
        )
...
__all__ = [
    ...
    "EQUITIES_MAKER_RUNTIME_PARAM_REGISTRY",
