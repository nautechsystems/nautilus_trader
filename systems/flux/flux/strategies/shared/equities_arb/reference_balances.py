...
except Exception as exc:  # pragma: no cover - exercised in environments without IBKR deps
    _IBKR_IMPORT_ERROR = exc

    class DockerizedIBGatewayConfig:  # type: ignore[no-redef]
        pass

    def drop_cached_ib_client(*args: Any, **kwargs: Any) -> None:  # type: ignore[no-redef]
        _ = args, kwargs
        return None

    def get_cached_ib_client(*args: Any, **kwargs: Any) -> Any:  # type: ignore[no-redef]
        raise RuntimeError(
            "Interactive Brokers adapter dependencies are unavailable in this environment",
        ) from _IBKR_IMPORT_ERROR
else:
    _IBKR_IMPORT_ERROR = None


if __name__ == "flux.strategies.shared.equities_arb.reference_balances":
    sys.modules.setdefault(
        "nautilus_trader.flux.strategies.shared.equities_arb.reference_balances",
        sys.modules[__name__],
    )
elif __name__ == "nautilus_trader.flux.strategies.shared.equities_arb.reference_balances":
    sys.modules.setdefault(
        "flux.strategies.shared.equities_arb.reference_balances",
        sys.modules[__name__],
    )
...
class IbkrReferenceBalanceSnapshotProvider:
    ...
    def _ensure_standalone_runtime(self) -> _StandaloneIbkrRuntime:
        runtime = self._standalone_runtime
        if runtime is not None and runtime.thread.is_alive() and not runtime.loop.is_closed():
            return runtime
    ...
    async def _fetch_account_payload(
        ...
        *,
        ...
        def _on_account_summary(tag: str, value: str, currency: str) -> None:
            if not currency:
                return
            if tag not in _ACCOUNT_SUMMARY_TAGS:
                return
