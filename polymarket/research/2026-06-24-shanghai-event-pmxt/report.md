...
这份报告是 pre-v1 上海温度事件 PMXT smoke 的历史结果说明。目录已经迁移到
`polymarket/research/2026-06-24-shanghai-event-pmxt/`，但**当前有效的 v1
回测入口不是旧脚本**，而是统一的：
...
本目录保留历史报告、compact summary、图和旧 smoke 结果，主要用于对照 PMXT
问题和早期策略输出。后续新增策略/实验应新建独立 research topic，并通过
`experiment.yml` 驱动 v1 adapter/backtest。旧的 `run_event_backtest.py` /
`run_strategy_suite.py` 口径已经被 v1 adapter、canonical dataset 和
`backtest_v1.py` 取代，不作为新的开发入口。
...
```text
polymarket/research/2026-06-24-shanghai-event-pmxt/data/strategy_suite_summary.csv
```
