...
```bash
python polymarket/research/2026-07-01-backtest-correctness/scripts/check_data_health.py
```

输出：

- `polymarket/research/2026-07-01-backtest-correctness/data/data_health_summary.json`
- `polymarket/research/2026-07-01-backtest-correctness/data_health_report.md`
...
```bash
python -m pytest polymarket/tests/test_synthetic_golden_replay.py polymarket/tests/test_adapter_contracts.py -q
```

当前 v1 覆盖 5 类通用 contract：
...
该 contract 现在由 `polymarket/tests/test_adapter_contracts.py` 保护，构造 source
timestamp 倒序、received timestamp 正序：
...
当前 v1 synthetic golden / adapter contract tests 全部通过，说明现有 harness 在这些基础语义上暂未暴露未来函数或明显记账错误：
