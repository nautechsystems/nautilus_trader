/**
 * Represents a valid strategy ID.
 */
import { getLib } from "../../lib";
import { toCString } from "../../_internal/memory";
import type { Pointer } from "../../_internal/types";

export class StrategyId {
  readonly _ptr: Pointer;

  private constructor(ptr: Pointer) {
    this._ptr = ptr;
  }

  static from(value: string): StrategyId {
    const raw = getLib().symbols.strategy_id_new(toCString(value));
    return new StrategyId(raw as number);
  }

  hash(): bigint {
    const buf = new BigInt64Array([BigInt(this._ptr)]);
    return getLib().symbols.strategy_id_hash(buf as unknown as Pointer) as bigint;
  }
}
