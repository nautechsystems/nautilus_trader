...
type SignalPayloadKey = 'maker_v4' | 'equities_arb';
...
function resolveAssetKey(row: Pick<SignalStrategy, 'id' | 'meta'>): string {
  const baseAsset = String(row.meta?.base_asset ?? '').trim().toUpperCase();
  if (baseAsset) return baseAsset;
  return String(row.id ?? '').split('_')[0]?.trim().toUpperCase() || '—';
}
  ...
  const data = useMemo<MakerV4DisplayRow[]>(() => {
    const enriched = sourceRows.map((row) => {
      ...
      return {
        ...
        _operator: payload?.operator ?? null,
        _statusLabel: describeTradingStatus(status),
        _lastUpdateMs: lastUpdateMs,
        _lastAgeMs: lastAgeMs,
        _variantLabel: resolveVariantLabel(row),
        _assetKey: resolveAssetKey(row),
      };
    });
    ...
    return enriched.sort((left, right) => {
      const assetCompare = left._assetKey.localeCompare(right._assetKey, undefined, { sensitivity: 'base' });
      if (assetCompare !== 0) return assetCompare;
      const variantCompare = VARIANT_ORDER[left._variantLabel] - VARIANT_ORDER[right._variantLabel];
  ...
  const columns = useMemo<ColumnDef<MakerV4DisplayRow>[]>(() => {
    ...
    if (showVariantColumn) {
      baseColumns.push({
        ...
        header: 'Variant',
        cell: ({ row }) => (
          <span className="text-xs text-neutral-100">{row.original._variantLabel}</span>
        ),
      });
    }
  ...
  return (
    <div data-testid={tableTestId}>
      <DataTable
        ...
        dense={false}
        loading={loading}
        emptyMessage={loading ? 'Loading strategies...' : emptyMessage}
        className={showVariantColumn ? 'min-w-[1320px]' : 'min-w-[1100px]'}
        widthMode="content"
