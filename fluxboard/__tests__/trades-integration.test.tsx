...
describe('Trades integration flows', () => {
  ...
  it('does not seed initial tokenmm standard subscribe from a polluted trades-store lastSeq', async () => {
    realtimeFlags.trades = true;
    window.history.replaceState({}, '', '/tokenmm/trades');
    useTradesStore.setState((state) => ({
      ...state,
      lastSeq: 7270134935126016,
    }));
    setNextSubscribeAck({
      accepted: true,
      contract_version: 2,
  ...
  it('does not fall back to row-level snapshot seq when tokenmm standard replay restarts from zero', async () => {
    realtimeFlags.trades = true;
    window.history.replaceState({}, '', '/tokenmm/trades');
    setNextSubscribeAck({
      accepted: true,
  ...
  it('reconnects with a fresh canonical snapshot after an older canonical request resolves late', async () => {
    ...
    const canonicalReentry = createDeferred<any>();
    getTrades
      .mockImplementationOnce(() => initialCanonical.promise)
      .mockResolvedValueOnce({
        rows: baseRows,
        total: baseRows.length,
        page: 1,
      ...
      .mockImplementationOnce(() => canonicalReentry.promise)
      .mockResolvedValueOnce({
        rows: [{ ...baseRows[0], row_id: 'reconnect-catchup', seq: 21, ts: 21 }],
        total: baseRows.length,
        page: 1,
        page_size: 50,
        last_seq: 21,
    ...
    await waitFor(() => {
      expect(getTrades).toHaveBeenCalledTimes(4);
    });
