...
describe('useRealtimeSurfaceController', () => {
  ...
  it('rerenders visible-row subscribers for in-place visible updates without rerendering canonical row subscribers', () => {
    const controller = createController();
    controller.applySnapshot([
      { id: 'alpha', rank: 4, value: 'A' },
      { id: 'beta', rank: 3, value: 'B' },
    ...
    function RowsProbe() {
      const rows = useRealtimeSurfaceController(controller, (snapshot) => snapshot.rows);
      rowsRenders.push(rows.map((row) => `${row.id}:${row.value}`).join('|'));
      return null;
    }

    function VisibleProbe() {
      const visibleRows = useRealtimeSurfaceController(controller, (snapshot) => snapshot.visibleRows);
      visibleRenders.push(visibleRows.map((row) => `${row.id}:${row.value}`).join('|'));
      return null;
    }
    ...
    act(() => {
      controller.applyDelta({
        kind: 'upsert',
        row: { id: 'delta', rank: 1, value: 'D+' },
      });
    });

    expect(rowsRenders).toHaveLength(1);
    expect(visibleRenders).toEqual(['alpha:A|beta:B']);

    act(() => {
      controller.applyDelta({
        kind: 'upsert',
        row: { id: 'alpha', rank: 4, value: 'A+' },
      });
    });

    expect(rowsRenders).toHaveLength(1);
    expect(visibleRenders).toEqual(['alpha:A|beta:B', 'alpha:A+|beta:B']);
  });
});
