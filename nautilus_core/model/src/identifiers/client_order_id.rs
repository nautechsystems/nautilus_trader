...
pub fn optional_ustr_to_vec_client_order_ids(s: Option<Ustr>) -> Option<Vec<ClientOrderId>> {
    s.map(|ustr| {
        ...
        s_str
            .split(',')
            .map(|s| ClientOrderId::new(s).unwrap()) // TODO: Propagate error
...
pub unsafe extern "C" fn client_order_id_new(ptr: *const c_char) -> ClientOrderId {
    ...
    ClientOrderId::new(CStr::from_ptr(ptr).to_str().expect("CStr::from_ptr failed")).unwrap()
}
...
mod tests {
    ...
    fn test_string_reprs() {
        let id = ClientOrderId::new("O-20200814-102234-001-001-1").unwrap();
    ...
    fn test_optional_vec_client_order_ids_to_ustr() {
        ...
        let client_order_ids = vec![
            ClientOrderId::new("id1").unwrap(),
            ClientOrderId::new("id2").unwrap(),
            ClientOrderId::new("id3").unwrap(),
        ];
