...
pub unsafe extern "C" fn client_id_new(ptr: *const c_char) -> ClientId {
    ...
    ClientId::new(CStr::from_ptr(ptr).to_str().expect("CStr::from_ptr failed")).unwrap()
}
...
mod tests {
    ...
    fn test_string_reprs() {
        let id = ClientId::new("BINANCE").unwrap();
    ...
    fn test_client_id_hash_c() {
        let id1 = ClientId::new("BINANCE").unwrap();
        let id2 = ClientId::new("BINANCE").unwrap();
        let id3 = ClientId::new("DYDX").unwrap();
