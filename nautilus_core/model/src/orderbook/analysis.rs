...
/// order book levels.
#[must_use]
pub fn get_avg_px_qty_for_exposure(target_exposure: Quantity, levels: &BTreeMap<BookPrice, Level>) -> (f64, f64, f64) {
    let mut cumulative_exposure = 0.0;
    let mut cumulative_size_raw = 0u64;
    let mut final_price = levels.first_key_value()
    ...
    for (book_price, level) in levels {
        ...
        let exposure_this_level = level_exposure.min(target_exposure.raw as f64 - cumulative_exposure);
