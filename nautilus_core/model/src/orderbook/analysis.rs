...
/// order book levels.
#[must_use]
pub fn get_avg_px_qty_for_exposure(target_exposure: Quantity, levels: &BTreeMap<BookPrice, Level>) -> (f64, f64) {
    ...
    for (book_price, level) in levels {
        ...
        let exposure_this_level = level_exposure.min(target_exposure.raw - cumulative_exposure);
        let size_this_level = (exposure_this_level / price).floor() as u64;

        cumulative_exposure += price * size_this_level as f64;
        cumulative_size_raw += size_this_level;

        if cumulative_exposure >= target_exposure {
            break;
        }
    }
