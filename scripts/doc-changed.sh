...
select_rust_inputs() {
  while IFS= read -r file; do
    case "$file" in
      *.rs | Cargo.toml | */Cargo.toml | Cargo.lock | rust-toolchain.toml | python/pyproject.toml | .cargo/config.toml)
        printf '%s\n' "$file"
        ;;
    esac
  done
...
if echo "$changed_files" | grep -qE '^(Cargo\.toml|Cargo\.lock|rust-toolchain\.toml|\.cargo/config\.toml)'; then
  run_full
fi
