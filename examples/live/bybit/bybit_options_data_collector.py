...
class BybitOptionsDataCollector(Strategy):
    ...
    def _setup_file_logging(self) -> None:
        """Setup file logging to save logs alongside data."""
        ...
        # Add handler to the strategy's logger
        ...
        # Log the setup
        self.log.info(f"File logging enabled: {log_filepath}")
    ...
    def _initialize_spot_data_storage(self) -> None:
        ...
        self.quote_ticks_files[spot_key] = os.path.join(self.spot_data_dir, f"{spot_name}_quote.parquet")
        self.order_book_deltas_files[spot_key] = os.path.join(self.spot_data_dir, f"{spot_name}_orderbook.parquet")
    ...
    def _discover_options(self) -> None:
        ...
        # Get all instruments and filter for options of the underlying asset
        all_instruments = self.cache.instruments()
        self.log.info(f"Total instruments loaded: {len(all_instruments)}")
        
        # Filter for options of the specified underlying asset
        option_instruments = [
            instrument for instrument in all_instruments 
            if (instrument.id.venue == BYBIT_VENUE and 
                "-OPTION" in str(instrument.id) and
                self.underlying_asset in str(instrument.id))
        ]
        
        self.log.info(f"Found {len(option_instruments)} {self.underlying_asset} options instruments")
        
        if len(option_instruments) == 0:
            self.log.warning(f"No {self.underlying_asset} options found in cache")
            # Let's also log all option instruments to see what's available
            all_option_instruments = [
                instrument for instrument in all_instruments 
                if instrument.id.venue == BYBIT_VENUE and "-OPTION" in str(instrument.id)
            ]
            self.log.info(f"Total options instruments found: {len(all_option_instruments)}")
            if len(all_option_instruments) > 0:
                self.log.info("Available option instruments:")
                for i, instrument in enumerate(all_option_instruments[:20]):  # Show first 20
                    self.log.info(f"  {i+1}. {instrument.id}")
                if len(all_option_instruments) > 20:
                    self.log.info(f"  ... and {len(all_option_instruments) - 20} more")
            return
        
        # Store discovered options
        self.discovered_options = [instrument.id for instrument in option_instruments]
        
        # Sort options by symbol for better logging
        sorted_options = sorted(option_instruments, key=lambda x: str(x.id))
        ...
        # Log expiry groups
        self.log.info(f"{self.underlying_asset} options grouped by expiry ({len(expiry_groups)} expiry dates):")
        for expiry in sorted(expiry_groups.keys()):
            count = len(expiry_groups[expiry])
            self.log.info(f"  {expiry}: {count} options")
        
        # Log summary instead of listing all options
        self.log.info(f"Total {self.underlying_asset} options to monitor: {len(self.discovered_options)}")
        
        # Show detailed breakdown only if verbose logging is enabled
        if self.config.verbose_logging:
            self.log.info(f"First 10 {self.underlying_asset} options instruments:")
            for i, instrument in enumerate(sorted_options[:10]):
                self.log.info(f"  {i+1:3d}. {instrument.id}")
            if len(sorted_options) > 10:
                self.log.info(f"  ... and {len(sorted_options) - 10} more")
    ...
    def _initialize_options_data_storage(self) -> None:
        ...
        for instrument_id in self.discovered_options:
            ...
            self.quote_ticks_files[instrument_key] = os.path.join(self.options_data_dir, f"{instrument_name}_quote.parquet")
            self.order_book_deltas_files[instrument_key] = os.path.join(self.options_data_dir, f"{instrument_name}_orderbook.parquet")
        
        # Log summary of options data files
        options_count = len([k for k in self.quote_ticks_files.keys() if k != str(self.config.spot_instrument_id)])
        ...
        if self.config.verbose_logging:
            ...
            for instrument_key in self.quote_ticks_files:
                if instrument_key != str(self.config.spot_instrument_id):
                    ...
                    self.log.info(f"  Order book deltas: {self.order_book_deltas_files[instrument_key]}")
    ...
    def _subscribe_to_data_streams(self) -> None:
        ...
        # Subscribe to order book deltas for all options instruments
        for instrument_id in self.discovered_options:
        ...
        self.log.info(f"Monitoring {len(self.discovered_options)} options across {len(expiry_groups)} maturities:")
    ...
    def _check_and_log_data(self) -> None:
        ...
        if current_time - self.last_data_time > 120:  # 2 minutes
            ...
            if self.connection_warnings <= 3:  # Only warn first 3 times
                self.log.warning(f"No data received for {int(current_time - self.last_data_time)} seconds - possible connection issue")
    ...
    def _save_all_data_to_parquet(self) -> None:
        ...
        if total_quotes_saved > 0 or total_deltas_saved > 0:
            self.log.info(f"Data saved: {total_quotes_saved} quotes ({instruments_with_quotes} instruments), {total_deltas_saved} deltas ({instruments_with_deltas} instruments)")
                
    def _append_to_parquet_file(self, data: List[Dict], filepath: str, data_type: str, instrument_key: str) -> None:
        ...
        if os.path.exists(filepath):
            try:
                ...
                if df_existing.empty and not df_new.empty:
                    # If existing file is empty, just save the new data
                    df_new.to_parquet(filepath, index=False)
                elif not df_existing.empty and df_new.empty:
                    # If new data is empty, keep existing file
                    pass
                else:
        ...
        # Clear the data from memory for this specific instrument
    ...
    def on_stop(self) -> None:
        ...
        self.log.info(f"Active instruments: {active_options}/{len(self.discovered_options)} options, 1/1 spot")
        self.log.info(f"Total data processed:")
        self.log.info(f"  Options: {total_options_quotes} quotes, {total_options_deltas} deltas")
        self.log.info(f"  Spot: {self.spot_quote_count} quotes, {self.instrument_delta_counts.get(str(self.config.spot_instrument_id), 0)} deltas")
    ...
    def save_logs_manually(self, log_message: str, level: str = "INFO") -> None:
        """
        Manually save a log message to the file.
        ...
        if hasattr(self, 'file_handler') and self.file_handler:
            ...
            # Fallback to console logging if file logging not enabled
            self.log.info(f"[MANUAL] {log_message}")
    ...
    def rotate_log_file(self) -> Optional[str]:
        """
...
def main():
    """Main function to run the Bybit options data collector."""
    ...
    config_node = TradingNodeConfig(
        ...
        logging=LoggingConfig(
            log_level="INFO",  # Allow INFO logs for strategy while suppressing some data client noise
            use_pyo3=True,
            log_colors=True,
        ),
        data_clients={
    ...
    strategy_config = BybitOptionsDataCollectorConfig(
        underlying_asset="BTC",  # Collect data for BTC options
    ...
    # strategy.save_logs_manually("Custom log message", "INFO")
    # strategy.rotate_log_file()  # Rotate logs if needed
    # log_path = strategy.get_log_filepath()  # Get current log file path
    ...
    if __name__ == "__main__":
        try:
            print(f"Starting Bybit Options Data Collector")
