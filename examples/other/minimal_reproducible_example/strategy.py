...
class DemoStrategy(Strategy):
    ...
    def on_bar(self, bar: Bar):
        ...
        self.log.info(f"Processed bars: {self.bars_processed}", color=LogColor.BLUE)
        self.log.info(f"Bars: {bar} | Time: {unix_nanos_to_dt(bar.ts_event)}", color=LogColor.BLUE)
        ...
        if not self.order_placed and self.bars_processed == 3:
            ...
            self.log.info(f"Market order placed at {bar.close}", color=LogColor.YELLOW)

        # Exit: BUY MARKET order (at 8th bar)
