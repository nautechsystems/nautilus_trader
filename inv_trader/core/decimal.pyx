...
from inv_trader.core.decimal cimport ROUNDING, rounding_string
...
cdef class Decimal:
    ...
    * [+|-].<frac>[<e|E>[+|-]<exp>].
    ...
    def __round__(self, precision=None):
        ...
        if precision is None:
            # return integer
            return int(self.adjusted(0, ROUNDING.ROUND_HALF_EVEN))
        # otherwise return Decimal
        return self.adjusted(precision, ROUNDING.ROUND_HALF_EVEN)
...
cpdef ROUNDING get_rounding():
    """
    :return: The rounding mode from current decimal context.
    """
    ctx = _getcontext()
...
cpdef void set_rounding(ROUNDING rounding):
    ...
    """
    ctx = _getcontext()
    ctx.rounding = rounding_string(rounding)
