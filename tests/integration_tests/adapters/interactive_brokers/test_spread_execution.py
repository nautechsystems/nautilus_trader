...
# Test for basic functionality - streamlined approach
def test_streamlined_spread_execution_integration():
    """
    Test that the streamlined spread execution integration works.
    """
    from nautilus_trader.adapters.interactive_brokers.execution import (
        InteractiveBrokersExecutionClient,
    )

    # Test that the execution client has the required methods
    methods = dir(InteractiveBrokersExecutionClient)

    # Check for spread-related methods
    assert "_handle_spread_execution" in methods
    assert "_generate_combo_fill" in methods
    assert "_generate_leg_fill" in methods
    assert "_extract_leg_instrument_id" in methods
    assert "generate_order_filled" in methods

    print("✅ All streamlined spread execution methods present")


def test_spread_detection_functionality():
    """
    Test spread detection logic.
    """
    from nautilus_trader.model.identifiers import InstrumentId

    def is_spread_instrument(instrument_id):
        id_str = str(instrument_id)
        return "_(" in id_str or ")_" in id_str or "SPREAD" in id_str.upper()

    # Test spread instruments
    spread_id = InstrumentId.from_str("(1)SPY C400_((1))SPY C410.SMART")
    assert is_spread_instrument(spread_id) is True

    # Test single instruments
    single_id = InstrumentId.from_str("SPY C400.SMART")
    assert is_spread_instrument(single_id) is False

    print("✅ Spread detection working correctly")
