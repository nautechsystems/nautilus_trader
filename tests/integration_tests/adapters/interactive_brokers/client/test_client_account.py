...
async def test_process_account_id(ib_client):
    ...
    # Test the process_managed_accounts method directly
    await ib_client.process_managed_accounts(accounts_list="DU1234567")

    # Assert that the account ID is properly set
    assert "DU1234567" in ib_client.accounts()

    # Test with multiple accounts
    await ib_client.process_managed_accounts(accounts_list="DU1234567,DU7654321")

    # Assert that both account IDs are set
    accounts = ib_client.accounts()
    assert "DU1234567" in accounts
    assert "DU7654321" in accounts
