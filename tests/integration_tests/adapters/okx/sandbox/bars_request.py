...
async def main():
    ...
    # Request bars (with no parameters 300 bars should be received)
    logger.info(f"Requesting bars for {bar_type}...")
    ...
    logger.info(f"Received {len(bars)} bar(s)")

    if bars:
        logger.info(f"bars[0]  = {bars[0]}")
        logger.info(f"bars[-1] = {bars[-1]}")
