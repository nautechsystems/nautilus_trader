...
class TestConsolidateDataByPeriod:
    ...
    def test_consolidate_predicate_exact_boundary_cases(self):
        """
        Validates deletion predicate handles exact boundary timestamps correctly.
        Each case is isolated by resetting the catalog between runs.
        ...
        def consolidate():
            self.catalog.consolidate_data_by_period(
                data_cls=Bar, identifier=bar_type_str, period=pd.Timedelta(days=1),
