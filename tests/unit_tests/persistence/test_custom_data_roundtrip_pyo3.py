...
#[pymodule]
fn market_tick_data(_py: Python<'_>, m: &Bound<'_, pyo3::types::PyModule>) -> PyResult<()> {{
    m.add_class::<MarketTickData>()?;
    Ok(())
}}
"""
...
def test_rustimport_custom_data_roundtrip(tmp_path):
    """Test custom data defined via rustimport - compile Rust at runtime and roundtrip via catalog."""
    import os

    import rustimport

    # rustimport compiles in debug mode by default (faster compile; do not set
    # RUSTIMPORT_RELEASE_BINARIES so the first run compiles quickly).
    # Use a fixed path for the .rs file and a cache dir *outside* the repo so
    # Cargo does not treat the build as part of the nautilus_trader workspace.
    test_dir = Path(__file__).resolve().parent
    rustimport_builds = test_dir / "rustimport_builds"
    rustimport_builds.mkdir(parents=True, exist_ok=True)
    cache_dir = Path.home() / ".cache" / "nautilus_rustimport"
    cache_dir.mkdir(parents=True, exist_ok=True)
    rustimport.settings.cache_dir = str(cache_dir)

    # macOS: resolve Python symbols at load time when linking the extension.
    if os.environ.get("RUSTFLAGS") is None:
        os.environ["RUSTFLAGS"] = "-C link-arg=-undefined -C link-arg=dynamic_lookup"
    nautilus_root = Path(__file__).resolve().parents[3]
    rust_content = _RUSTIMPORT_MARKET_TICK_RS.format(nautilus_root=nautilus_root)
    rs_path = rustimport_builds / "market_tick_data.rs"
    rs_path.write_text(rust_content, encoding="utf-8")

    module = rustimport.imp_from_path(str(rs_path), fullname="market_tick_data", opt_in=False)
    ...
    MarketTickData = module.MarketTickData
    register_custom_data_class(MarketTickData)

    metadata = {"source": "rustimport", "version": "1"}
    identifier = "rustimport-1"
    data_type = DataType("MarketTickData", metadata, identifier)
    original_data = [
        MarketTickData("AAPL", 150.5, 1000, 1, 1),
        MarketTickData("GOOGL", 2800.0, 500, 2, 2),
    ]
    wrapped = [CustomData(data_type, item) for item in original_data]
    assert [custom_data_backend_kind(item) for item in wrapped] == ["external", "external"]

    catalog_path = tmp_path / "catalog_rustimport"
    catalog_path.mkdir(parents=True, exist_ok=True)
    pyo3_catalog = ParquetDataCatalogV2(str(catalog_path))
    ...
    for item in result:
        assert isinstance(item, CustomData), f"Expected CustomData, found {type(item)}"
        assert custom_data_backend_kind(item) == "external"
        inner = item.data
        assert isinstance(inner, MarketTickData), (
            f"Expected MarketTickData in .data, found {type(inner)}"
        )
        roundtripped.append(inner)
    ...
    # DataType (type_name, metadata, identifier) should be restored on catalog decode
    for item in result:
        assert item.data_type.type_name == "MarketTickData"
        assert item.data_type.metadata == metadata
        assert item.data_type.identifier == identifier
    ...
    # JSON roundtrip: CustomData -> JSON bytes -> CustomData.from_json_bytes
    def assert_market_tick_data(orig, rt):
        assert orig.symbol == rt.symbol
        assert orig.price == rt.price
        assert orig.volume == rt.volume
        assert orig.ts_event == rt.ts_event
    ...
    _assert_custom_data_json_roundtrip(result, MarketTickData, assert_market_tick_data)
