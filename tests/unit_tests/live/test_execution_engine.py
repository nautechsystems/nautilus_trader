...
class TestLiveExecutionEngine:
    ...
    def test_should_cleanup_stale_startup_netting_positions_uses_missing_refs_for_accountless_reports(
        self,
    ):
        stale_position_order = self.order_factory.market(
            instrument_id=AUDUSD_SIM.id,
        ...
        stale_position_order.apply(
            TestEventStubs.order_accepted(
                stale_position_order,
                account_id=self.client.account_id,
                venue_order_id=VenueOrderId("V-STARTUP-POSITION-ACCOUNTLESS-001"),
            ),
        )
        stale_position_fill = TestEventStubs.order_filled(
            stale_position_order,
            instrument=AUDUSD_SIM,
            account_id=self.client.account_id,
            position_id=PositionId("P-STARTUP-ACCOUNTLESS-001"),
            last_qty=Quantity.from_int(100),
            last_px=Price.from_str("1.00000"),
            trade_id=TradeId("T-STARTUP-ACCOUNTLESS-001"),
        )
        stale_position_order.apply(stale_position_fill)
        self.cache.update_order(stale_position_order)
        stale_position = Position(instrument=AUDUSD_SIM, fill=stale_position_fill)
        ...
        startup_open_order.apply(
            TestEventStubs.order_accepted(
                startup_open_order,
                account_id=self.client.account_id,
                venue_order_id=VenueOrderId("V-STARTUP-MISSING-ACCOUNTLESS-001"),
            ),
        )
        self.cache.update_order(startup_open_order)
        ...
        self.exec_engine._startup_reconciliation_snapshot = {
            (
                ...
                instrument_id=AUDUSD_SIM.id,
                strategy_id=StrategyId("EXTERNAL"),
                open_position_ids=(stale_position.id,),
                open_position_qty=Decimal("100"),
                open_order_refs=(startup_ref,),
    ...
    async def test_reconcile_execution_state_filters_follow_up_positions_to_scoped_instruments(
        ...
        self.exec_engine.reconciliation_instrument_ids = [AUDUSD_SIM.id]
        original_generate_mass_status = self.client.generate_mass_status
        original_generate_order_status_reports = self.client.generate_order_status_reports
        original_generate_fill_reports = self.client.generate_fill_reports
        original_generate_position_status_reports = self.client.generate_position_status_reports

        self.client.generate_mass_status = _unexpected_generate_mass_status
        self.client.generate_order_status_reports = _generate_order_status_reports
        self.client.generate_fill_reports = _generate_fill_reports
        self.client.generate_position_status_reports = _generate_position_status_reports
        self.exec_engine._reconcile_execution_mass_status = (
            lambda _mass_status, allow_startup_external_cleanup=False: True
        )
        self.exec_engine._startup_reconciliation_event.clear()
        ...
        try:
            ...
            self.client.generate_mass_status = original_generate_mass_status
            self.client.generate_order_status_reports = original_generate_order_status_reports
            self.client.generate_fill_reports = original_generate_fill_reports
            self.client.generate_position_status_reports = original_generate_position_status_reports

        assert result is True
        assert requested_position_reports.count(AUDUSD_SIM.id) >= 1
        assert GBPUSD_SIM.id not in requested_position_reports
        assert self.exec_engine._startup_reconciliation_event.is_set() is True
    ...
    @pytest.mark.asyncio
    async def test_generate_startup_mass_status_synthesizes_flat_report_for_empty_netting_query(
        self,
    ):
        self.exec_engine.reconciliation_instrument_ids = [AUDUSD_SIM.id]
        ...
        mass_status = await self.exec_engine._generate_startup_mass_status(netting_client, 60)
        ...
        assert position_reports[0].venue_position_id is None

    @pytest.mark.asyncio
    async def test_reconcile_execution_state_follow_up_synthesizes_flat_once_per_instrument(
        self,
    ):
        requested_position_reports: list[InstrumentId | None] = []
        reconciled_mass_statuses: list[ExecutionMassStatus] = []
        netting_client = MockLiveExecutionClient(
            loop=self.loop,
            client_id=self.client.id,
            venue=SIM,
        ...
        _add_open_position(AUDUSD_SIM, suffix="AUD-1")
        _add_open_position(AUDUSD_SIM, suffix="AUD-2")
        ...
        netting_client.generate_order_status_reports = _generate_order_status_reports
        netting_client.generate_fill_reports = _generate_fill_reports
        netting_client.generate_position_status_reports = _generate_position_status_reports
        self.exec_engine._clients.clear()
        self.exec_engine._clients[netting_client.id] = netting_client
        self.exec_engine._reconcile_execution_mass_status = (
            lambda mass_status, allow_startup_external_cleanup=False: (
                reconciled_mass_statuses.append(mass_status) or True
            )
        )
        self.exec_engine._startup_reconciliation_event.clear()
        ...
        assert len(reconciled_mass_statuses) == 1
        position_reports = reconciled_mass_statuses[0].position_reports[AUDUSD_SIM.id]
        assert len(position_reports) == 1
