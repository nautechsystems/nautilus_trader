...
class TestDataBufferEngine:
    ...
    def test_subscribe_bars_passthrough_for_matching_spec(self):
        # Arrange
        self.data_engine.register_client(self.binance_client)
        instrument = TestInstrumentProvider.default_fx_ccy("AUD/USD", venue=BINANCE)
        self.cache.add_instrument(instrument)
        handler = []

        # Create composite bar type with matching step and aggregation
        bar_spec = BarSpecification(1, BarAggregation.MINUTE, PriceType.LAST)
        bar_type = BarType.new_composite(
            instrument.id,
            bar_spec,
            AggregationSource.INTERNAL,
            bar_spec.step,  # Same step
            bar_spec.aggregation,  # Same aggregation
            AggregationSource.EXTERNAL,
        )

        # Subscribe handler to receive bars via msgbus
        ...
        # Create a test bar with revision flag
        composite_bar = Bar(
            bar_type=bar_type.composite(),
        ...
        # Act - process the bar
        self.data_engine.process(composite_bar)

        # Assert - passthrough should emit immediately with correct bar_type and preserved is_revision
        assert len(handler) == 1
        assert handler[0].bar_type == bar_type.standard()
        assert handler[0].is_revision is True
        assert handler[0].close == Price.from_str("1.00007")

    def test_subscribe_bars_no_passthrough_for_different_spec(self):
        # Arrange
        self.data_engine.register_client(self.binance_client)
        instrument = TestInstrumentProvider.default_fx_ccy("AUD/USD", venue=BINANCE)
        self.cache.add_instrument(instrument)
        ...
        # Create composite bar type with different step
        ...
        # Assert - no passthrough, bar should be aggregated not immediately emitted
        assert len(handler) == 0
