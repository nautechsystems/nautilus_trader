...
class TestModelEvents:
    ...
    def test_order_event_dicts_do_not_expose_causation_id(self):
        for event_type, event in self._order_event_dict_cases():
            assert "causation_id" not in event_type.to_dict(event)

    def _order_event_dict_cases(self):
        trader_id = TraderId("TRADER-001")
        strategy_id = StrategyId("SCALPER-001")
        instrument_id = InstrumentId(Symbol("BTCUSDT"), Venue("BINANCE"))
        client_order_id = ClientOrderId("O-2020872378423")
