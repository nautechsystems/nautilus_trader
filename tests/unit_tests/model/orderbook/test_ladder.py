...
def test_dict():
    orders = [Order(price=103, volume=1, side=OrderSide.BUY, order_id="a"), Order(price=102, volume=1, side=OrderSide.BUY, order_id="b")]
    ladder = Ladder.from_orders(orders=orders)
    result = ladder.dict()
    expected = {
        "exchange_order_ids": False,
        "levels": [
            {"orders": [{"order_id": "b", "price": 102.0, "side": OrderSide.BUY, "volume": 1.0}], "price": 102.0, "side": BID},
            {"orders": [{"order_id": "a", "price": 103.0, "side": OrderSide.BUY, "volume": 1.0}], "price": 103.0, "side": BID},
        ],
        "reverse": True,
        "side": OrderSide.BUY,
    }
...
def test_serialization():
    ladder = Ladder.from_orders([Order(price=103, volume=1, side=OrderSide.BUY), Order(price=102, volume=1, side=OrderSide.BUY)])
    raw = ladder.dumps()
    assert len(raw) == 673
