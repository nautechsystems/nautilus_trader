...
from nautilus_trader.core.nautilus_pyo3 import CryptoFuture as CryptoFuturePyo3
...
def test_to_dict():
    ...
    assert CryptoFuturePyo3.from_dict(result) == _BTCUSDT_FUTURE
    assert result == {
        "type": "CryptoFuture",
...
def test_pyo3_cython_conversion():
    ...
    crypto_future_cython = CryptoFuture.from_dict(crypto_future_pyo3_dict)
    crypto_future_cython_dict = CryptoFuture.to_dict(crypto_future_cython)
    crypto_future_pyo3_back = CryptoFuturePyo3.from_dict(crypto_future_cython_dict)
