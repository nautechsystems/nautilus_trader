...
class ExecutionEngineTests(unittest.TestCase):
    ...
    def test_is_flat_when_strategy_registered_returns_true(self):
        ...
        # Assert
        self.assertTrue(self.exec_engine.is_flat(strategy_id=strategy.id))
    ...
    def test_handle_order_fill_event(self):
        ...
        self.assertFalse(self.exec_engine.is_flat(strategy_id=strategy.id))
        self.assertFalse(self.exec_engine.is_flat())
        self.assertEqual(Position, type(self.exec_db.get_position(expected_position_id)))
        self.assertTrue(expected_position_id in self.exec_db.get_position_ids())
        self.assertTrue(expected_position_id not in self.exec_db.get_position_closed_ids(strategy_id=strategy.id))
        self.assertTrue(expected_position_id not in self.exec_db.get_position_closed_ids())
        self.assertTrue(expected_position_id in self.exec_db.get_position_open_ids(strategy_id=strategy.id))
        self.assertTrue(expected_position_id in self.exec_db.get_position_open_ids())
    ...
    def test_handle_position_opening_with_position_id_none(self):
        ...
        self.assertTrue(expected_id in self.exec_db.get_position_ids())
        self.assertTrue(expected_id not in self.exec_db.get_position_closed_ids(strategy_id=strategy.id))
        self.assertTrue(expected_id not in self.exec_db.get_position_closed_ids())
        self.assertTrue(expected_id in self.exec_db.get_position_open_ids(strategy_id=strategy.id))
        self.assertTrue(expected_id in self.exec_db.get_position_open_ids())
        self.assertEqual(1, self.exec_db.positions_total_count())
        self.assertEqual(1, self.exec_db.positions_open_count())
        self.assertEqual(0, self.exec_db.positions_closed_count())
        self.assertTrue(self.exec_db.position_exists_for_order(order.cl_ord_id))
    ...
    def test_add_to_existing_position_on_order_fill(self):
        ...
        self.assertFalse(self.exec_engine.is_flat())
        self.assertEqual(Position, type(self.exec_db.get_position(expected_position_id)))
        self.assertEqual(0, len(self.exec_db.get_positions_closed(strategy_id=strategy.id)))
        self.assertEqual(0, len(self.exec_db.get_positions_closed()))
        self.assertEqual(1, len(self.exec_db.get_positions_open(strategy_id=strategy.id)))
    ...
    def test_close_position_on_order_fill(self):
        ...
        self.assertEqual(position_id, self.exec_db.get_positions(strategy_id=strategy.id)[0].id)
        self.assertEqual(position_id, self.exec_db.get_positions()[0].id)
        self.assertEqual(0, len(self.exec_db.get_positions_open(strategy_id=strategy.id)))
        self.assertEqual(0, len(self.exec_db.get_positions_open()))
        self.assertEqual(position_id, self.exec_db.get_positions_closed(strategy_id=strategy.id)[0].id)
        self.assertEqual(position_id, self.exec_db.get_positions_closed()[0].id)
        self.assertTrue(position_id not in self.exec_db.get_position_open_ids(strategy_id=strategy.id))
        self.assertTrue(position_id not in self.exec_db.get_position_open_ids())
        self.assertEqual(1, self.exec_db.positions_total_count())
    ...
    def test_multiple_strategy_positions_opened(self):
        ...
        self.assertFalse(self.exec_engine.is_flat(strategy_id=strategy1.id))
        self.assertFalse(self.exec_engine.is_flat(strategy_id=strategy2.id))
        self.assertFalse(self.exec_engine.is_flat())
        ...
        self.assertTrue(position1_id in self.exec_db.get_position_ids(strategy_id=strategy1.id))
        self.assertTrue(position2_id in self.exec_db.get_position_ids(strategy_id=strategy2.id))
        self.assertTrue(position1_id in self.exec_db.get_position_ids())
        self.assertTrue(position2_id in self.exec_db.get_position_ids())
        self.assertEqual(2, len(self.exec_db.get_position_open_ids()))
        self.assertEqual(1, len(self.exec_db.get_positions_open(strategy_id=strategy1.id)))
        self.assertEqual(1, len(self.exec_db.get_positions_open(strategy_id=strategy2.id)))
        self.assertEqual(1, len(self.exec_db.get_positions_open(strategy_id=strategy2.id)))
        self.assertEqual(2, len(self.exec_db.get_positions_open()))
        self.assertEqual(1, len(self.exec_db.get_positions_open(strategy_id=strategy1.id)))
        self.assertEqual(1, len(self.exec_db.get_positions_open(strategy_id=strategy2.id)))
        self.assertTrue(position1_id in self.exec_db.get_position_open_ids(strategy_id=strategy1.id))
        self.assertTrue(position2_id in self.exec_db.get_position_open_ids(strategy_id=strategy2.id))
        self.assertTrue(position1_id in self.exec_db.get_position_open_ids())
        self.assertTrue(position2_id in self.exec_db.get_position_open_ids())
        self.assertTrue(position1_id not in self.exec_db.get_position_closed_ids(strategy_id=strategy1.id))
        self.assertTrue(position2_id not in self.exec_db.get_position_closed_ids(strategy_id=strategy2.id))
        self.assertTrue(position1_id not in self.exec_db.get_position_closed_ids())
        self.assertTrue(position2_id not in self.exec_db.get_position_closed_ids())
    ...
    def test_multiple_strategy_positions_one_active_one_closed(self):
        ...
        self.assertTrue(self.exec_engine.is_flat(strategy_id=strategy1.id))
        self.assertFalse(self.exec_engine.is_flat(strategy_id=strategy2.id))
        self.assertFalse(self.exec_engine.is_flat())
        self.assertTrue(position_id1 in self.exec_db.get_position_ids(strategy_id=strategy1.id))
        self.assertTrue(position_id2 in self.exec_db.get_position_ids(strategy_id=strategy2.id))
        self.assertTrue(position_id1 in self.exec_db.get_position_ids())
        self.assertTrue(position_id2 in self.exec_db.get_position_ids())
        self.assertEqual(0, len(self.exec_db.get_positions_open(strategy_id=strategy1.id)))
        self.assertEqual(1, len(self.exec_db.get_positions_open(strategy_id=strategy2.id)))
        self.assertEqual(1, len(self.exec_db.get_positions_open()))
        self.assertEqual(1, len(self.exec_db.get_positions_closed()))
        self.assertEqual(2, len(self.exec_db.get_positions()))
        self.assertTrue(position_id1 not in self.exec_db.get_position_open_ids(strategy_id=strategy1.id))
        self.assertTrue(position_id2 in self.exec_db.get_position_open_ids(strategy_id=strategy2.id))
        self.assertTrue(position_id1 not in self.exec_db.get_position_open_ids())
        self.assertTrue(position_id2 in self.exec_db.get_position_open_ids())
        self.assertTrue(position_id1 in self.exec_db.get_position_closed_ids(strategy_id=strategy1.id))
        self.assertTrue(position_id2 not in self.exec_db.get_position_closed_ids(strategy_id=strategy2.id))
        self.assertTrue(position_id1 in self.exec_db.get_position_closed_ids())
        self.assertTrue(position_id2 not in self.exec_db.get_position_closed_ids())
        self.assertEqual(2, self.exec_db.positions_total_count())
        self.assertEqual(1, self.exec_db.positions_open_count())
        self.assertEqual(1, self.exec_db.positions_closed_count())
