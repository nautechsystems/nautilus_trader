...
class ExecutionCacheTests(unittest.TestCase):
    ...
    def test_update_order_for_accepted_order(self):
        ...
        self.assertIn(order, self.cache.orders_active(strategy_id=self.strategy.id))
        self.assertIn(order, self.cache.orders_active(symbol=order.symbol, strategy_id=self.strategy.id))
    ...
    def test_update_order_for_completed_order(self):
        ...
        self.assertNotIn(order, self.cache.orders_active())
        self.assertNotIn(order, self.cache.orders_active(symbol=order.symbol))
        self.assertNotIn(order, self.cache.orders_active(strategy_id=self.strategy.id))
        self.assertNotIn(order, self.cache.orders_active(symbol=order.symbol, strategy_id=self.strategy.id))
        ...
        self.assertEqual(0, self.cache.orders_active_count())
