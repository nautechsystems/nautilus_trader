...
class TestBacktestNodeWithBacktestDataIterator:
    ...
    # TODO: use below with log_path fixtures in backtest/conftest.py for quick log comparisons
    ...
    def test_backtest_logs_same_with_and_without_data_configs(self, log_path) -> None:
        ...
        # Act
        run_backtest(with_data=True, log_path=str(log_path))
        shutil.copy(log_file, log_file_with_data)

        run_backtest(with_data=False, log_path=str(log_path))
        shutil.copy(log_file, log_file_without_data)

        assert compare_files(log_file_with_data, log_file_without_data)
...
def compare_files(file_1, file_2):
    ...
    with open(file_1) as f1, open(file_2) as f2:
        ...
        if len(lines1) != len(lines2):
            print(f"\nFile 1 has {len(lines1)} lines")
            print(f"File 2 has {len(lines2)} lines")
            print(
                (
                    "\nExtra lines in file 1:"
                    if len(lines1) > len(lines2)
                    else "\nExtra lines in file 2:"
                ),
            )

            for line in (
                lines1[len(lines2) :] if len(lines1) > len(lines2) else lines2[len(lines1) :]
            ):
                print(line.rstrip())

    # assert the content of the two log files is the same
    return filecmp.cmp(str(file_1), str(file_2))
