...
Use the public facade by default: `clock()`, `cache()`, `order()`,
`portfolio()`, and the order management methods on `Strategy`.
...
as `cache_ref()`, `cache_rc()`, `clock_mut()`, `order_factory()`,
`order_factory_rc()`, `order_manager()`, and `portfolio_rc()`. These types do
not cross Python or plug-in boundaries. Once the trait is in scope, call the
native methods on the strategy value, not through its `core` field.
