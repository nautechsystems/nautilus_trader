...
| --- | --- | --- | --- |
| Overall | completed | main | Tasks 1-4 completed. A late quality review reopened Task 3 for freshness semantics; the follow-up fix now uses projection `server_ts_ms` for shared-source freshness, keeps row timestamps only for local-activity ordering, and live Makerv3 equities nodes again show matching shared Hyperliquid maker positions in Signal truth without polluting unrelated strategies. |
...
| Task 3: Reconcile Exact Maker Positions In Makerv3 | completed | main | Initial completion was reopened by a late quality review that identified two real edge cases: borrowed shared inventory freshness was tied to republish time, and older shared rows could resurrect pre-activity local state. Follow-up fixes are green locally (`10 passed` across the shared projection + Makerv3 reconciliation slice) and now use projection `server_ts_ms` for freshness, propagate that timestamp into published portfolio components, and reject shared rows older than local maker activity. |
| Task 4: Verify Live Signal Truth And Update Trackers | completed | main | Restarted all 10 retained equities Makerv3 nodes with `sudo systemctl restart ...` twice: once for the original reconciliation rollout and once after the late-review freshness follow-up. Final live `/api/v1/signals?profile=equities` shows `googl_tradexyz_makerv3 local_qty_base=-6`, `nvda_tradexyz_makerv3 local_qty_base=-9.111`, both with `local_inventory_source=shared_account_projection`, while `tsla_tradexyz_makerv3` stays flat at `0`. `/api/v1/balances?profile=equities` still serves the shared HL position rows for `NVDA`, `COIN`, and `GOOGL`, and no equities node units are failed. |
