...
| Task 3: `OrderActionPersistenceActor` | DONE | Added orders actor/config with selected event filtering, enqueue-only hot path, bounded batch flush, threaded durability barrier semantics, and strict/non-strict backlog shutdown tests. Follow-up hardened stop-timeout lifecycle consistency (non-strict async writer ref cleanup + error signaling), clarified enqueue-only hot-path intent, and added coverage for `log_and_drop` write-failure + OrderInitialized tag parsing edge cases. Later follow-up finalized strict stop-timeout deterministic ref cleanup + guaranteed finalization gating and strict timeout cleanup/restart tests. Latest follow-up fixes timeout-cleanup drain behavior for backlog > batch (do not detach connection early), ensures cleanup completion signaling, and adds backlog regression coverage. Refs: `241adae5f`, `484d7b539`, `97ae7b629`, `4a2115c1d`, `71b53646c` | `241adae5f`, `484d7b539`, `97ae7b629`, `4a2115c1d`, `71b53646c` |
