...
| Task 6: Add the first formal `equities-pilot` lane | in_progress | main | Task 3, Task 5 | `ops/scripts/deploy/install_equities_systemd.sh`, `deploy/equities/README.md`, `docs/runbooks/equities-pilot-rollout.md`, `/etc/flux/equities-pilot*.env`, `~/releases/pilot/equities`, `tests/unit_tests/ops/deploy/test_install_equities_systemd.py`, `tests/unit_tests/flux/pulse/test_api.py` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | none | `bash -n PASS; . .venv/bin/activate && pytest -q tests/unit_tests/ops/deploy/test_install_equities_systemd.py tests/unit_tests/flux/pulse/test_api.py PASS; systemctl status 'flux@equities-pilot*' --no-pager PASS (no units yet); curl -fsS http://127.0.0.1:5022/api/pulse/jobs ... PASS (no pilot jobs yet)` | Added repo-side pilot target coverage and operator docs; next step is materializing the pilot release root and `/etc/flux/equities-pilot*.env` on the host without auto-starting the lane. |
