...
| Task 2: Add shared release-root helpers and immutable-root tests | completed | main | Task 1 | `ops/scripts/deploy/shared_strategy_stack.sh`, `ops/scripts/deploy/create_release_root.sh`, `tests/unit_tests/ops/deploy/test_shared_strategy_stack.py`, `tests/unit_tests/ops/deploy/test_create_release_root.py` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | pending | `bash -n PASS; . .venv/bin/activate && pytest -q tests/unit_tests/ops/deploy/test_shared_strategy_stack.py tests/unit_tests/ops/deploy/test_create_release_root.py PASS` | Cleared stale build artifacts, rebuilt the worktree env with `uv sync --all-groups --all-extras`, and verified the helper layer in the worktree venv |
