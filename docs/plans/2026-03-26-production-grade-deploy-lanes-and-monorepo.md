...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Task 1: Write and land the deploy-lane governance contract | completed | main | none | `docs/runbooks/deploy-lanes.md`, `docs/runbooks/equities-pilot-rollout.md`, `AGENTS.md`, `docs/repo/workflows.md`, `ops/README.md` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | `825b00cf07` | `tooling/ci/check-repo-structure.sh PASS; rg deploy to pilot... PASS` | Landed lane governance docs and agent contract updates |
| Task 2: Add shared release-root helpers and immutable-root tests | completed | main | Task 1 | `ops/scripts/deploy/shared_strategy_stack.sh`, `ops/scripts/deploy/create_release_root.sh`, `tests/unit_tests/ops/deploy/test_shared_strategy_stack.py`, `tests/unit_tests/ops/deploy/test_create_release_root.py` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | `1e2ef3fd74` | `bash -n PASS; . .venv/bin/activate && pytest -q tests/unit_tests/ops/deploy/test_shared_strategy_stack.py tests/unit_tests/ops/deploy/test_create_release_root.py PASS` | Cleared stale build artifacts, rebuilt the worktree env with `uv sync --all-groups --all-extras`, and committed the shared immutable release-root helper layer |
| Task 3: Harden equities install flow for immutable roots and lane namespaces | completed | main | Task 2 | `ops/scripts/deploy/install_equities_systemd.sh`, `deploy/equities/README.md`, `deploy/equities/systemd/common.env.example`, `tests/unit_tests/ops/deploy/test_install_equities_systemd.py` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | `317661d5bf` | `bash -n PASS; . .venv/bin/activate && pytest -q tests/unit_tests/ops/deploy/test_install_equities_systemd.py tests/unit_tests/ops/deploy/test_shared_strategy_stack.py PASS; rg immutable release root... PASS` | Equities installer now resolves immutable release roots, preserves existing lane roots on reruns, and renders lane-aware `equities-pilot-*` service/env names |
| Task 4: Canonicalize the shared-host dev repo and worktree layout | completed | main | Task 1 | `/home/ubuntu/nautilus_trader`, `/home/ubuntu/nautilus-trader-dev`, `/home/ubuntu/nautilus_trader_prod`, `/home/ubuntu/.config/superpowers/worktrees/nautilus_trader`, `/home/ubuntu/.worktrees/nautilus_trader`, `docs/runbooks/deploy-lanes.md` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | `4a9492dad4` | `git -C /home/ubuntu/nautilus_trader worktree list PASS; find /home/ubuntu ... PASS; du -sh /home/ubuntu/nautilus_trader /home/ubuntu/nautilus-trader-dev /home/ubuntu/nautilus_trader_prod PASS` | Moved every registered external worktree into `~/nautilus_trader/.worktrees` and archived extra top-level Nautilus clones/backups under `~/archive/` |
| Task 5: Repoint current prod lanes to immutable release roots | completed | main | Task 2, Task 3, Task 4 | `/etc/flux/tokenmm*.env`, `/etc/flux/equities*.env`, `~/releases/prod/tokenmm`, `~/releases/prod/equities`, `deploy/tokenmm/README.md`, `deploy/equities/README.md` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | `c1e9568d05` | `sudo rg -n '^(WORKDIR|PYTHONPATH|CMD)=' /etc/flux/tokenmm*.env /etc/flux/equities*.env PASS; sudo rg -n '/home/ubuntu/nautilus_trader|/\\.worktrees/' /etc/flux/tokenmm*.env /etc/flux/equities*.env PASS; systemctl status flux@tokenmm-api.service flux@equities-api.service PASS; curl -fsS http://127.0.0.1:5022/api/pulse/jobs ... PASS` | Materialized pinned prod release roots under `~/releases/prod/{tokenmm,equities}`, repointed `/etc/flux` and Pulse to those roots, and updated the prod READMEs. `tokenmm-api` is already running from the release root; `equities-api` is still the pre-cutover process until an explicit restart window. |
| Task 6: Add the first formal `equities-pilot` lane | completed | main | Task 3, Task 5 | `ops/scripts/deploy/install_equities_systemd.sh`, `deploy/equities/README.md`, `docs/runbooks/equities-pilot-rollout.md`, `/etc/flux/equities-pilot*.env`, `~/releases/pilot/equities`, `tests/unit_tests/ops/deploy/test_install_equities_systemd.py`, `tests/unit_tests/flux/pulse/test_api.py` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | `68c9f053cd` | `bash -n PASS; . .venv/bin/activate && pytest -q tests/unit_tests/ops/deploy/test_install_equities_systemd.py tests/unit_tests/flux/pulse/test_api.py PASS; sudo rg -n '^(WORKDIR|PYTHONPATH|PORT|EQUITIES_REDIS_DB|PULSE_GROUP_KEY|PULSE_GROUP_LABEL|CMD)=' /etc/flux/equities-pilot*.env PASS; sudo rg -n '/home/ubuntu/nautilus_trader|/\\.worktrees/' /etc/flux/equities-pilot*.env PASS; systemctl status flux-equities-pilot.target flux@equities-pilot-api.service PASS; curl -fsS http://127.0.0.1:5022/api/pulse/jobs ... PASS` | Added lane-aware pilot installer defaults and operator docs, materialized `~/releases/pilot/equities/releases/20260326T144911Z`, and installed `/etc/flux/equities-pilot*.env` plus `flux-equities-pilot.target` without auto-starting the lane. Pulse now discovers `equities-pilot` as its own inactive group. |
| Task 7: Standardize LP and TG-bot installers on the same root policy | completed | main | Task 2 | `ops/scripts/deploy/install_lp_systemd.sh`, `ops/scripts/deploy/install_tg_bots_systemd.sh`, `deploy/lp/README.md`, `deploy/lp/systemd/common.env.example`, `deploy/tg_bots/systemd/common.env.example`, `tests/unit_tests/ops/deploy/test_install_lp_systemd.py`, `tests/unit_tests/ops/deploy/test_install_tg_bots_systemd.py` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | `52f727fd26` | `bash -n ops/scripts/deploy/install_lp_systemd.sh ops/scripts/deploy/install_tg_bots_systemd.sh PASS; . .venv/bin/activate && pytest -q tests/unit_tests/ops/deploy/test_install_lp_systemd.py tests/unit_tests/ops/deploy/test_install_tg_bots_systemd.py tests/unit_tests/ops/deploy/test_shared_strategy_stack.py PASS` | LP and TG bots now resolve immutable deploy roots, reject mutable checkouts, render release-local Python plus `WORKDIR`/`PYTHONPATH`, and the shared common-env docs no longer normalize `~/nautilus-trader` as the live root. Final follow-up made the TG-bot installer honor `FLUX_DEPLOY_TEST_MODE` for source-based tests. |
| Task 8: Finalize monorepo naming and structure follow-ups | completed | main | Task 1, Task 5, Task 6, Task 7 | `README.md`, `docs/repo/structure.md`, `docs/repo/standards.md`, `docs/repo/workflows.md`, `ops/README.md`, `apps/README.md`, `engine/README.md` | `codex/prod-lanes-exec-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/prod-lanes-exec-20260326` | `7f8bb9d361` | `tooling/ci/check-repo-structure.sh PASS; tooling/ci/check-flux-leakage.sh PASS; rg -n "nautilus_trader\\.flux|systems/flux|product/repo/deploy" README.md docs/repo ops/README.md apps/README.md engine/README.md PASS` | Repo docs now make the naming split explicit: `flux` is the product/deploy surface, `flux.*` remains the system namespace under `systems/flux/`, and `nautilus_trader.*` remains the engine/runtime namespace. |
