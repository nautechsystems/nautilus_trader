...
2. FluxAPI backend is the production `flux` API app (`systems/flux/flux/api/*`) with a small TokenMM compatibility layer.
...
All HTTP responses must use the FluxAPI envelope shape (already used in `systems/flux/flux/api/*`):
...
Implement these in the production FluxAPI app (not a deprecated shim):
...
- Create: `systems/flux/flux/api/socketio.py` (or similar)
- Modify: `systems/flux/flux/api/app.py` (wire Socket.IO into app factory)
...
- Modify: `examples/live/makerv3/run_api.py`
...
- [2026-03-04 07:17 UTC] Mono-PR sync: synchronized `fluxboard-tokenmm-migration` with local PR branch, resolved conflicts in `docs/flux/api.md`, `examples/live/makerv3/{README.md,run_api.py}`, `systems/flux/flux/api/{app.py,payloads.py}`, and `tests/unit_tests/flux/api/test_app.py` while preserving host-default expectations and TokenMM contract changes / evidence: preserved `_resolve_bind_host` helper compatibility / next: rerun strategy-side targeted tests.
- [2026-03-04 07:20 UTC] Rebased compatibility verification: strategy/API guard tests passed after conflict resolution / evidence: `pytest -q tests/unit_tests/examples/strategies/test_makerv3_run_api.py tests/unit_tests/examples/strategies/test_makerv3_run_bridge.py --confcutdir=tests/unit_tests/examples/strategies` (9 passed), `pytest -q tests/unit_tests/scripts/test_check_flux_leakage.py --confcutdir=tests/unit_tests/scripts` (4 passed) / note: `tests/unit_tests/analysis/test_tearsheet.py` requires compiled Nautilus modules unavailable in this `/tmp` PYTHONPATH test harness.
- Historical note: references in this section to `test_makerv3_run_*` and
  `scripts/deploy/makerv3_stack.sh` reflect the March 4 rollout shape. The
  current production/operator surface is `tokenmm` only, and the legacy command
  path below is historical evidence rather than current operator guidance.
...
- [2026-03-04 07:35 UTC] Post-remediation verification: regression subset green on synchronized branch / evidence: `pytest -q tests/unit_tests/flux/api/test_tokenmm_compat.py tests/unit_tests/flux/api/test_socketio_tokenmm.py tests/unit_tests/flux/api/test_app.py --confcutdir=tests/unit_tests/flux/api` (31 passed), `pytest -q tests/unit_tests/examples/strategies/test_makerv3_run_api.py tests/unit_tests/examples/strategies/test_makerv3_run_bridge.py --confcutdir=tests/unit_tests/examples/strategies` (9 passed), `pytest -q tests/unit_tests/scripts/test_check_flux_leakage.py --confcutdir=tests/unit_tests/scripts` (4 passed), `pnpm --dir fluxboard exec vitest run __tests__/api.test.ts Alerts.test.tsx sockets.test.ts main.routes.test.tsx config/uiProfiles.test.ts` (49 passed), `pnpm --dir fluxboard build`, `python3 -m py_compile examples/live/makerv3/run_api.py`.
- [2026-03-04 08:40 UTC] Self-review improvement wave (3 items): improved synthesized trade `row_id` stability (`seq` + normalized `ts_ms` + `version`), added payload-level trade-contract unit tests, and added frontend compatibility tests for legacy fallback keys (`rows`, `parameters`) while preserving primary parsing on backend keys (`strategies`, `params`) / evidence: `pytest -q tests/unit_tests/flux/api/test_payloads.py tests/unit_tests/flux/api/test_tokenmm_compat.py --confcutdir=tests/unit_tests/flux/api` (15 passed), `pnpm --dir fluxboard exec vitest run __tests__/api.test.ts` (28 passed) / next: rerun broader regression subset.
- [2026-03-04 08:41 UTC] Post-improvement verification: API/Fluxboard regressions green; strategy bridge collection currently blocked by local binary incompatibility (`ValueError: ...UUID4 size changed...`) unrelated to touched files / evidence: `pytest -q tests/unit_tests/flux/api/test_tokenmm_compat.py tests/unit_tests/flux/api/test_socketio_tokenmm.py tests/unit_tests/flux/api/test_app.py tests/unit_tests/flux/api/test_payloads.py --confcutdir=tests/unit_tests/flux/api` (36 passed), `pnpm --dir fluxboard exec vitest run __tests__/api.test.ts Alerts.test.tsx sockets.test.ts main.routes.test.tsx config/uiProfiles.test.ts` (51 passed), `pnpm --dir fluxboard build`, `python3 -m py_compile examples/live/makerv3/run_api.py`, `pytest -q tests/unit_tests/examples/strategies/test_makerv3_run_api.py --confcutdir=tests/unit_tests/examples/strategies` (4 passed), `pytest -q tests/unit_tests/scripts/test_check_flux_leakage.py --confcutdir=tests/unit_tests/scripts` (4 passed).
- [2026-03-04 09:58 UTC] Post-migration hardening follow-up: completed Socket.IO emitter lifecycle/perf hardening in `systems/flux/flux/api/socketio.py` (explicit profile refcounts, no room-scan loop, idle wait when no profiles, per-profile exception logging + bounded backoff, zero-ref state cleanup) and added targeted lifecycle/error-isolation tests / evidence: `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q tests/unit_tests/flux/api/test_socketio_tokenmm.py --confcutdir=tests/unit_tests/flux/api` (15 passed) / SHA: `ec1625a45`.
- [2026-03-04 11:46 UTC] Post-migration Fluxboard hardening follow-up: normalized TokenMM balances/trades rows in the client model, hardened realtime trade message parsing, improved Signal delta merge resilience without wiping full params objects, and added Params single-strategy auto-selection test coverage / evidence: SHAs `8a2369974`, `11a4d3e3f` / next: re-run Vitest subset + `pnpm --dir fluxboard build` in the target runtime before deployment.
- [2026-03-04 12:05 UTC] Post-migration Fluxboard wiring follow-up: fixed TokenMM typed-param/schema compatibility and remaining UI data wiring gaps (signal strategy id canonicalization, params schema/value normalization, balances base-currency coin mapping, and trade notional derivation when upstream `mv=0`) / evidence: `pnpm --dir fluxboard exec vitest run api.flux.test.ts __tests__/utils/validation.test.ts __tests__/Params.single-selection.test.tsx Balances.test.tsx` (71 passed), `scripts/ci/check-flux-leakage.sh` (OK), `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q tests/unit_tests/flux/api/test_socketio_tokenmm.py --confcutdir=tests/unit_tests/flux/api` (15 passed), `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q tests/unit_tests/flux/api --confcutdir=tests/unit_tests/flux/api` (47 passed), `pnpm --dir fluxboard build`; live TokenMM bundle now serves `assets/index-UX_U-Q-f.js` at `/tokenmm` / SHA: `0976e58a5`.
- [2026-03-04 12:24 UTC] Post-migration Params wiring remediation: fixed single-strategy TokenMM selection lifecycle so Params no longer remains stuck at "No strategies selected" after selection clears while only one strategy is visible; added RED->GREEN regression test (`re-selects the only visible strategy after selection is cleared`), rebuilt/redeployed stack, and verified live bundle refresh / evidence: `pnpm --dir fluxboard exec vitest run __tests__/Params.single-selection.test.tsx` (2 passed after failing RED first), `pnpm --dir fluxboard exec vitest run api.flux.test.ts __tests__/utils/validation.test.ts __tests__/Params.single-selection.test.tsx Balances.test.tsx` (73 passed), `scripts/ci/check-flux-leakage.sh` (OK), `scripts/deploy/makerv3_stack.sh restart`, `scripts/deploy/makerv3_stack.sh health`; live `/tokenmm` serves `assets/index-DkCUsnkU.js` / SHA: `d2d192a23`.
- [2026-03-05 03:52 UTC] Post-migration contract hardening follow-up: fixed TokenMM profile-based REST strategy resolution across signal/strategy/balances/trades/alerts endpoints, prioritized discovered TokenMM strategy IDs over stale defaults, hardened stream row parsing for payloadless Redis entries, and removed dex/gas UI surfaces from TokenMM-visible trades/alerts/signal tooltips with shared alert normalization (`rows`/`alerts` fallbacks + websocket parity) / evidence: `scripts/ci/check-flux-leakage.sh` (OK), `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q tests/unit_tests/flux/api/test_socketio_tokenmm.py --confcutdir=tests/unit_tests/flux/api` (15 passed), `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q tests/unit_tests/flux/api --confcutdir=tests/unit_tests/flux/api` (55 passed), `pnpm --dir fluxboard vitest run api.flux.test.ts components/trades/columns.test.tsx` (21 passed) / SHAs: `317a37c80`, `45b23d7f6`.
- [2026-03-05 13:50 UTC] Post-migration UI polish follow-up: removed remaining gas surfaces from Trades decision modal (Summary + Copy TSV), aligned Trades footer rollups fee totals to prefer displayed `fee_quote` values, and backfilled missing `exchange` on Socket.IO leg deltas from `contract_id` to avoid blank venue labels during streaming updates / evidence: `pnpm --dir fluxboard vitest run` (114 passed) / SHAs: `8b65dd5da`, `2504f6824`, `829f3f8ae`.
...
2. TokenMM REST scope is profile-aware by default: when `strategy` is omitted, profile mapping resolves strategy context (with TokenMM discovery preference), and Socket.IO room scoping uses the same normalized profile contract.
...
| Emitter lifecycle | Done | Implemented in `systems/flux/flux/api/socketio.py`: explicit profile refcounts updated in connect/disconnect/set_profile handlers, event-driven idle emitter loop when no active profiles, per-profile exception isolation with bounded backoff/logging, and zero-ref cleanup of cached profile state maps. Follow-up fixed an import-blocking indentation regression before verification. Evidence: `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q tests/unit_tests/flux/api/test_socketio_tokenmm.py --confcutdir=tests/unit_tests/flux/api` (15 passed). SHAs: `ec1625a45`, `317a37c80`. |
| Fluxboard TokenMM data wiring | Done | Resolved TokenMM surface wiring gaps in `fluxboard/api.ts` / `fluxboard/utils/validation.ts` and completed Params lifecycle remediation in `fluxboard/Params.tsx` so single visible strategy selection self-heals after clear events. Verification evidence: `pnpm --dir fluxboard exec vitest run api.flux.test.ts __tests__/utils/validation.test.ts __tests__/Params.single-selection.test.tsx Balances.test.tsx` (73 passed), `pnpm --dir fluxboard build`, live stack restart + health checks; SHAs `0976e58a5`, `d2d192a23`. |
...
| REST vs Socket scoping | Done | Profile-aware REST strategy resolution is now enforced for TokenMM-read endpoints and aligned with Socket profile normalization/mapping; regression coverage added in `tests/unit_tests/flux/api/test_app.py` (`test_signals_profile_prefers_discovered_tokenmm_strategy_over_default_mapping`, `test_trades_endpoint_uses_profile_strategy_resolution_when_strategy_query_is_omitted`). SHA: `317a37c80`. |
