...
| Emitter lifecycle | Done | Implemented in `nautilus_trader/flux/api/socketio.py`: explicit profile refcounts updated in connect/disconnect/set_profile handlers, event-driven idle emitter loop when no active profiles, per-profile exception isolation with bounded backoff/logging, and zero-ref cleanup of cached profile state maps. Verification evidence: `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q tests/unit_tests/flux/api/test_socketio_tokenmm.py --confcutdir=tests/unit_tests/flux/api` (15 passed). |
