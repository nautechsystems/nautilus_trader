...
| Fluxboard TokenMM data wiring | Done | Resolved remaining TokenMM surface wiring gaps in `fluxboard/api.ts` and `fluxboard/utils/validation.ts`: typed param/schema normalization (`boolean`/`number` -> UI-safe param defs/values), canonical signal strategy id preference, balances child-coin mapping from `base_currency` when `asset=UNKNOWN`, and trade notional fallback from `price*qty` when upstream `mv` is zero. Verification evidence: `pnpm --dir fluxboard exec vitest run api.flux.test.ts __tests__/utils/validation.test.ts __tests__/Params.single-selection.test.tsx Balances.test.tsx` (71 passed), `pnpm --dir fluxboard build`; SHA `0976e58a5`. |
