...
| Task 5: Controlled Prod Cutover And Atomic Rollback Plan | in_progress | controller | Grouped-node cutover is live on `20260327T080639Z-readiness-route`; all 19 grouped node units are running, `/api/v1/readiness?profile=equities` now exists, but live readiness is still failing on stale signal/balance freshness so the task remains open |
