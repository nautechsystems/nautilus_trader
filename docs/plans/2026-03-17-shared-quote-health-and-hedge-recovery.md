...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Overall | in_progress | main | none | `systems/flux/flux/strategies/shared`, `systems/flux/flux/strategies/makerv3`, `systems/flux/flux/strategies/makerv4`, `systems/flux/flux/api`, `systems/flux/flux/runners/equities`, `fluxboard`, `tests`, `docs/plans` | `shared` | `shared` | none | `TMPDIR=.tmp PYTHONDONTWRITEBYTECODE=1 pytest test_equities_profile_contract.py PASS (36 passed); shared/api/strategy/readiness slice PASS (22 passed, 155 deselected); vitest MakerV4SignalTable PASS (5 passed)` | 2026-03-17 UTC quote-health, hedge backlog, readiness, overnight IBKR policy, and Signal UX slices are green locally; remaining gap is reviewer-lane follow-through, not failing verification |
| Task 1: Lock Shared Quote-Health Semantics In Tests And Docs | in_review_spec | Spec Reviewer | none | `docs/plans/2026-03-17-shared-quote-health-and-hedge-recovery.md`, `tests/unit_tests/flux/api`, `tests/unit_tests/flux/strategies/makerv3`, `tests/unit_tests/flux/strategies/makerv4`, `fluxboard/docs` | `shared` | `shared` | none | `TMPDIR=.tmp PYTHONDONTWRITEBYTECODE=1 pytest test_equities_profile_contract.py PASS (36 passed)` | 2026-03-17 UTC failing tests now pass for shared evaluator import, Makerv4 stale-maker block, backlog gating, and API quote-health fields; tokenmm/equities contract docs now define shared quote-health semantics; spec review pending |
...
| Task 4: Refactor MakerV4 Pricing And Hedge Gates Onto Shared Quote Health | in_review_spec | Spec Reviewer | Task 2: Implement Shared Quote-Health Evaluator Module | `systems/flux/flux/strategies/makerv4`, `tests/unit_tests/flux/strategies/makerv4` | `shared` | `shared` | none | `TMPDIR=.tmp PYTHONDONTWRITEBYTECODE=1 pytest ...test_strategy.py -k 'stale_quote or pending_hedge or take_take or hedge_backlog or hedge_disabled_reason or partial_hedge_fill' PASS (20 passed)` | 2026-03-17 UTC Makerv4 now publishes explicit leg quote health, blocks old maker quotes for new risk, and persists hedge backlog state; spec review pending |
| Task 5: Add Makerv4 Hedge Backlog, Retry, And Inventory Reconciliation | in_review_spec | Spec Reviewer | Task 4: Refactor MakerV4 Pricing And Hedge Gates Onto Shared Quote Health | `systems/flux/flux/strategies/makerv4`, `tests/unit_tests/flux/strategies/makerv4`, `tests/unit_tests/flux/common` | `shared` | `shared` | none | `TMPDIR=.tmp PYTHONDONTWRITEBYTECODE=1 pytest ...test_strategy.py -k 'recoverable_hedge_backlog or restores_hedge_backlog_state' PASS (2 passed)` | 2026-03-17 UTC stale-reference hedge failures now create retryable backlog state, retry on fresh quotes, and survive snapshot/restore; spec review pending |
| Task 6: Align API Payloads And Readiness On Shared Semantics | in_review_spec | Spec Reviewer | Task 3: Refactor MakerV3 To Use Shared Quote-Health Contracts, Task 4: Refactor MakerV4 Pricing And Hedge Gates Onto Shared Quote Health | `systems/flux/flux/api`, `systems/flux/flux/runners/equities/readiness.py`, `tests/unit_tests/flux/api`, `tests/unit_tests/examples/strategies` | `shared` | `shared` | none | `TMPDIR=.tmp PYTHONDONTWRITEBYTECODE=1 pytest ...test_equities_profile_contract.py ...test_equities_readiness.py -k 'tradeable or quote_state or feed_state or backlog or explicit_old_quote_state or explicit_missing_quote_state or explicit_feed_down_state or operator' PASS (22 passed, 155 deselected); full test_equities_profile_contract.py PASS (36 passed)` | 2026-03-17 UTC `/signals` now suppresses MakerV4 tradeable on explicit quote-health and hedge-backlog blockers, readiness consumes shared leg semantics, and the overnight steady-state hedge-policy contract is green; spec review pending |
| Task 7: Update Fluxboard Signal And Alerts UX For Feed State, Quote Age, And Trading Eligibility | in_review_spec | Spec Reviewer | Task 6: Align API Payloads And Readiness On Shared Semantics | `fluxboard/components/domain/signal`, `fluxboard/utils`, `fluxboard/types.ts`, `fluxboard/tests/signal`, `fluxboard/docs` | `shared` | `shared` | none | `pnpm --dir fluxboard exec vitest run tests/signal/MakerV4SignalTable.test.tsx PASS (5 passed)` | 2026-03-17 UTC MakerV4 Signal now shows feed/quote state separately from age and surfaces hedge backlog context in the hedge cell; spec review pending |
