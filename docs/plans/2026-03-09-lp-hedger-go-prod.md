...
| --- | --- | --- | --- |
| Overall | completed | main | Completed in isolated worktree after materializing the LP port dependency commit locally because current `origin/main` did not yet contain it. Final LP-scoped verification passed: `python3 -m pytest -q --noconftest tests/unit_tests/examples/lp/test_lp_prod_rollout_contract.py` -> `9 passed`; `PYTHONPATH=. uv run --no-project --with pytest pytest -q --noconftest tests/unit_tests/examples/lp/test_lp_stack_contract.py tests/unit_tests/examples/lp/test_lp_prod_rollout_contract.py` -> `18 passed`; overlay pytest on `test_tokenmm_run_api.py -k 'shared_asset_prefix or lp'` -> `4 passed, 22 deselected`; `pnpm --dir fluxboard exec vitest run Hedger.test.tsx main.routes.test.tsx config/uiProfiles.test.ts` -> `3 passed, 40 passed`; `pnpm --dir fluxboard build` -> PASS. Separate unrelated issue remains recorded from the broader `pnpm --dir fluxboard test:run -- ...` run that exposed a pre-existing `Trades.recovery.test.tsx` failure outside the LP verification scope. Post-plan deployment rehearsal on 2026-03-09 surfaced four rollout gaps now fixed or documented in-branch: the LP installer wrote a literal `${LP_SYSTEM_CONFIG:-...}` hedger command that did not expand under `exec ${CMD}`, `lp.runners.run_hedger` was missing its `python3 -m` entrypoint guard, the runbook/README now require materializing both shared frontend bundles (`fluxboard` plus a rebuilt or copied `pulse-ui/dist`) before restarting the shared `tokenmm-api` host, and the preflight/runbook now require `/etc/flux/lp-system.ini` to remain readable by the Flux service user `ubuntu`. |
...
| Task 2: Audit And Port Remaining Chainsaw LP GUI Parity To `/lp` | completed | main | Added explicit `/lp` home-route/parity tests and documented the remaining intentional monorepo deltas in `fluxboard/docs/lp_contract.md`. Verification: `pnpm --dir fluxboard exec vitest run Hedger.test.tsx -t "documents the intentional monorepo lp deltas from Chainsaw"` -> `1 passed`; `pnpm --dir fluxboard exec vitest run Hedger.test.tsx main.routes.test.tsx config/uiProfiles.test.ts` -> `3 passed, 40 passed`. Separate unrelated issue: `pnpm --dir fluxboard test:run -- Hedger.test.tsx main.routes.test.tsx config/uiProfiles.test.ts` also pulled broader suites and exposed a pre-existing `Trades.recovery.test.tsx` failure outside LP scope. |
...
| Task 6: Add Rollout Health Checks, Canary Procedure, And Rollback Evidence | completed | main | Added `ops/scripts/deploy/check_lp_rollout.sh`, the dated rollout review template, and explicit canary/go-no-go/rollback guidance in the runbook and deploy README. Verification: `python3 -m pytest -q --noconftest tests/unit_tests/examples/lp/test_lp_prod_rollout_contract.py -k "rollout or rollback"` -> `9 passed`; `uv run --no-project --with pytest pytest -q --noconftest tests/unit_tests/examples/lp/test_lp_prod_rollout_contract.py -k "rollout or rollback"` -> `9 passed`; `bash ops/scripts/deploy/check_lp_rollout.sh --help` -> PASS. |
