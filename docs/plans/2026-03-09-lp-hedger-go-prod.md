...
| --- | --- | --- | --- |
| Overall | in_progress | main | Deployment continuation: the Pulse rollout gate is fixed and the host cutover now succeeds internally and externally (`/lp`, `/api/v1/hedgers/eth_plume_lp`, `/api/pulse/jobs` all respond on `13.213.194.42:5022`). The remaining operator-visible gap is GUI parity with the production constraint: the `/lp` hedger selector is still driven by the full registry, so the public instances API exposes disabled template hedgers in addition to Band1/Band2. Added a failing API contract test, changed the LP API default registry export to the active production pair only, tightened the rollout docs, and re-verified locally with `python3 -m pytest -q --noconftest tests/unit_tests/lp/api/test_app.py tests/unit_tests/lp/test_registry.py tests/unit_tests/examples/lp/test_lp_prod_rollout_contract.py tests/unit_tests/examples/lp/test_lp_stack_contract.py tests/unit_tests/lp/runners/test_run_hedger.py` -> `37 passed`. Next step is the minimal live restart of `flux@lp-api.service` on 2026-03-09. |
...
| Task 2: Audit And Port Remaining Chainsaw LP GUI Parity To `/lp` | in_progress | main | Reopened after live external validation on `13.213.194.42:5022/lp`: the deployed GUI is up, but its selector is still populated from the full LP registry, which exposes disabled template hedgers beyond Band1/Band2. Added an LP API regression test for “Band1/Band2 only by default”, changed `create_lp_api_app()` to publish only `default_enabled` hedgers in the public instances list while preserving the full registry internally, and updated the docs to state that `/api/v1/hedgers/instances` drives the `/lp` selector with Band1/Band2 only during rollout. Fresh local verification: `37 passed`. Pending live `lp-api` restart and external verification before closing the task. |
