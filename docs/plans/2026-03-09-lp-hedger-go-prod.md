...
| --- | --- | --- | --- |
| Overall | in_progress | main | Deployment continuation: local/remote evidence isolated the final remaining rollout gate to the Pulse health-check contract in `ops/scripts/deploy/check_lp_rollout.sh`. Added a reproduction test that executes the script against a mock host returning the real `/api/pulse/jobs` payload shape, then fixed the script to validate `jobs/total/active/failed` instead of `ok=true`. Fresh local verification: `python3 -m pytest -q --noconftest tests/unit_tests/examples/lp/test_lp_prod_rollout_contract.py -k real_pulse_jobs_payload_shape` -> `1 passed, 11 deselected`; `bash ops/scripts/deploy/check_lp_rollout.sh --help` -> PASS; `python3 -m pytest -q --noconftest tests/unit_tests/examples/lp/test_lp_prod_rollout_contract.py tests/unit_tests/examples/lp/test_lp_stack_contract.py tests/unit_tests/lp/runners/test_run_hedger.py` -> `28 passed`. Next step is remote prepare/cutover rerun on 2026-03-09. |
