...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Overall | in_progress | main | none | `docs/plans/2026-03-26-pr72-finish-remediation.md`, `docs/plans/realtime-status/rt-standard-transport.md`, `docs/plans/realtime-status/rt-market-balances.md`, `docs/plans/2026-03-19-fluxboard-performance-improvement-plan.md` | `lanes/pr72-finish-controller` | `/home/ubuntu/nautilus_trader/.worktrees/pr72-finish-controller` | `f31e9709c3` | `review baseline captured; controller merge smoke green on 2026-03-26 UTC; Task 2 spec review passed; Task 3 targeted vitest slice green (86 passed) plus api.flux params-write tests green (3 passed); backend pytest still blocked in current env` | Task 1 committed; Task 2 lane now points at `d9b37ac302`; Task 3 lane now points at `e6e1461c70`; the pre-existing `api.flux.test` params-write blocker was resolved by aligning the test harness with the fetch-based params write path |
...
| Task 2: Harden Backend Standard Subscription Lifecycle | in_review_quality | quality_reviewer | Task 1: Rebase Controller Branch And Resolve Mainline Conflicts | `systems/flux/flux/api/socketio.py`, `systems/flux/docs/api.md`, `tests/unit_tests/flux/api/test_realtime_contract.py`, `tests/unit_tests/flux/api/test_app.py`, `tests/unit_tests/flux/api/test_socketio_tokenmm.py` | `lanes/task-2-pr72-backend-lifecycle` | `/home/ubuntu/nautilus_trader/.worktrees/task-2-pr72-backend-lifecycle` | `d9b37ac302` | `python3 -m py_compile systems/flux/flux/api/socketio.py systems/flux/flux/api/app.py; git diff --check; pytest still blocked locally by missing built module nautilus_trader.core.data` | Spec review passed after the heartbeat-doc correction; advancing to code-quality review with the backend pytest environment still blocked locally |
| Task 3: Harden Frontend Standard Subscription And Socket Lifecycle | in_review_spec | spec_reviewer | Task 1: Rebase Controller Branch And Resolve Mainline Conflicts | `fluxboard/hooks/useWebSocket.ts`, `fluxboard/sockets.ts`, `fluxboard/PnL.tsx`, `fluxboard/Trades.tsx`, `fluxboard/Alerts.tsx`, `fluxboard/components/domain/signal/SignalTable.tsx`, `fluxboard/hooks/useVirtualizedRows.ts`, `fluxboard/sockets.test.ts`, `fluxboard/api.flux.test.ts`, `fluxboard/__tests__/realtime/standard-socket-client.test.tsx`, `fluxboard/__tests__/trades-integration.test.tsx`, `fluxboard/__tests__/panels/signal.test.tsx`, `fluxboard/__tests__/hooks/useVirtualizedRows.test.ts` | `lanes/task-3-pr72-frontend-runtime` | `/home/ubuntu/nautilus_trader/.worktrees/task-3-pr72-frontend-runtime` | `e6e1461c70` | `VITEST_FULL=1 pnpm --dir fluxboard exec vitest run sockets.test.ts __tests__/realtime/standard-socket-client.test.tsx __tests__/trades-integration.test.tsx __tests__/panels/signal.test.tsx __tests__/hooks/useVirtualizedRows.test.ts (86 passed); VITEST_FULL=1 pnpm --dir fluxboard exec vitest run api.flux.test.ts --testNamePattern \"patchStrategyParams|updateParams\" (3 passed)` | Hook/runtime fixes cover equivalent-lineage rerenders, shared socket rebinding after recreate, canonical trades page-size normalization, viewport resize response, and accepted-start-seq recovery resubscribe; lane also picked up the params-write test-harness alignment needed for final verification |
