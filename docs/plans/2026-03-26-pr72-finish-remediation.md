...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Overall | in_progress | main | none | `docs/plans/2026-03-26-pr72-finish-remediation.md`, `docs/plans/realtime-status/rt-standard-transport.md`, `docs/plans/realtime-status/rt-market-balances.md`, `docs/plans/2026-03-19-fluxboard-performance-improvement-plan.md` | `lanes/pr72-finish-controller` | `/home/ubuntu/nautilus_trader/.worktrees/pr72-finish-controller` | `working tree atop c499ec8eff` | `Task 3 targeted vitest slice green (86 passed) plus api.flux params-write tests green (3 passed); Task 4 cleanup slice green: VITEST_FULL=1 pnpm --dir fluxboard exec vitest run __tests__/realtime/baseline-budgets.test.tsx __tests__/realtime/compatibility-matrix.test.tsx (10 passed); git diff --check; backend pytest still blocked in current env` | Task 1, Task 2, and Task 3 are integrated onto the controller branch; Task 4 cleanup work was reclaimed onto the controller worktree after the dedicated lane stayed untouched, and it now clarifies committed reference fixtures plus polling-only recovery semantics for the standard transport |
...
| Task 4: Clean Up Rollout Evidence, Capability Fixtures, And Trackers | in_review_spec | main | Task 2: Harden Backend Standard Subscription Lifecycle; Task 3: Harden Frontend Standard Subscription And Socket Lifecycle | `fluxboard/components/trades/PerfHarness.tsx`, `fluxboard/__tests__/realtime/baseline-budgets.test.tsx`, `fluxboard/__tests__/realtime/compatibility-matrix.test.tsx`, `fluxboard/e2e/realtime-cutovers/trades.spec.ts`, `docs/plans/2026-03-19-fluxboard-performance-improvement-plan.md`, `docs/plans/realtime-surfaces/signal-cutover.md`, `docs/plans/realtime-surfaces/trades-cutover.md`, `docs/plans/realtime-status/rt-standard-transport.md`, `docs/plans/realtime-status/rt-market-balances.md` | `lanes/pr72-finish-controller` | `/home/ubuntu/nautilus_trader/.worktrees/pr72-finish-controller` | `working tree atop c499ec8eff` | `VITEST_FULL=1 pnpm --dir fluxboard exec vitest run __tests__/realtime/baseline-budgets.test.tsx __tests__/realtime/compatibility-matrix.test.tsx (10 passed); git diff --check` | The original cleanup lane remained untouched, so Task 4 was reclaimed into the controller worktree; the current diff makes the perf artifacts explicitly committed-reference data and clarifies that standard steady-state traffic is Socket.IO while recovery capabilities remain invalidate-only / polling-only |
| Task 5: Verification Sweep And Merge-Readiness Review | in_progress | main | Task 2: Harden Backend Standard Subscription Lifecycle; Task 3: Harden Frontend Standard Subscription And Socket Lifecycle; Task 4: Clean Up Rollout Evidence, Capability Fixtures, And Trackers | `shared verification only` | `lanes/pr72-finish-controller` | `/home/ubuntu/nautilus_trader/.worktrees/pr72-finish-controller` | `working tree atop c499ec8eff` | `VITEST_FULL=1 pnpm --dir fluxboard exec vitest run __tests__/realtime/standard-socket-client.test.tsx __tests__/trades-integration.test.tsx __tests__/panels/signal.test.tsx __tests__/hooks/useVirtualizedRows.test.ts __tests__/realtime/baseline-budgets.test.tsx __tests__/realtime/compatibility-matrix.test.tsx api.flux.test.ts __tests__/realtime/market-balances-standard.test.tsx (138 passed); pnpm --dir fluxboard build:test; E2E_BASE_URL=http://127.0.0.1:4173 pnpm --dir fluxboard exec playwright test -c playwright.smoke.config.ts e2e/realtime-cutovers/trades.spec.ts (1 passed); python3 -m py_compile systems/flux/flux/api/socketio.py systems/flux/flux/api/app.py; pytest still blocked locally by missing built module nautilus_trader.core.data` | Final controller verification sweep is running on the reclaimed Task 4 diff; frontend combined slice, build, and owned Trades cutover smoke are green, while backend pytest remains blocked by the missing local build artifact |
