...
| Task 3: Replace Single-Base Portfolio Snapshot With Multi-Asset Snapshot V2 | completed | main | Completed in commit `0a1e27bf3`: whole-book `inventory_by_asset` snapshot, single-write portfolio publish path, shared-account rows included, single-asset compatibility preserved, `47 passed` focused common/runner tests, `6 passed` tokenmm portfolio-snapshot API coverage, import-order lint clean, and `git diff --check` clean. |
| Task 4: Move Equities Balances API To Portfolio Snapshot V2 | completed | main | Completed in commits `e059c8e88` and `479edadd7`: equities now prefers fresh portfolio snapshot v2 rows with shared-account provenance, shared rows stop overloading `strategy_id`, legacy tokenmm snapshot coverage still passes, `32 passed` balances/API contract tests, `6 passed` tokenmm snapshot regression, import-order lint is green, and commit-range `git diff --check` is clean. |
| Task 5: Migrate MakerV3 Inventory And Balance Publishing To The Canonical Model | completed | main | Completed: `run_node.py` injects canonical `portfolio_asset_id` from `strategy_contracts`, MakerV3 publishes portfolio inventory components under that canonical asset key while keeping maker-leg local exposure math intact, balances publishing no longer merges strategy-owned IBKR/shared-account rows, price-based base-exposure conversion stays consistent across balances/local summary/portfolio publication, `99 passed` on the Task 5 pytest slice, and `git diff --check` is clean. |
