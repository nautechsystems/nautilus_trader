...
| Task 3: Replace Single-Base Portfolio Snapshot With Multi-Asset Snapshot V2 | completed | main | Completed with controller-side spec/quality review after subagent handoffs stalled: whole-book `inventory_by_asset` snapshot, single-write portfolio publish path, asset-scoped inventory aggregation via `strategy_contracts`, shared-account rows included, single-asset compatibility preserved, `41 passed` focused common/equities-runner tests, `7 passed` tokenmm snapshot compatibility slice, clean targeted `ruff`, clean `git diff --check`. |
| Task 4: Move Equities Balances API To Portfolio Snapshot V2 | not_started | unassigned | Waiting for Task 3 to clear spec and quality review. |
