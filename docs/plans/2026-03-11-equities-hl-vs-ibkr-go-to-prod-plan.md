...
| Task 5: Migrate MakerV3 Inventory And Balance Publishing To The Canonical Model | in_progress | implementer | Starting from the current live MakerV3 publication path: write red tests for canonical `portfolio_asset_id` inventory keys and for removing strategy-owned IBKR/shared-account rows from MakerV3 balance snapshots. |
