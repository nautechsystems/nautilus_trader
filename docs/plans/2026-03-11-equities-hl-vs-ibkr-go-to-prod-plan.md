...
| Task 4: Move Equities Balances API To Portfolio Snapshot V2 | completed | main | Completed with local TDD and controller-side review: equities now prefers fresh portfolio snapshot v2 rows with shared-account provenance, shared rows stop overloading `strategy_id`, legacy tokenmm snapshot coverage still passes, `32 passed` balances/API contract tests, `6 passed` tokenmm snapshot regression, import-order lint clean, and `git diff --check` clean. |
