...
| Task 3: Replace Single-Base Portfolio Snapshot With Multi-Asset Snapshot V2 | completed | main | Completed with controller-side spec/quality review after subagent handoffs stalled: whole-book `inventory_by_asset` snapshot, single-write portfolio publish path, shared-account rows included, single-asset compatibility preserved, `47 passed` focused common/runner tests, `6 passed` tokenmm portfolio-snapshot API coverage, clean targeted `ruff`, clean `git diff --check`. |
