...
- **Pending cancel saturation**: once cancel backlog is present, suspend further repricing until backlog drains or times out.
...
- optional venue cooldowns when rate-limit or order-state mismatch bursts are detected
...
| Task 4: Fix Cancel-Reject Reconciliation And State Export Correctness | not_started | unassigned | Task 3: Integrate Planner Into Quote Cycle With Venue Budgets | `systems/flux/flux/strategies/makerv3/strategy.py`, `systems/flux/flux/strategies/makerv3/publisher.py`, `tests/unit_tests/flux/strategies/makerv3/test_order_safety.py`, `tests/unit_tests/flux/strategies/makerv3/test_observability_and_exports.py`, `tests/unit_tests/flux/api/test_app.py` | `shared` | `/home/ubuntu/nautilus_trader/.worktrees/codex-bybit-makerv3-ladder-hardening-20260312` | none | not_run | Plan created |
...
- max total change actions per cycle
- optional venue or strategy change-rate cooldown knobs if needed
...
**Step 4: Ensure pending-cancel backlog gates further movement**

Once a side or the whole strategy has sufficient pending-cancel backlog, suspend further replacement work until backlog drains or transitions into a blocked state.
...
**Dependencies:** `Task 3: Integrate Planner Into Quote Cycle With Venue Budgets`
