...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Overall | in_progress | controller | Task 1-6 | docs/plans/2026-03-28-equities-market-data-recovery-v1.md | fix/equities-market-data-recovery-v1-impl-20260328 | /home/ubuntu/nautilus_trader/.worktrees/equities-market-data-recovery-v1-impl-20260328 | none | `BUILD_MODE=debug CARGO_TARGET_DIR=/home/ubuntu/nautilus_trader/target uv run --active --no-sync build.py` PASS; `./.venv.py312/bin/python -m pytest -q tests/unit_tests/examples/strategies/test_equities_readiness.py -k 'down'` PASS (`1 passed, 22 deselected`) | 2026-03-28: Controller worktree created from plan/equities-market-data-recovery-v1-20260328 at 8b45a58700; tracker normalized for execution; bootstrap build restored worktree-native modules and smoke pytest passed |
...
| Task 3: Harden Hyperliquid Recovery Semantics | blocked | controller | Task 1 | crates/adapters/hyperliquid/src/python/websocket.rs; crates/adapters/hyperliquid/src/websocket/client.rs; nautilus_trader/adapters/hyperliquid/data.py; crates/adapters/hyperliquid/tests/http.rs; crates/adapters/hyperliquid/tests/websocket.rs; tests/integration_tests/adapters/hyperliquid/test_data.py; nautilus_trader/adapters/hyperliquid/factories.py; systems/flux/flux/runners/equities/run_node.py; systems/flux/flux/runners/shared/quote_feed_supervisor.py; tests/unit_tests/examples/strategies/test_equities_run_node.py; tests/unit_tests/examples/strategies/test_quote_feed_supervisor.py | lanes/task-3-hyperliquid-recovery | /home/ubuntu/nautilus_trader/.worktrees/task-3-hyperliquid-recovery | working tree diff vs 978bebebd7 | `./.venv.py312/bin/python build.py` PASS; `cargo test -p nautilus-hyperliquid --test http recovery -- --nocapture` PASS (`1 passed`); `cargo test -p nautilus-hyperliquid --test websocket recovery -- --nocapture` PASS (`4 passed`); `./.venv.py312/bin/python -m pytest -q tests/integration_tests/adapters/hyperliquid/test_data.py -k 'recovery or cache or subscribe_quotes'` PASS (`1 passed, 17 deselected`); `git diff --check` PASS | 2026-03-28: Spec Reviewer the 20th found the remaining Hyperliquid gap is not adapter-only: runner-issued resets still never reach the live adapter path and the runner ingress still drops explicit transport outcome fields. Task 2 re-review also confirmed the same seam is still missing any production adapter-result / blocker source wiring, so controller is keeping Task 3 blocked until the overlapping runner attachment path stabilizes and can be fixed in one lane. |
| Task 4: Harden Binance Recovery Semantics | in_progress | controller | Task 1 | nautilus_trader/adapters/binance/data.py; nautilus_trader/adapters/binance/websocket/client.py; tests/unit_tests/adapters/binance/test_data_recovery.py | lanes/task-4-binance-recovery | /home/ubuntu/nautilus_trader/.worktrees/task-4-binance-recovery | a29bacf62b (integrated from lane 4b61728d79; controller reconciliation in progress) | Lane `./.venv.py312/bin/python -m pytest -q tests/unit_tests/adapters/binance/test_data_recovery.py -k 'recovery or book_ticker or subscribe'` PASS (`16 passed`); lane `./.venv.py312/bin/python -m pytest -q tests/unit_tests/adapters/binance` PASS (`36 passed`); controller `./.venv.py312/bin/python -m pytest -q tests/unit_tests/adapters/binance/test_data_recovery.py -k 'recovery or book_ticker or subscribe'` FAIL (`16 failed`) | 2026-03-28: Post-cherry-pick controller verification exposed an integration regression, not a lane regression. The controller tree is missing the lane-validated Binance recovery primitives (`_desired_streams`, `recover_book_ticker`, boolean `_send`, and `recover_quote_ticks`), so Task 4 is reopened until the integrated controller diff is green and re-reviewed. |
| Task 5: Add Honest Recovering-State Semantics Without Regressing Down-State Behavior | not_started | unassigned | Task 2, Task 3, Task 4 | systems/flux/flux/api/_payloads_signals.py; tests/unit_tests/flux/api/test_payloads.py; tests/unit_tests/examples/strategies/test_equities_readiness.py; tests/unit_tests/flux/api/test_equities_profile_contract.py | pending | pending | none | not_run | 2026-03-28: Wait for supervisor, strategy gating, and adapter recovery states |
| Task 6: Validate Live Recovery And Cut The V1 Prod Release | not_started | unassigned | Task 1, Task 2, Task 3, Task 4, Task 5 | docs/plans/2026-03-27-equities-shared-symbol-node.md; docs/runbooks/equities-shared-node-cutover.md; ops/scripts/deploy/check_equities_live_readiness.sh | pending | pending | none | not_run | 2026-03-28: Final verification/release task after implementation tasks land |
