...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Task 1: Auto-repair stale startup spot state when venue is already flat | completed | main | none | `nautilus_trader/live/execution_engine.py`, `tests/unit_tests/live/test_execution_recon.py` | `fix/tokenmm-startup-auto-repair-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/tokenmm-startup-auto-repair-20260326` | `working_tree` | `uv run --active --no-sync pytest -q tests/unit_tests/live/test_execution_recon.py -k "auto_repairs_stale_startup_position_with_open_order_and_recent_closing_fill_binance_spot_shape or cleans_stale_startup_position_after_missing_targeted_open_order_query"` PASS | 2026-03-26: added Binance-spot startup stale-position regression and bounded unmatched-fill cleanup path; Task 1 green in working tree |
| Task 2: Prevent startup perp fill replay from drifting local qty above venue truth | completed | main | Task 1: Auto-repair stale startup spot state when venue is already flat | `nautilus_trader/live/execution_engine.py`, `nautilus_trader/live/reconciliation.py`, `tests/unit_tests/live/test_execution_recon.py` | `fix/tokenmm-startup-auto-repair-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/tokenmm-startup-auto-repair-20260326` | `working_tree` | `uv run --active --no-sync pytest -q tests/unit_tests/live/test_execution_recon.py -k "does_not_replay_claimed_startup_fill_when_cached_netting_position_already_matches_venue_okx_shape or uses_open_only_with_targeted_open_order_queries_when_startup_positions_exist"` PASS | 2026-03-26: existing open-only startup replay regressions stayed green after the startup fill-adjustment gate changed; no additional code delta required |
| Task 3: Restore bounded grouped orphan lineage without reopening ambiguous subset guessing | completed | main | Task 2: Prevent startup perp fill replay from drifting local qty above venue truth | `nautilus_trader/live/execution_engine.py`, `tests/unit_tests/live/test_execution_recon.py` | `fix/tokenmm-startup-auto-repair-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/tokenmm-startup-auto-repair-20260326` | `working_tree` | `uv run --active --no-sync pytest -q tests/unit_tests/live/test_execution_recon.py -k "startup_restores_grouped_orphan_lineage_exactly or startup_does_not_combine_multiple_orphan_positions"` PASS | 2026-03-26: bounded grouped orphan lineage contract was already green in the current engine; verified unchanged |
| Task 4: Emit explicit startup repair classifications and alerts | completed | main | Task 3: Restore bounded grouped orphan lineage without reopening ambiguous subset guessing | `nautilus_trader/live/execution_engine.py`, `tests/unit_tests/live/test_execution_recon.py` | `fix/tokenmm-startup-auto-repair-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/tokenmm-startup-auto-repair-20260326` | `working_tree` | `uv run --active --no-sync pytest -q tests/unit_tests/live/test_execution_engine.py -k "startup_flattening_publishes_execution_alert or startup_fail_closed_alert_includes_classification or closes_stale_startup_netting_positions_when_venue_is_flat_and_no_open_orders"` PASS | 2026-03-26: startup reconciliation alerts now emit explicit `cause` / `action` classifications for cleanup, mismatch-reconcile, and fail-closed paths |
| Task 5: Verify the focused suite and update rollout config and notes | blocked | main | Task 1: Auto-repair stale startup spot state when venue is already flat, Task 2: Prevent startup perp fill replay from drifting local qty above venue truth, Task 3: Restore bounded grouped orphan lineage without reopening ambiguous subset guessing, Task 4: Emit explicit startup repair classifications and alerts | `deploy/tokenmm/tokenmm.live.toml`, `docs/plans/2026-03-26-tokenmm-startup-auto-repair.md` | `fix/tokenmm-startup-auto-repair-20260326` | `/home/ubuntu/nautilus_trader/.worktrees/tokenmm-startup-auto-repair-20260326` | `working_tree` | `uv run --active --no-sync pytest -q tests/unit_tests/live/test_execution_recon.py -k "does_not_replay_claimed_startup_fill_when_cached_netting_position_already_matches_venue_okx_shape or uses_open_only_with_targeted_open_order_queries_when_startup_positions_exist or startup_restores_grouped_orphan_lineage_exactly or startup_does_not_combine_multiple_orphan_positions or auto_repairs_stale_startup_position_with_open_order_and_recent_closing_fill_binance_spot_shape or cleans_stale_startup_position_after_missing_targeted_open_order_query"` PASS; `uv run --active --no-sync pytest -q tests/unit_tests/live/test_execution_engine.py -k "startup_flattening_publishes_execution_alert or startup_fail_closed_alert_includes_classification or closes_stale_startup_netting_positions_when_venue_is_flat_and_no_open_orders"` PASS; `uv run --active --no-sync pytest -q tests/unit_tests/flux/runners/test_tokenmm_readiness.py` PASS; `uv run --active --no-sync pytest -q tests/unit_tests/examples/strategies/test_tokenmm_stack_contract.py -k "tokenmm_registry_requires_all_supported_live_core_strategies"` PASS; `curl -fsS http://127.0.0.1:5022/api/v1/readiness?profile=tokenmm` still returns the old 5-strategy required set | 2026-03-26: repo config now requires Binance spot/perp, but the running local readiness API has not reloaded the new config yet |
