...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Overall | in_progress | main | none | `systems/flux/flux/common`, `systems/flux/flux/runners/equities`, `systems/flux/flux/runners/shared`, `systems/flux/flux/api`, `systems/flux/flux/strategies`, `deploy/equities`, `fluxboard`, `tests/unit_tests`, `docs/plans` | `shared` | `shared` | `58bc0eb512` integrated; Task 3 next | Task 2 is complete and Task 3 is now the next ready lane on the shared branch | Execution started via `subagent-driven-development`; Task 1 reopened after quality review found the live manifest outpaced the checked-in split deploy surface, the follow-up deploy-surface repair is complete, and Task 2 passed spec review plus quality review with no findings against upstream multivenue account/balance source `c362ff46485492e07168336e9de2c39d8150984b` |
| Task 1: Expand Shared Equities Route And Account Contracts | completed | main | none | `systems/flux/flux/common/account_scopes.py`, `systems/flux/flux/common/strategy_contracts.py`, `deploy/equities/equities.live.toml`, `deploy/equities/strategies`, `deploy/equities/equities_stack.env.example`, `deploy/equities/systemd`, `ops/scripts/deploy/install_equities_systemd.sh`, `ops/scripts/deploy/binance_equities_universe.py`, `tests/unit_tests/examples/strategies/test_equities_run_api.py`, `tests/unit_tests/examples/strategies/test_equities_run_node.py`, `tests/unit_tests/examples/strategies/test_equities_stack_contract.py` | `shared` | `shared` | `aadbd8b032..e89b5734ed` | `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run --group test pytest tests/unit_tests/examples/strategies/test_equities_stack_contract.py -q` -> PASS (`35 passed in 0.15s`); `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run --group test pytest tests/unit_tests/examples/strategies/test_equities_stack_contract.py tests/unit_tests/examples/strategies/test_equities_run_api.py tests/unit_tests/examples/strategies/test_equities_run_node.py -q` -> PASS (`98 passed in 1.35s`); `git -C /home/ubuntu/nautilus_trader/.worktrees/makerv4-split-dual-arb-impl-20260317 diff --check` -> PASS | 2026-03-19 UTC reopened quality gap is fixed in `e89b5734ed` by translating the missing split strategy TOMLs, static `systemd` target/sudoers assets, and Binance env examples so the widened live manifest only advertises routes the checked-in deploy surface can actually start. Fresh spec review on `aadbd8b032..e89b5734ed` returned no findings, fresh quality review on the same diff returned no findings, and Task 1 is complete again |
| Task 2: Port Shared Account Projection And Portfolio Balance Changes | completed | implementer | Task 1: Expand Shared Equities Route And Account Contracts | `systems/flux/flux/runners/shared/profile_accounts.py`, `systems/flux/flux/api/app.py`, `systems/flux/flux/api/_payloads_balances.py`, `tests/unit_tests/flux/common/test_portfolio_inventory.py`, `tests/unit_tests/flux/api/test_balances_merge_dedupe.py`, `tests/unit_tests/flux/api/test_app.py`, `tests/unit_tests/flux/api/test_equities_profile_contract.py`, `tests/unit_tests/examples/strategies/test_equities_run_portfolio.py` | `shared` | `shared` | `5da1751bec..58bc0eb512` | RED: `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run --group test pytest tests/unit_tests/flux/common/test_portfolio_inventory.py tests/unit_tests/flux/api/test_balances_merge_dedupe.py tests/unit_tests/flux/api/test_app.py tests/unit_tests/flux/api/test_equities_profile_contract.py tests/unit_tests/examples/strategies/test_equities_run_portfolio.py -q` -> FAIL (`4 failed, 162 passed in 1.46s`); GREEN: same command -> PASS (`166 passed in 1.17s`) | 2026-03-19 UTC red-first Task 2 test additions are in place. The committed implementation in `58bc0eb512` adds Binance futures shared-account provider bindings, loads profile-account projections into the split equities balances API, preserves same-venue shared-position dedupe while keeping multivenue same-asset rows distinct, and aligns the stale live-config assertion with the widened split multivenue manifest. Spec review on `5da1751bec..58bc0eb512` returned no findings, quality review on the same diff returned no findings, and Task 2 is complete |
