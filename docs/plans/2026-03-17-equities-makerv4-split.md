...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Overall | in_progress | main | none | `systems/flux/flux/strategies`, `systems/flux/flux/api`, `systems/flux/flux/runners/equities`, `deploy/equities`, `ops/scripts/deploy`, `fluxboard`, `tests`, `docs/plans` | `shared` | `shared` | `b3b038c6a0` + Task 6 tracker updates | Task 0, Task 2, Task 3, Task 4, and Task 5 verification bundles passing on the shared branch; Task 6 red baseline confirmed on the lane across both focused Fluxboard verification suites | Task 0 naming alignment and Tasks 2-5 completed; Task 6 started on 2026-03-19 UTC with a dedicated lane branch/worktree provisioned for the Fluxboard implementation lane, and the implementer lane has now confirmed the red baseline with only Task 6 test-surface edits and no production changes yet |
...
| Task 6: Update Fluxboard To A Shared Equities-Arb Surface | in_progress | implementer | Task 5: Replace MakerV4 API Metadata, Strategy-Scoped Params, Signal Payload, And Readiness Contracts | `fluxboard/api.ts`, `fluxboard/components/domain/signal`, `fluxboard/config`, `fluxboard/types.ts`, `fluxboard/Params.tsx`, `fluxboard/stores.ts`, `fluxboard/tests/signal`, `fluxboard/__tests__`, `fluxboard/api.flux.test.ts`, `fluxboard/Signal.delta-pass-through.test.tsx` | `lanes/task-6-fluxboard` | `.worktrees/task-6-fluxboard` | none | `pnpm --dir fluxboard exec vitest run tests/signal/EquitiesArbSignalTable.test.tsx tests/signal/SignalFamilyFilter.test.tsx api.flux.test.ts Signal.delta-pass-through.test.tsx components/domain/signal/SignalTable.store.test.ts` failed in red phase; `pnpm --dir fluxboard exec vitest run __tests__/components/ParamsProfileColumns.test.tsx __tests__/Params.short-headers.test.tsx __tests__/config/paramsProfiles.test.ts` failed in red phase | 2026-03-19 UTC Task 6 lane opened on branch `lanes/task-6-fluxboard`; dedicated worktree provisioned. The implementer lane has now confirmed the red phase with only Task 6 test-file edits in place, no production changes yet, expected failures in `api.flux.test.ts`, `Signal.delta-pass-through.test.tsx`, `tests/signal/SignalFamilyFilter.test.tsx`, `__tests__/config/paramsProfiles.test.ts`, and `__tests__/Params.short-headers.test.tsx`, and no external blockers before green work |
