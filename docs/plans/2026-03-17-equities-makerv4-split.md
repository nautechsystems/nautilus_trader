...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Overall | in_progress | main | none | `systems/flux/flux/strategies`, `systems/flux/flux/api`, `systems/flux/flux/runners/equities`, `deploy/equities`, `ops/scripts/deploy`, `fluxboard`, `tests`, `docs/plans` | `shared` | `shared` | `29dc2906fd`, `b2b5a9ae67`, `7c6359b8fb`, `fa1098fd16`, `a73e97978d`, `f36630b37c`, `2c1a489885`, `91e11d0429`, `1b3f7ed9c9`, `9c7371c163` + Task 6 tracker updates | Task 0, Task 2, Task 3, Task 4, and Task 5 verification bundles passing on the shared branch; Task 6 focused Fluxboard verification is green on lane commit `9c7371c163` for the prior fix set (`5` files / `64` tests and `3` files / `26` tests), and the latest quality re-review has now found two more mixed-rollout compatibility gaps that require another focused fix slice | Task 0 naming alignment and Tasks 2-5 completed; Task 6 started on 2026-03-19 UTC with a dedicated lane branch/worktree provisioned for the Fluxboard implementation lane. After the fresh implementer stalled at the red-only checkpoint, the controller reclaimed local TDD execution in that lane, landed the green slice in `7c6359b8fb`, fixed follow-up spec-review findings in `fa1098fd16`, fixed the remaining `/equities/params` selector gap in `a73e97978d`, passed spec re-review, fixed the first three quality-review follow-up risks in `f36630b37c`, fixed the remaining legacy `maker_v4` rollout-compatibility gaps in `2c1a489885`, fixed the next two params-profile persistence/schema-race findings in `91e11d0429`, fixed generalized default-route profile recovery plus split-over-legacy schema selection in `1b3f7ed9c9`, and fixed the last shared-maker params leak in `9c7371c163` by hiding blended legacy maker/take controls from the `equities_maker` surface even during legacy-schema fallback. The latest quality re-review found two remaining mixed-rollout gaps: the shared maker params surface can still drop valid legacy `maker_v4` fields because `PROFILE_TO_APPLIES_TO.equities_maker` does not include legacy maker aliases, and the shared equities signal table can still mislabel/sort rows when top-level `strategy_family` disagrees with legacy `meta` because `MakerV4SignalTable.tsx` still derives the variant from `deriveStrategyProfile(row)` instead of the authoritative signal family source |
...
| Task 6: Update Fluxboard To A Shared Equities-Arb Surface | in_progress | main | Task 5: Replace MakerV4 API Metadata, Strategy-Scoped Params, Signal Payload, And Readiness Contracts | `fluxboard/api.ts`, `fluxboard/components/domain/signal`, `fluxboard/config`, `fluxboard/types.ts`, `fluxboard/Params.tsx`, `fluxboard/stores.ts`, `fluxboard/tests/signal`, `fluxboard/__tests__`, `fluxboard/api.flux.test.ts`, `fluxboard/Signal.delta-pass-through.test.tsx`, `fluxboard/vite.config.ts` | `lanes/task-6-fluxboard` | `.worktrees/task-6-fluxboard` | `9c7371c163` | Prior green bundle on lane commit `9c7371c163`: `pnpm --dir /home/ubuntu/nautilus_trader/.worktrees/task-6-fluxboard/fluxboard exec vitest run tests/signal/EquitiesArbSignalTable.test.tsx tests/signal/SignalFamilyFilter.test.tsx api.flux.test.ts Signal.delta-pass-through.test.tsx components/domain/signal/SignalTable.store.test.ts` pass (`5` files, `64` tests); `pnpm --dir /home/ubuntu/nautilus_trader/.worktrees/task-6-fluxboard/fluxboard exec vitest run __tests__/components/ParamsProfileColumns.test.tsx __tests__/Params.short-headers.test.tsx __tests__/config/paramsProfiles.test.ts` pass (`3` files, `26` tests). Current red reproduction on uncommitted lane edits: `pnpm --dir /home/ubuntu/nautilus_trader/.worktrees/task-6-fluxboard/fluxboard exec vitest run __tests__/components/ParamsProfileColumns.test.tsx` fails on missing `allow_cex_margin_sell` under legacy `makerv4` `applies_to`, and `pnpm --dir /home/ubuntu/nautilus_trader/.worktrees/task-6-fluxboard/fluxboard exec vitest run tests/signal/EquitiesArbSignalTable.test.tsx` fails because stale legacy metadata still sorts the top-level taker row as `Maker` | 2026-03-19 UTC Task 6 lane opened on branch `lanes/task-6-fluxboard`; dedicated worktree provisioned. The fresh implementer confirmed the red phase with only Task 6 test-file edits in place, then stalled without starting production changes, so the controller reclaimed local TDD execution in the same lane worktree and landed the green slice in `7c6359b8fb`. Spec review then found three follow-up gaps: the shared table dropped required hedge-policy/fee/quote-health observability, `Trading` status semantics regressed, and default Vitest path excludes still hid `components/domain/signal/SignalTable.store.test.ts` plus `__tests__/components/ParamsProfileColumns.test.tsx`; the controller fixed those gaps in `fa1098fd16`, spec re-review then found one remaining `/equities/params` issue because the profile selector still exposed non-equities legacy options, the controller fixed that final gap in `a73e97978d`, and spec re-review passed with no findings. Quality review then found three follow-up risks: the shared equities table needs a mixed-rollout fallback from `maker_v4` to `equities_arb`, metadata-only taker rows need the same family derivation as `SignalTable.tsx`, and split-family params schema selection should be deterministic across row ordering; the controller reproduced those risks with new failing focused tests, fixed them in `f36630b37c`, reran the exact plan verification bundles successfully, and dispatched a fresh quality re-review. That re-review then found three remaining mixed-rollout compatibility gaps: `/equities/signal` still dropped legacy `maker_v4` deltas, `/equities/params` still hid legacy `maker_v4` rows during a frontend-before-backend rollout, and the shared signal family filter still omitted those legacy rows from the `Maker` selection; the controller reproduced those compatibility gaps with new failing focused tests, fixed them in `2c1a489885`, reran the exact plan verification bundles successfully, and dispatched another fresh quality re-review on the updated diff. The latest quality review then found two more issues in the params surface: persisted `maker_v4` -> `equities_maker` migration was still global in `stores.ts`, which could blank the default `/params` grid during a legacy-only rollout, and the first mount of `/equities/params` could still request the wrong schema because route-profile correction and the initial `loadData()` raced. The controller reproduced those issues with new focused red tests, fixed route-effective profile resolution plus legacy profile persistence behavior in `91e11d0429`, updated the legacy migration test to keep `maker_v4` active while still copying column prefs into `equities_maker` and `equities_taker`, reran the exact Task 6 verification bundles successfully, and dispatched a fresh quality re-review on that diff. That re-review then found two remaining params issues: default `/params` could still recover into an unavailable stale equities profile and render an empty grid after cross-surface navigation, and `/equities/params` could still choose a legacy `maker_v4` schema row when legacy and split Maker rows coexisted because schema strategy selection was still grouped by the normalized route family. The controller reproduced both gaps with new focused red tests, fixed generalized default-route profile recovery plus split-over-legacy schema selection in `1b3f7ed9c9`, reran the exact Task 6 verification bundles successfully, and dispatched another fresh quality re-review on that diff. That re-review found one remaining shared-maker params issue: when `/equities/params` falls back to a legacy `maker_v4` schema, the shared Maker surface could still leak blended-contract fields such as `execution_mode` and taker-only keys because the `equities_maker` hidden-key contract did not yet suppress them. The controller reproduced that leak with a focused red surface test, fixed the `equities_maker` hidden-key contract in `9c7371c163` without removing those controls from the real `equities_taker` profile, reran the exact Task 6 verification bundles successfully, and dispatched another fresh quality re-review on that diff. That re-review then found two remaining mixed-rollout compatibility gaps: the shared maker params surface can still drop valid legacy `maker_v4` fields because `PROFILE_TO_APPLIES_TO.equities_maker` does not include the legacy maker aliases expected by the fallback schema path, and the shared equities signal table can still mislabel/sort rows when top-level `strategy_family` disagrees with legacy `meta` because `MakerV4SignalTable.tsx` still derives the variant from `deriveStrategyProfile(row)` instead of the authoritative signal family source. The controller has now reproduced both gaps with focused red tests on the lane worktree and is applying the minimal production fix slice next |
