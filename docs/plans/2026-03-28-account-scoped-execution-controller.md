...
| Task 11: Migrate TokenMM shared Binance writer domains after the multi-box gates pass | in_progress | controller | Task 10: Add replicated ownership-log and multi-box stale-writer rejection hardening | `deploy/tokenmm/tokenmm.live.toml`, `deploy/tokenmm/systemd/flux-tokenmm.target`, `ops/scripts/deploy/install_tokenmm_systemd.sh`, `systems/flux/flux/runners/tokenmm/run_node.py`, `systems/flux/flux/runners/tokenmm/run_controller.py`, `systems/flux/flux/runners/tokenmm/run_api.py`, `systems/flux/flux/api/app.py`, `docs/runbooks/tokenmm-risk-validation.md`, `tests/unit_tests/examples/strategies/test_tokenmm_stack_contract.py`, `tests/unit_tests/examples/strategies/test_tokenmm_run_node.py`, `tests/unit_tests/examples/strategies/test_tokenmm_run_api.py`, `tests/unit_tests/examples/strategies/test_tokenmm_run_controller.py`, `tests/unit_tests/flux/api/test_balances_merge_dedupe.py` | `codex/account-execution-controller-platform-cut-20260328` | `/home/ubuntu/nautilus_trader/.worktrees/account-execution-controller-platform-cut-20260328` | `ad8f7908fe..working_tree` | `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/pytest -q tests/unit_tests/examples/strategies/test_tokenmm_stack_contract.py` FAIL (`3 failed, 55 passed`) on the same pre-existing unrelated monitoring/docs failures outside Task 11 scope; `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/pytest -q tests/unit_tests/examples/strategies/test_tokenmm_stack_contract.py -k 'live_config_binds_shared_binance_writer_domains_to_a_controller_scope or systemd_contract_adds_a_controller_service_for_managed_binance_domains'` PASS (`2 passed, 56 deselected`); `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/pytest -q tests/unit_tests/examples/strategies/test_tokenmm_run_api.py` PASS (`33 passed`); `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/pytest -q tests/unit_tests/flux/api/test_balances_merge_dedupe.py` PASS (`12 passed`); `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/pytest -q tests/unit_tests/examples/strategies/test_tokenmm_risk_validation_contract.py` PASS (`4 passed`); `python ops/scripts/failover/controller_scope_failover.py --profile tokenmm --scope binance.pm.main --multi-box --check-thresholds` PASS; `git diff --check` PASS | 2026-03-29: user approved reopening Task 11 with a narrow addendum at the TokenMM runner seam. The expanded write scope adds `systems/flux/flux/runners/tokenmm/run_node.py` plus dedicated runner tests so the managed Binance nodes can disable node-owned execution/startup reconciliation, publish controller requests, and consume controller canonical state without widening into `makerv3` internals unless a failing red proves the runner seam is insufficient. The controller half of the task must now grow from lease-only scaffolding into the actual request/reply, venue-write, and startup-reconciliation owner for `tokenmm.binance.pm.main`; fresh red tests are next |
