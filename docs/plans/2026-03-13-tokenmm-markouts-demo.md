...
| --- | --- | --- | --- | --- | --- |
| Task 1: Add helper tests and module | completed | none | `tests/unit_tests/research/test_telemetry_helpers.py`, `research/tokenmm/telemetry_helpers.py` | `python3 -m pytest tests/unit_tests/research/test_telemetry_helpers.py -q PASS` | Helper module implemented and green on targeted unit tests. |
| Task 2: Add Redis FV freeze script | completed | Task 1: Add helper tests and module | `ops/scripts/export_tokenmm_markout_inputs.py` | `python3 ops/scripts/export_tokenmm_markout_inputs.py --help PASS` | Script implemented with config-aware Redis URL defaults and password overrides. |
| Task 3: Build demo notebook | completed | Task 1: Add helper tests and module | `research/tokenmm/notebooks/tokenmm_markouts_edge_pnl_demo.ipynb` | `python3 notebook cell executor PASS` | Notebook built with a SQLite-first core and optional frozen-FV sections. |
| Task 4: Validate notebook and spot-check outputs | completed | Task 2: Add Redis FV freeze script, Task 3: Build demo notebook | repo-local validation only | `json parse PASS; notebook exec PASS; raw SQLite spot-check PASS` | Notebook JSON validated, code cells executed end-to-end on a local snapshot, and sample fill/markout joins were spot-checked against raw SQLite rows. |
