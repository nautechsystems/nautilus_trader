...
| Task 2: Make trades health truthful for canonical and non-canonical views | in_review_spec | spec-reviewer | Task 1: Make resync completion surface-aware | `fluxboard/Trades.tsx`, `fluxboard/lib/realtime/types.ts`, `fluxboard/__tests__/trades-status.test.tsx`, `fluxboard/Trades.test.tsx`, `fluxboard/__tests__/trades-integration.test.tsx` | `fix/trades-blotter-hardening-20260326` | `.worktrees/trades-blotter-hardening` | `working tree diff on top of bacd41fb5f` | `PASS: \`VITEST_FULL=1 pnpm --dir fluxboard exec vitest run __tests__/trades-status.test.tsx Trades.test.tsx __tests__/trades-integration.test.tsx\`` | Non-canonical snapshot views now report `LIVE` when fresh; reconnect coverage preserves `OFFLINE` -> `RECOVERING` -> `LIVE` on canonical catch-up |
