...
| --- | --- | --- | --- |
| Overall | blocked | main | `2026-03-16 06:16 UTC` live readiness remains green after a fresh rerun: `./ops/scripts/deploy/check_equities_live_readiness.sh --json` is `ok=true`, `/api/v1/balances?profile=equities` is `source=\"portfolio_snapshot_v2\"`, `degraded=false`, and the MakerV4 local pre-gate bundle is still green (`84 passed, 47 deselected`). The canary remains blocked only on Hyperliquid funded-account request headroom, which still reports `nRequestsSurplus=0`. |
...
| Task 8: Enable Controlled Live Trading Canary | blocked | main | `2026-03-16 06:16 UTC` reran the canary gates after the IBKR recovery note. Shared `/equities` surfaces are still green: `./ops/scripts/deploy/check_equities_live_readiness.sh --json` -> `ok=true`; `/api/v1/balances?profile=equities` -> `degraded=false`, `stale_required=[]`; local MakerV4 pre-gate verification -> `84 passed, 47 deselected`. The remaining blocker is Hyperliquid funded-account quota only: `sudo -n env PYTHONPATH=/home/ubuntu/nautilus_trader/.worktrees/equities-live-pr /home/ubuntu/nautilus_trader/.worktrees/equities-live-pr/.venv/bin/python /home/ubuntu/nautilus_trader/.worktrees/equities-live-pr/ops/scripts/deploy/hyperliquid_request_quota.py --show-only` -> `nRequestsSurplus=0`. The one-per-side MakerV4 canary stays paused until request headroom is positive. |

## March 13 MakerV4 Review Follow-On

- A focused MakerV4 cutover review/implementation plan now exists in `docs/plans/2026-03-13-makerv4-hl-ibkr-prod-cutover.md`.
- `2026-03-13 17:18 UTC`: execution of the MakerV4 cutover plan started from this worktree under `executing-plans`; the primary plan's `## Progress Tracker` is now the source of truth for batch 1 (Tasks 1-3), and this readiness tracker remains the live-rollout ledger.
- `2026-03-13 17:25 UTC`: Task 1 red-state verification from the MakerV4 cutover plan is captured in the primary tracker. The focused Makerv4 bundle is now failing on the intended live-readiness gaps: missing maker quote lifecycle, stubbed live hedge submission path, missing distinct maker-vs-hedge state payload, the pre-existing balances/provider contract failure, and a mixed-family `/balances` coexistence gap where V3+V4 rows drop expected `portfolio_snapshot_v2` source metadata.
- `2026-03-13 17:37 UTC`: batch 1 review checkpoint reached in the MakerV4 cutover plan. Task 2 and Task 3 strategy slices are now green from this worktree, but rollout work remains blocked on two control-plane/balance contract failures tracked in the primary MakerV4 plan: strategy-published reference snapshot rows and mixed-family `/balances` provenance under MakerV3+MakerV4 coexistence.
- `2026-03-13 17:52 UTC`: follow-up review against batch 1 validated three real spec gaps in the primary MakerV4 plan: the wrong strategy-owned IBKR balance expectation, missing MakerV4 shared-account position reconciliation via the profile-account-projection feed, and missing maker-order lifecycle reconciliation. The primary MakerV4 plan has been corrected in place to treat MakerV4 as a one-per-side canary for this wave and to require shared-account truth before any live cutover.
- March 13 review findings from the worktree:
  - MakerV4 is conceptually aligned with the intended HL-maker / IBKR-immediate-hedge model.
  - MakerV4 is not yet live-ready: `systems/flux/flux/strategies/makerv4/strategy.py` still has no MakerV3-equivalent maker quote-management path and `_submit_hedge_intent(...)` is a stub that returns `None`.
  - Existing mixed-family control-plane wiring is partially in place already: `test_equities_run_node.py -k 'makerv4 or maker_v4'` passed (`6 passed`), and `test_equities_profile_contract.py -k 'makerv4 or mixed_family'` passed (`2 passed`).
  - The current Makerv4 unit slice is not fully green even before new work: `tests/unit_tests/flux/strategies/makerv4/test_strategy.py` is `54 passed, 1 failed`, with the existing failing test in `test_makerv4_publish_balances_includes_reference_snapshot_rows`.
- Treat the new MakerV4 plan as the execution source of truth for strategy-family cutover work; this readiness plan remains the source of truth for live rollout state.
- `2026-03-13 18:18 UTC` corrected MakerV4 batch-2 red state is now explicitly captured in the worktree tests before any further implementation:
  - `tests/unit_tests/flux/strategies/makerv4/test_strategy.py -k 'profile_owned or maker_order_cancel_reconciles'` -> `1 failed, 1 passed, 21 deselected`
  - `tests/unit_tests/flux/strategies/makerv4/test_inventory_contract.py -k 'shared_account_projection_when_local_cache_is_flat'` -> `1 failed, 5 deselected`
  - `tests/unit_tests/examples/strategies/test_equities_run_node.py -k 'real_makerv4_strategy_receives_profile_account_projection_feed'` -> `1 failed, 26 deselected`
  - failures are the intended design gaps from review: missing MakerV4 `configure_profile_account_projection_feed(...)`, missing shared-account inventory fallback, and missing maker-order cancel reconciliation. No canary/live rollout work should proceed until those are green.
- `2026-03-13 18:32 UTC` MakerV4 strategy-side correction work is green in the worktree:
  - `tests/unit_tests/flux/strategies/makerv4/test_strategy.py::{test_makerv4_maker_order_cancel_reconciles_managed_order_state,test_makerv4_maker_order_reject_reconciles_managed_order_state,test_makerv4_maker_order_expire_reconciles_managed_order_state,test_makerv4_publish_balances_keeps_ibkr_reference_rows_profile_owned}` -> `4 passed`
  - `tests/unit_tests/flux/strategies/makerv4/test_inventory_contract.py::test_makerv4_state_snapshot_uses_shared_account_projection_when_local_cache_is_flat` + `tests/unit_tests/examples/strategies/test_equities_run_node.py::test_build_node_real_makerv4_strategy_receives_profile_account_projection_feed` -> `2 passed`
  - `tests/unit_tests/examples/strategies/test_equities_run_node.py -k 'makerv4 or maker_v4'` -> `7 passed, 21 deselected`
  - remaining MakerV4 batch-2 blocker is now isolated to the API control plane: `tests/unit_tests/flux/api/test_equities_profile_contract.py::test_balances_profile_equities_preserves_shared_account_provenance_when_makerv3_and_makerv4_coexist` still fails because the equities `/balances` handler drops out of the `portfolio_snapshot_v2` branch when the snapshot has fresh shared-account rows but an empty `inventory_by_asset`.
- `2026-03-13 18:39 UTC` corrected MakerV4 batch-2 fix set is green in the worktree:
  - `tests/unit_tests/flux/strategies/makerv4/test_strategy.py -k 'profile_owned or maker_order_cancel_reconciles'` -> `2 passed, 23 deselected`
  - `tests/unit_tests/flux/strategies/makerv4/test_inventory_contract.py -k 'shared_account_projection_when_local_cache_is_flat'` -> `1 passed, 5 deselected`
  - `tests/unit_tests/examples/strategies/test_equities_run_node.py -k 'makerv4 or maker_v4'` -> `7 passed, 21 deselected`
  - `tests/unit_tests/flux/api/test_equities_profile_contract.py -k 'preserves_shared_account_provenance_when_makerv3_and_makerv4_coexist or makerv4 or mixed_family'` -> `3 passed, 25 deselected`
  - `tests/unit_tests/flux/api/test_equities_profile_contract.py` -> `28 passed`
  - `tests/unit_tests/flux/strategies/makerv4` -> `65 passed`
  - `tests/unit_tests/flux/strategies/makerv3/test_quote_engine.py tests/unit_tests/flux/strategies/makerv4/test_strategy.py` -> `48 passed`
  - `tests/unit_tests/flux/api/test_payloads.py -k 'combine_portfolio_snapshot_rows'` -> `1 passed`
  - `git diff --check` -> clean
- Result: the current gating items from review are resolved in local/unit/API verification. Live canary work is still intentionally deferred to the separate MakerV4 cutover plan and should not start until a human review accepts this batch.
- `2026-03-13 18:47 UTC` follow-up review found one remaining MakerV4 canary blocker after the otherwise-green batch: maker fill events still do not reconcile the filled maker-side order out of `_managed_maker_orders`, so the next quote refresh can incorrectly believe that side is still open and skip replacement. The primary MakerV4 plan has Task 2 reopened for a test-first fix; Task 5 remains out of scope until that is green.
- `2026-03-13 18:53 UTC` MakerV4 maker-fill reconciliation fix is now green in the worktree:
  - `tests/unit_tests/flux/strategies/makerv4/test_strategy.py -k 'filled_maker_quote_before_pending_hedge or maker_fill_allows_next_refresh_to_requote'` -> `2 passed, 24 deselected`
  - `tests/unit_tests/flux/strategies/makerv4/test_strategy.py -k 'fill or reconcile or quote_state'` -> `12 passed, 14 deselected`
  - `tests/unit_tests/flux/strategies/makerv4` -> `66 passed`
  - `tests/unit_tests/flux/strategies/makerv3/test_quote_engine.py tests/unit_tests/flux/strategies/makerv4/test_strategy.py` -> `49 passed`
  - `git diff --check` -> clean
- Result: the fill-reconciliation canary blocker raised in follow-up review is resolved locally. Task 5 remains intentionally untouched until human review decides whether to proceed to the live canary step.
- `2026-03-13 18:58 UTC` human review approved moving from Tasks 1-4 into Task 5. Current live work is still at the pre-gate stage only: verify Hyperliquid request headroom, rerun the required local/unit/API bundle from the primary MakerV4 plan, then prepare a single-symbol one-per-side canary with explicit rollback to MakerV3.
- `2026-03-13 19:34 UTC` cross-stack follow-up plan created at `docs/plans/2026-03-13-equities-cross-stack-hardening-followups.md`. This is the umbrella implementation plan for post-review hardening that is not cleanly contained inside the existing MakerV4 cutover plan: fail-closed gating semantics, maker managed-order lifecycle correctness, IBKR hedge metadata propagation, mixed-family balance row identity, Hyperliquid quota tooling/telemetry, and repo-wide parity/regression cleanup. Use it alongside the MakerV4 cutover plan, not as a replacement for the live canary gate.
- `2026-03-13 20:11 UTC` cross-stack follow-up Tasks 1-3 are now green in the worktree. New coverage proves: MakerV4 stops quoting when hedge disable paths trip, partial maker fills and restart-time claimed orders preserve maker-side truth, and actual submitted IBKR hedge orders now carry `IBOrderTags(outsideRth=True)` through the Nautilus order path. Verification:
  - `tests/unit_tests/flux/strategies/makerv4` -> `71 passed`
  - `tests/unit_tests/flux/strategies/makerv4 tests/integration_tests/adapters/interactive_brokers/test_execution_order_transform.py -k 'outside_rth or outsideRth or IBOrderTags or hedge'` -> `24 passed, 74 deselected`
  - `git diff --check` -> clean
- `2026-03-13 20:18 UTC` cross-stack follow-up Task 4 is now green in the worktree. Shared `portfolio_snapshot_v2` position row ids are now account-unique, and the equities `/balances` contract keeps distinct stable ids when the same instrument exists on multiple shared accounts. Verification:
  - `tests/unit_tests/flux/api/test_payloads.py tests/unit_tests/flux/api/test_equities_profile_contract.py -k 'distinct_position_row_ids_per_account or row_id or provenance or portfolio_snapshot_v2'` -> `6 passed, 79 deselected`
  - `tests/unit_tests/flux/api/test_equities_profile_contract.py tests/unit_tests/flux/api/test_payloads.py` -> `85 passed`
  - `git diff --check` -> clean
- `2026-03-13 20:24 UTC` cross-stack follow-up Tasks 5-6 are now green in the worktree. Hyperliquid quota tooling works from the checkout with a readable env source, MakerV3 and MakerV4 now share venue-protection/quota parsing, and the repo-wide equities/tokenmm regression bundle is green. Verification:
  - `./.venv/bin/python ops/scripts/deploy/hyperliquid_request_quota.py --common-env <tmp> --show-only` -> live `cumVlm=5856.56 nRequestsUsed=17118 nRequestsCap=15856 nRequestsSurplus=0`
  - `tests/unit_tests/flux/strategies/makerv3/test_order_safety.py tests/unit_tests/flux/strategies/makerv4/test_strategy.py -k 'quota or rate_limit or venue_protection'` -> `7 passed, 44 deselected`
  - `cargo test -p nautilus-hyperliquid --test exec_client -- --nocapture` -> `21 passed`
  ...
  - `git diff --check` -> clean
- Result: the cross-stack code hardening work is complete locally. Remaining blockers are operational/live only: restore IBKR auth, verify usable Hyperliquid request headroom on the funded account, and then run the explicit one-per-side MakerV4 canary.
- `2026-03-16 07:42 UTC` follow-up plan created at `docs/plans/2026-03-16-makerv4-take-take-and-overnight-hedge.md`. This plan covers the remaining production-grade Makerv4 work after the baseline canary code: session-aware IBKR overnight hedge policy (`SMART + includeOvernight`, not overnight IOC), fee-aware pricing/observability, and an explicit `take_take` execution mode that aggresses HL first and hedges on IBKR after fill confirmation. Use it alongside the existing MakerV4 cutover plan, not as a replacement for the current Hyperliquid request-headroom gate.
...
## 2026-03-13 14:36 UTC Update

- Reproduced the Fluxboard params-save failure end to end on the public `:5022` surface. `PATCH /api/v1/params` against `http://13.213.194.42:5022` failed with `unknown_strategy_id` for equities strategies, while the same request succeeded locally on `http://127.0.0.1:5024`.
- Root cause was profile-scoped routing drift in `fluxboard/api.ts`: equities reads already appended `?profile=equities`, but params writes and strategy-specific config/params requests were still hitting bare `/api/v1/...` paths. Public `:5022` is served by the TokenMM API process, so unscoped write requests were validated against the TokenMM allowlist and rejected with `Strategy is not configured for this API.`
- Added focused Fluxboard regressions in `fluxboard/api.flux.test.ts` covering equities-scoped params writes and strategy config requests, then patched the shared API client path builder so those requests now include the active route profile before fetch/signing.
- Verification:
  - `pnpm --dir fluxboard exec vitest run api.flux.test.ts` -> `45 passed`
  - `pnpm --dir fluxboard exec vitest run __tests__/api.test.ts` -> `30 passed`
- `pnpm --dir fluxboard build`
- Public `http://13.213.194.42:5022/equities` now serves rebuilt assets `assets/index-6WB-g714.js` and `assets/index-BCpW5E6y.css`
- Operator note: existing browser tabs may still hold the old bundle; a hard refresh is required before retrying param edits from the public UI.

## 2026-03-13 15:00 UTC Update

- Root-caused the live `amd_tradexyz_makerv3` `blocked_portfolio_inventory` quote block. Shared portfolio inventory was healthy under canonical asset key `flux:v1:portfolio:inventory:equities:AMD`, but Makerv3 quote gating in `quote_engine.py` was still checking the aggregate under the maker base currency (`XYZ:AMD`) instead of the configured canonical `portfolio_asset_id` (`AMD`).
- Added a regression in `tests/unit_tests/flux/strategies/makerv3/test_quote_engine.py` proving that a strategy with maker base `XYZ:AMD` and canonical `portfolio_asset_id = "AMD"` must accept the shared portfolio aggregate keyed by `AMD`.
- Patched `systems/flux/flux/strategies/makerv3/quote_engine.py` to use `strategy._portfolio_asset_id()` for the shared portfolio inventory gate, falling back to maker base only if no canonical asset id is configured.
- Verification:
  - `./.venv/bin/pytest -q tests/unit_tests/flux/strategies/makerv3/test_quote_engine.py -p no:rerunfailures` -> `23 passed`
  - `git diff --check`
- Live verification after restarting only `flux@equities-node-amd_tradexyz_makerv3` from the worktree:
  - direct `PATCH /api/v1/params?profile=equities` with `bot_on=true` now yields `tradeable=true`, `blocked=false`, and `state=quotes_replaced` for AMD
  - fresh journald confirms the strategy is generating buy/sell Hyperliquid quote intents off the IBKR reference leg
  - the remaining AMD canary blocker is now purely operational: the node still has no Hyperliquid execution client, and ExecEngine logs `Cannot execute command: no execution client found for client_id=None, venue=HYPERLIQUID`
