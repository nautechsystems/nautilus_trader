...
| Task 4: Reconcile Live Balances And Portfolio Snapshot Inputs | in_progress | main | Starting TDD to remove the remaining legacy assumptions that still keep `/api/v1/balances?profile=equities` empty or degraded even after shared account-provider wiring is fixed. |
