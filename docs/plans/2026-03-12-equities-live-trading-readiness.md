...
| Task 8: Enable Controlled Live Trading Canary | in_progress | main | `2026-03-13 10:07 UTC` pre-canary prep is materially cleaner, but a new Hyperliquid account-surface gap is now explicitly tracked before broader live enablement. Fresh live evidence showed the funded master `0x6ed25...` does have live `xyz` perp positions (`NVDA`, `COIN`, `GOOGL`) when queried with `clearinghouseState(dex="xyz")`, while the current equities balances surface still shows only Hyperliquid cash rows (`USDC`, `USDE`, `USDH`). The raw discrepancy is now documented in `docs/plans/2026-03-13-hyperliquid-xyz-account-surface.md`: `clearinghouseState` without `dex` returns the wrong empty perp scope for this account, and the current adapter/provider path is still dropping the builder-deployed `xyz:*` positions. Until that plan lands, the balances/model surface remains incomplete for Hyperliquid and should not be treated as the final risk view for live canary expansion. |
