...
| Task 4: Reconcile Live Balances And Portfolio Snapshot Inputs | in_progress | main | Runtime/provider fix is implemented locally: dockerized IBKR account providers now force `ibg_port=None`, explicit `ibg_client_id=0` is preserved, shared IBKR scope client IDs are separated from node client IDs, and new regressions cover the shared-scope provider path plus client-id overlap. Local verification: `38 passed` across `test_equities_run_portfolio.py` + `test_equities_run_node.py`, `25 passed` in `test_equities_stack_contract.py`, `git diff --check` clean. Next step is review + live service restart to confirm profile account projections and non-degraded balances. |
