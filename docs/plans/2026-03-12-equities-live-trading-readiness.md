...
| --- | --- | --- | --- |
| Overall | in_progress | main | Tasks 1-7A are closed on branch head. Shared-account balances visibility is now correct live from the worktree; the remaining hold before any canary work is runtime freshness/readiness. Task 7B is in progress with the shared websocket reconnect probe/stabilization patch plus Hyperliquid subscription-state replay fixes landed locally; live re-verification is still pending. |
...
| Task 7B: Fix Hyperliquid Reconnect Freshness Loop | in_progress | main | `2026-03-12`: live node journals showed repeated `Failed to send message: WebSocket protocol error: Sending after closing is not allowed` immediately after `Received RECONNECTED sentinel` and `Resubscribing to ... active subscriptions after reconnection`. Local work now covers two layers: shared websocket reconnect probe/stabilization in `nautilus-network`, and Hyperliquid subscription-state confirmation plus confirmed-only replay on reconnect. Local verification is green for the new Hyperliquid unit slice and `nautilus-network --no-run`; the next step is host-side redeploy and readiness recheck. |
