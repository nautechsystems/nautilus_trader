...
- IBKR remains the shared reference/account side for equities live trading
...
1. Does the design create one clear recovery owner per node plus instrument?
...
4. Are Hyperliquid and Binance repair responsibilities assigned at the right layer?
...
- one recovery owner per node plus instrument
...
2. One node plus instrument has one recovery state machine.
...
One supervisor instance exists per `TradingNode`. It tracks each quote-backed instrument used by attached strategies and owns the recovery lifecycle for that node plus instrument pair.

Its responsibilities are:

- maintain desired subscription state
- record last successful quote timestamp per instrument
- track active strategy claimants for each instrument
...
- registers its tracked instruments and runtime freshness budgets with the shared supervisor during startup
...
Each node-scoped instrument feed follows this state machine:
...
- `blocked`
  A required precondition is missing, for example venue session/auth not ready, client cache not hydrated, or market/session policy disallows live recovery.
- `recovering`
...
- strategy quote placement, hedge placement, and any live-quote-dependent execution path must be gated off while required feeds are not tradeable

### 5. Recovery Execution Contract

The supervisor needs one canonical command emitter per node-scoped instrument.

For V1, the runner should wire one shared reset emitter owned by the node build path, not by an arbitrary sibling strategy object. The key point is that only one shared owner may perform the side effect for a given node plus instrument pair, and that owner should remain stable even if one strategy is later refactored or removed.
...
2. Apply the canonical lifecycle action:
   - startup subscribe when the first claimant registers
   - bounded reset when a stale feed is admitted for recovery
   - shutdown unsubscribe when the last claimant deregisters
3. Prime cached quote state to stale locally.
4. Invoke venue-specific reset logic.
5. Start a bounded observation window.
6. Mark `healthy` only after a newer quote arrives.
...
- quote resets must be idempotent per instrument
...
- logs show one recovery owner and one attempt sequence per node plus instrument
- we can answer "why is this row stale?" without tailing multiple sibling strategy logs
- supervisor state transitions emit structured logs with instrument id, node-group id, prior state, new state, attempt count, and last adapter error
...
- balances remain non-degraded under the canonical readiness gate
