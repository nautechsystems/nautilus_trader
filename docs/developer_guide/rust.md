...
6. **Error Codes**: All `thiserror` error types must include structured error codes in
   the format `[NT-XX-YYYZZ]` where:

   - `NT` — project prefix (NautilusTrader).
   - `XX` — two-letter crate indicator (e.g., `MD` for model, `CR` for core). The full
     mapping is defined in `CRATE_ABBREVIATIONS` in `crates/xtask/src/main.rs`.
   - `YYY` — three-digit error type identifier, unique within the crate.
   - `ZZ` — two-digit variant identifier within the error type.
   ...
   The error code index at `docs/error-codes.md` is auto-generated by the xtask:

   ```bash
   cargo run -p nautilus-xtask -- error-index          # Generate
   cargo run -p nautilus-xtask -- error-index --check   # Verify (CI)
   ```

   The xtask validates no duplicate codes, no missing codes within adopted types, and
   that crate indicators match the crate where the error is defined.
