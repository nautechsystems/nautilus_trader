...
Commands:
  start      Local smoke only: build and start redis (optional), N nodes, 1 portfolio, 1 bridge, and 1 API + UIs
  stop       Local smoke only: stop API, bridge, portfolio, all node processes, and managed redis process
  restart    Stop then start
  status     Show process and endpoint status
  health     Run health checks for API, tokenmm route, and socket handshake
  logs <svc> Tail service log (svc: redis|portfolio|bridge|api|node_<strategy_slug>)
...
Production note:
  ...
  at /pulse after installing flux@ units with scripts/deploy/install_tokenmm_systemd.sh.
USAGE
}
...
validate_mode() {
  ...
  if [[ "${MODE}" == "live" ]]; then
    echo "[tokenmm-stack] live deployments are not supported via tokenmm_stack.sh" >&2
    echo "[tokenmm-stack] install flux@ services with scripts/deploy/install_tokenmm_systemd.sh" >&2
    echo "[tokenmm-stack] manage services and nodes from Pulse at /pulse or /api/pulse/*" >&2
    exit 1
  fi
}
...
build_pulse_ui() {
  if [[ "${SKIP_PULSE_BUILD}" == "1" ]]; then
    echo "[tokenmm-stack] skipping pulse ui build (TOKENMM_SKIP_PULSE_BUILD=1)"
    return
  fi
  require_cmd pnpm
  if [[ ! -d "${ROOT_DIR}/pulse-ui/node_modules" ]]; then
    echo "[tokenmm-stack] installing pulse ui dependencies"
    pnpm --dir "${ROOT_DIR}/pulse-ui" install --frozen-lockfile
  fi
  echo "[tokenmm-stack] building pulse ui"
  pnpm --dir "${ROOT_DIR}/pulse-ui" build
}
...
start_stack() {
  ...
  local -a portfolio_cmd=(
    ...
    "${pybin}"
    -m
    nautilus_trader.flux.runners.tokenmm.run_portfolio
    --config
    "${CONFIG_PATH}"
    --mode
    "${MODE}"
  ...
  if [[ -n "${live_flag}" ]]; then
    portfolio_cmd+=("${live_flag}")
  fi
  start_process "portfolio" "${portfolio_cmd[@]}"
  ...
  api_cmd+=(--host "${API_HOST}" --port "${API_PORT}" --serve-fluxboard --serve-pulse)
...
logs_stack() {
  local svc="${1:-}"
  if [[ -z "${svc}" ]]; then
    echo "[tokenmm-stack] choose service: redis|portfolio|bridge|api|node_<strategy_slug>" >&2
    exit 1
  fi
