...
class BinanceUserTrade(msgspec.Struct, frozen=True):
    """
    HTTP response from `Binance Spot/Margin` `GET /api/v3/myTrades` HTTP
    response from `Binance USD-M Futures` `GET /fapi/v1/userTrades` HTTP
    response from `Binance COIN-M Futures` `GET /dapi/v1/userTrades`
    """
...
class BinanceOrder(msgspec.Struct, frozen=True):
    """
    HTTP response from `Binance Spot/Margin` `GET /api/v3/order` HTTP response
    from `Binance USD-M Futures` `GET /fapi/v1/order` HTTP response from
    `Binance COIN-M Futures` `GET /dapi/v1/order`
    """
