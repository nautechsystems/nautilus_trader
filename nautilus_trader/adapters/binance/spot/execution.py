...
from nautilus_trader.adapters.binance.spot.parsing.account import parse_spot_account_balances_http
from nautilus_trader.adapters.binance.spot.parsing.account import parse_spot_account_balances_ws
...
class BinanceSpotExecutionClient(LiveExecutionClient):
    ...
    def __init__(
        ...
        # Parsers
        self._exec_parser = BinanceSpotExecutionParser()
    ...
    def _update_account_state(self, info: BinanceSpotAccountInfo) -> None:
        self.generate_account_state(
            balances=parse_spot_account_balances_http(raw_balances=info.balances),
            margins=[],
            reported=True,
    ...
    async def generate_order_status_report(
        ...
        report: OrderStatusReport = self._exec_parser.parse_binance_order_report_http(
    ...
    async def generate_order_status_reports(  # noqa (C901 too complex)
        ...
        try:
            open_order_msgs: list[BinanceOrder] = await self._http_account.get_open_orders(
    ...
    async def generate_trade_reports(  # noqa (C901 too complex)
        ...
        for data in reports_raw:
            ...
            report: TradeReport = self._exec_parser.parse_binance_trade_report_http(
                account_id=self.account_id,
    ...
    def _handle_account_update(self, msg: BinanceSpotAccountUpdateMsg) -> None:
        self.generate_account_state(
            balances=parse_spot_account_balances_ws(raw_balances=msg.B),
            margins=[],
            reported=True,
    ...
    def _handle_execution_report(self, data: BinanceSpotOrderUpdateData) -> None:
        ...
        if data.x == BinanceExecutionType.NEW:
            ...
            self.generate_order_filled(
                ...
                order_type=self._exec_parser.parse_binance_order_type(data.o),
    ...
    def _generate_external_order_report(
        ...
        report = OrderStatusReport(
            ...
            order_type=self._exec_parser.parse_binance_order_type(data.o),
            time_in_force=self._exec_parser.parse_binance_time_in_force(data.f),
