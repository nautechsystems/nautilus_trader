...
from nautilus_trader.adapters.binance.futures.parsing.account import (
    parse_futures_account_balances_http,
)
from nautilus_trader.adapters.binance.futures.parsing.account import (
    parse_futures_account_balances_ws,
)
from nautilus_trader.adapters.binance.futures.parsing.account import (
    parse_futures_account_margins_http,
)
...
class BinanceFuturesExecutionClient(LiveExecutionClient):
    ...
    def __init__(
        ...
        # Parsers
        self._execution_parser = BinanceFuturesExecutionParser()
    ...
    async def _connect(self) -> None:
        ...
        msg: BinanceListenKey = await endpoint.create_listen_key()
    ...
    async def _update_account_state(
        ...
        self.generate_account_state(
            balances=parse_futures_account_balances_http(assets=account_info.assets),
            margins=parse_futures_account_margins_http(assets=account_info.assets),
            reported=True,
    ...
    async def _ping_listen_keys(self) -> None:
        while True:
            ...
            if self._listen_key:
                ...
                endpoint = self._http_user.endpoint_listenkey
                parameters = endpoint.PutDeleteParameters(
    ...
    async def _sync_clock_with_binance_server(self) -> None:
        while True:
            ...
            # )
            server_time = await self._http_market.endpoint_time.request_server_time()
    ...
    async def generate_order_status_report(
        ...
        try:
            binance_order: Optional[BinanceOrder]
            if venue_order_id:
        ...
        report: OrderStatusReport = self._execution_parser.parse_binance_order_report_http(
    ...
    async def generate_order_status_reports(  # noqa (C901 too complex)
        ...
        try:
            ...
            binance_open_orders: list[BinanceOrder]
    ...
    async def generate_trade_reports(  # noqa (C901 too complex)
        ...
        try:
            ...
            for data in binance_positions:
                if Decimal(data.positionAmt) == 0:
                    continue  # Flat position
                # Add active symbol
                active_symbols.append(data.symbol)
        ...
        for trade in binance_trades:
            report: TradeReport = self._execution_parser.parse_binance_trade_report_http(
                account_id=self.account_id,
    ...
    async def generate_position_status_reports(
        ...
        for data in binance_positions:
            ...
            report: PositionStatusReport = (
                self._execution_parser.parse_futures_position_report_http(
                    account_id=self.account_id,
                    instrument_id=self._get_cached_instrument_id(data.symbol),
                    data=data,
                    report_id=UUID4(),
    ...
    async def _submit_limit_order(self, order: LimitOrder) -> None:
        ...
        await self._http_account.new_order(
            symbol=BinanceSymbol(order.instrument_id.symbol.value),
            side=order_side_to_str(order.side),
            type=self._execution_parser.parse_binance_order_type(order).value,
    ...
    def _handle_account_update(self, msg: BinanceFuturesAccountUpdateMsg) -> None:
        self.generate_account_state(
            balances=parse_futures_account_balances_ws(raw_balances=msg.a.B),
            margins=[],
            reported=True,
    ...
    def _handle_order_trade_update(self, msg: BinanceFuturesOrderUpdateMsg) -> None:
        ...
        if data.x == BinanceExecutionType.NEW:
            ...
            self.generate_order_filled(
                ...
                order_type=self._execution_parser.parse_binance_order_type(data.o),
    ...
    def _generate_external_order_report(
        ...
        report = OrderStatusReport(
            ...
            order_side=OrderSide.BUY if data.S == BinanceOrderSide.BUY else OrderSide.SELL,
            order_type=self._execution_parser.parse_binance_order_type(data.o),
            time_in_force=self._execution_parser.parse_binance_time_in_force(data.f),
            ...
            trigger_type=self._execution_parser.parse_binance_trigger_type(data.wt.value),
            ...
            avg_px=None,
            post_only=data.f == BinanceTimeInForce.GTX,
            reduce_only=data.R,
            report_id=UUID4(),
