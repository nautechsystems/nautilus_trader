...
class PolymarketExecClientConfig(LiveExecClientConfig, frozen=True):
    ...
    use_rust : bool, default False
        If True, uses the Rust ``py_clob_rust_adaptor`` to sign orders before
        posting them via the Python CLOB client. Posting always goes through
        the Python client. All other operations (market data, account queries,
        cancellations, etc.) continue using the Python client.
