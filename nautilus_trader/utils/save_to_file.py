...
def save_struct_to_file(filepath: str, obj, force_create=False):
...
def save_dict_to_file(filepath: str, obj: dict, force_create=False):
    item_json = json.dumps(obj, indent=4)
    if not force_create and os.path.isfile(filepath):
        return
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(item_json)
