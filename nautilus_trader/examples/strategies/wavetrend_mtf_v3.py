...
class WaveTrendMultiTimeframeV3Config(StrategyConfig, frozen=True, kw_only=True):
    """
    Configuration for WaveTrend Multi-Timeframe strategy V3 (Regime Filtered).
    ...
    3. Same V2 improvements:
       - Requires all 3 timeframes aligned (3/3)
       - Wider stops (ATR 4.5x)
       - Higher profit threshold (4.0%)
       - Tighter trailing (1.0%)
       - Larger position (0.01 BTC)
       - 4h trend filter

    Notes
    -----
    The `use_atr_min_filter` and `range_lookback` control regime detection.
    ATR < 0.5% of price = low volatility (avoid trading)
    No new highs/lows in lookback period = stuck in range (avoid trading)
...
class WaveTrendMultiTimeframeV3(Strategy):
    """
    Multi-timeframe WaveTrend strategy V3 (Regime Filtered) with trend alignment.

    V3 Improvements over V2:
    - ADX regime filter: only trades when ADX > 25 (avoids choppy markets)
    - ATR minimum filter: pauses when volatility too low
    - All V2 improvements retained (3/3 alignment, wider stops, etc.)
    ...
    (5m, 1h, 4h) show alignment AND market regime is trending.
    Uses combination trailing stop.
    """

    def __init__(self, config: WaveTrendMultiTimeframeV3Config) -> None:
