...
def get_binance_historical_bars(
    ...
    """Fetch historical Kline data from the Binance API for a specific symbol and return it as a DataFrame.
    ...
    columns = ['time', 'open', 'high', 'low', 'close', 'volume', 'end_time', 
...
def compute_bollinger_bands(df, period=20, num_std=2):
    """Computes Bollinger Bands for a given DataFrame."""
...
def compute_atr(df, period=14):
    """Computes the ATR for a given DataFrame."""
...
def calculate_slope(df, window=8):
    """Calculates slope of the close price using linear regression."""
...
def generate_metrics(df_dict, period_bollinger=20, period_atr=14, slope_window=8):
    """
    Computes metrics for multiple symbols and returns them as a DataFrame.
    ...
    for column in ['mean_volatility', 'mean_natr', 'mean_bb_width', 'latest_trend', 'avg_volume_per_hour']:
        df_metrics[f'normalized_{column}'] = normalize(df_metrics[column])

    # Compute average score as the mean of all normalized metrics
    normalized_columns = [f'normalized_{col}' for col in ['mean_volatility', 'mean_natr', 'mean_bb_width', 'latest_trend', 'avg_volume_per_hour']]
