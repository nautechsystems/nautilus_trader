...
class WaveTrendMultiTimeframeConfig(StrategyConfig, frozen=True, kw_only=True):
    """
    Configuration for WaveTrend Multi-Timeframe strategy.

    Notes
    -----
    The `min_aligned_timeframes` parameter must be between 1 and 3 (inclusive),
    representing the minimum number of timeframes that must align for a signal.
    ...
    profit_threshold_pct: PositiveFloat = 2.0
    percentage_trail: PositiveFloat = 1.5
...
class WaveTrendMultiTimeframe(Strategy):
    ...
    def __init__(self, config: WaveTrendMultiTimeframeConfig) -> None:
    ...
    def _check_entry_signals(self) -> None:
        ...
        if bullish:
            ...
            self.log.info(
                ...
                f"4h:{self.wt_4h.is_bullish() if self.wt_4h.initialized else 'uninit'})"
            )
        ...
        # Check for bearish cross on 5m
            ...
            self.log.info(
                ...
                f"4h:{self.wt_4h.is_bearish() if self.wt_4h.initialized else 'uninit'})"
            )
