...
class WaveTrendMultiTimeframeV2Config(StrategyConfig, frozen=True, kw_only=True):
    """
    Configuration for WaveTrend Multi-Timeframe strategy V2 (Optimized).

    V2 Improvements:
    1. Requires all 3 timeframes aligned (3/3 instead of 2/3)
    2. Wider stops (ATR multiplier 4.5 instead of 3.0)
    3. Higher profit threshold (4.0% instead of 2.0%)
    4. Tighter trailing stop (1.0% instead of 1.5%)
    5. Larger default position size (0.01 BTC instead of 0.001)
    6. Trend filter - only trade in direction of 4h WaveTrend trend

    Notes
    -----
    The `min_aligned_timeframes` is set to 3 by default (all timeframes must agree).
    The `use_trend_filter` parameter enables/disables the 4h trend filter.
...
class WaveTrendMultiTimeframeV2(Strategy):
    """
    Multi-timeframe WaveTrend strategy V2 (Optimized) with trend alignment.

    V2 Improvements:
    - Requires all 3 timeframes aligned (stricter filtering)
    - Wider stops for better breathing room (ATR 4.5x)
    - Higher profit target before trailing (4.0%)
    - Tighter trailing stop to lock profits (1.0%)
    - Trend filter: only trades in direction of 4h trend

    Enters on 5m WaveTrend crosses when all 3 timeframes
    (5m, 1h, 4h) show alignment. Uses combination trailing stop.
    """

    def __init__(self, config: WaveTrendMultiTimeframeV2Config) -> None:
    ...
    def _check_entry_signals(self) -> None:
        ...
        if bullish:
            ...
            self.log.info(
                ...
                f"4h:{self.wt_4h.is_bullish() if self.wt_4h.initialized else 'uninit'}){trend_status}"
            )
            ...
            if aligned_count >= self.min_aligned and trend_ok:
                ...
                self.log.info("Aligned but trend filter blocked entry")
        ...
        # Check for bearish cross on 5m
            ...
            self.log.info(
                ...
                f"4h:{self.wt_4h.is_bearish() if self.wt_4h.initialized else 'uninit'}){trend_status}"
            )
