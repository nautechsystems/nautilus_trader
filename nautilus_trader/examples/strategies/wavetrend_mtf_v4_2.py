...
class WaveTrendMultiTimeframeV4_2Config(StrategyConfig, frozen=True, kw_only=True):
    """
    Configuration for WaveTrend Multi-Timeframe strategy V4.2 (Volatility-Enhanced Sizing).

    V4.2 Improvement over V4.1:
    ...
    # V4.2: Position sizing enhancement
    low_vol_size_multiplier: PositiveFloat = 1.25  # Size boost in LOW volatility
...
class WaveTrendMultiTimeframeV4_2(Strategy):
    """
    Multi-timeframe WaveTrend strategy V4.2 (Volatility-Enhanced Sizing).
    ...
    - ELEVATED (1.1-1.5x): Chop continuing → BLOCK
    - NORMAL (0.9-1.1x): Normal → 1.0x size
    - LOW (<0.9x): Chop ending → 1.25x size (ENHANCED)

    Expected: Same trades as V4.1, larger positions in LOW vol, +4-6% returns.
    """
    ...
    def __init__(self, config: WaveTrendMultiTimeframeV4_2Config) -> None:
        ...
        # V4.2: Position sizing multiplier (set dynamically based on volatility)
        ...
        # V4.2: Position sizing enhancement
        self.low_vol_size_multiplier = config.low_vol_size_multiplier
    ...
    def _check_entry_signals(self, bar: Bar) -> None:
        ...
        if bullish:
            ...
            if aligned_count >= self.min_aligned and trend_ok and range_ok and atr_ok and vol_ok:
                # V4.2: Set position size based on volatility regime
                vol_regime = self._detect_volatility_regime()
                if vol_regime == "LOW":
                    self.size_multiplier = self.low_vol_size_multiplier
                    self.log.info(f"LOW volatility detected - using {self.size_multiplier}x size (chop ending)")
                else:  # NORMAL
                    self.size_multiplier = 1.0
                    self.log.info("NORMAL volatility - using 1.0x size")
    ...
    def _enter_long(self) -> None:
        ...
        # V4.2: Apply size multiplier based on volatility regime
    ...
    def _set_atr_stop(self, entry_side: OrderSide) -> None:
        ...
        # V4.2: Calculate stop quantity with size multiplier
        trade_size = self.base_trade_size * Decimal(str(self.size_multiplier))
