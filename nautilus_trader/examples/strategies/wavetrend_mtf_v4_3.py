...
class WaveTrendMultiTimeframeV4_3Config(StrategyConfig, frozen=True, kw_only=True):
    """
    Configuration for WaveTrend Multi-Timeframe strategy V4.3 (Dynamic Exits).

    V4.3 Improvement over V3 (Most Complex):
    ...
    - Net effect: Better risk-adjusted returns than blocking

    V3 Features (All Retained):
    1. ATR minimum filter: Blocks low volatility
    2. Range filter: Blocks stuck markets
    3. Multi-timeframe alignment (3/3)
    4. Trend filter: Blocks counter-trend
    ...
    # V4.3: Volatility regime detection (for dynamic exits)
    ...
    # V4.3: Dynamic exit parameters per volatility regime
    high_vol_atr_mult: PositiveFloat = 3.0  # Tight stops in HIGH vol
    high_vol_profit_pct: PositiveFloat = 2.5  # Lower targets in HIGH vol
    elevated_vol_atr_mult: PositiveFloat = 4.0  # Slightly tight in ELEVATED
    elevated_vol_profit_pct: PositiveFloat = 3.5  # Slightly lower targets
    normal_vol_atr_mult: PositiveFloat = 4.5  # Standard (same as V3)
    normal_vol_profit_pct: PositiveFloat = 4.0  # Standard (same as V3)
    low_vol_atr_mult: PositiveFloat = 6.0  # Wide stops in LOW vol (trends)
    low_vol_profit_pct: PositiveFloat = 6.0  # Higher targets in LOW vol

    # Order management
    order_id_tag: str = "WT_MTF_V4_3"
...
class WaveTrendMultiTimeframeV4_3(Strategy):
    """
    Multi-timeframe WaveTrend strategy V4.3 (Dynamic Exits).

    V4.3 Improvement over V3 (Most Complex):
    - Keep V3's entry logic EXACTLY (no volatility blocking)
    - Add dynamic exit adjustment based on volatility regime
    - HIGH volatility: Tighter stops (3.0x), lower targets (2.5%)
    - ELEVATED: Slightly tighter (4.0x), lower targets (3.5%)
    - NORMAL: Standard (4.5x ATR, 4.0% target)
    - LOW: Wider stops (6.0x), higher targets (6.0%)

    Hypothesis:
    - Trading through chop with tight stops = fewer losses
    - Trading trends with wide stops/targets = bigger wins
    - Net effect: Better than blocking trades entirely

    V3 Entry Logic (Retained):
    - ATR minimum, range filter, trend filter (all blocking)
    - Multi-timeframe alignment (3/3)
    - NO volatility blocking on entries

    Expected: Same 141 trades as V3, better risk management.
    """

    def __init__(self, config: WaveTrendMultiTimeframeV4_3Config) -> None:
        ...
        # V4.3: ATR history for volatility regime detection
        ...
        # V4.3: Current profit target (set dynamically per trade based on regime)
        ...
        # V4.3: Volatility regime detection configuration
        ...
        # V4.3: Dynamic exit parameters per regime
        self.high_vol_atr_mult = config.high_vol_atr_mult
        self.high_vol_profit_pct = config.high_vol_profit_pct
        self.elevated_vol_atr_mult = config.elevated_vol_atr_mult
        self.elevated_vol_profit_pct = config.elevated_vol_profit_pct
    ...
    def _on_bar_5m(self, bar: Bar) -> None:
        ...
        # V4.3: Track ATR history for volatility regime detection
    ...
    def _detect_volatility_regime(self) -> str:
        """
        V4.3: Detect current volatility regime for dynamic exit adjustment.
    ...
    def _set_atr_stop(self, entry_side: OrderSide) -> None:
        ...
        # V4.3: Detect volatility regime and adjust stop multiplier + profit target
        vol_regime = self._detect_volatility_regime()

        if vol_regime == "HIGH":
            atr_mult = self.high_vol_atr_mult  # 3.0x - tight stop
            self.current_profit_target = self.high_vol_profit_pct / 100.0  # 2.5%
        elif vol_regime == "ELEVATED":
            atr_mult = self.elevated_vol_atr_mult  # 4.0x
            self.current_profit_target = self.elevated_vol_profit_pct / 100.0  # 3.5%
        elif vol_regime == "LOW":
            atr_mult = self.low_vol_atr_mult  # 6.0x - wide stop (give trend room)
            self.current_profit_target = self.low_vol_profit_pct / 100.0  # 6.0%
        else:  # NORMAL
            atr_mult = self.normal_vol_atr_mult  # 4.5x - standard (same as V3)
            self.current_profit_target = self.normal_vol_profit_pct / 100.0  # 4.0%
    ...
    def _update_trailing_stop(self, bar: Bar) -> None:
        ...
        # V4.3: Check if we should switch to percentage trail (using dynamic target)
    ...
    def on_position_closed(self, position) -> None:
        ...
        self.use_percentage_trail = False
        self.current_profit_target = None  # V4.3: Reset dynamic profit target
