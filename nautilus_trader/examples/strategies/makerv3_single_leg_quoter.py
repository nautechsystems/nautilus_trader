    ...
    class MakerV3SingleLegQuoter(Strategy):
        ...
        def _spot_balance_total(self, currency_code: str) -> Decimal | None:
            ...
            if not accounts and hasattr(self, "portfolio"):
                try:
                    account = self.portfolio.account(venue=Venue(BYBIT))
                except Exception:
                    account = None
        ...
        def _compute_inventory_skew(self) -> dict[str, Any]:
            ...
            spot_qty = self._spot_balance_total("PLUME")
            if position_qty is not None:
                inventory_qty = position_qty
                inventory_source = "bybit_perp_position"
            elif spot_qty is not None:
                inventory_qty = spot_qty
                inventory_source = "bybit_spot_balance"
            else:
                inventory_qty = None
        ...
        def _refresh_quotes(self, now_ns: int) -> None:
            ...
            bybit_bbo = self._best_bid_ask(self.config.bybit_instrument_id)
            if bybit_bbo is None:
                return
            best_bid_px, best_ask_px = bybit_bbo
            bybit_mid = (best_bid_px + best_ask_px) / Decimal("2")

            bybit_age_ms = None
            if self._last_bbo_ts_ns.get(self.config.bybit_instrument_id, 0) > 0:
                bybit_age_ms = int((now_ns - self._last_bbo_ts_ns[self.config.bybit_instrument_id]) / 1_000_000)
            binance_age_ms = None
            if self._last_bbo_ts_ns.get(self.config.binance_instrument_id, 0) > 0:
                binance_age_ms = int((now_ns - self._last_bbo_ts_ns[self.config.binance_instrument_id]) / 1_000_000)
            max_age_ms = self._runtime_int("max_age_ms")
            bybit_fresh = bool(bybit_age_ms is not None and bybit_age_ms < max_age_ms)
            binance_fresh = bool(binance_age_ms is not None and binance_age_ms < max(max_age_ms, 30_000))

            ref_bbo = self._best_bid_ask(self.config.binance_instrument_id)
            if ref_bbo is None or not binance_fresh:
                self._cancel_managed_quotes("reference_md_unavailable")
                self._publish_state("blocked_ref_md")
                return
            ...
            binance_mid = (ref_bid + ref_ask) / Decimal("2") if ref_bid is not None and ref_ask is not None else None
            if binance_mid is not None:
                fair_value = (bybit_mid + binance_mid) / Decimal("2")
            else:
            ...
            tick = self._bybit_instrument.price_increment.as_decimal()
            ...
            for bid_place, bid_cancel in bid_levels:
                ...
                if bid_place_rounded > 0 and bid_cancel_rounded > 0:
                    desired_buys.append(
                        (
                            self._bybit_instrument.make_price(bid_place_rounded),
                            bid_cancel_rounded,
                            match_tol,
                        ),
                    )
            for ask_place, ask_cancel in ask_levels:
                ...
                if ask_place_rounded > 0 and ask_cancel_rounded > 0:
                    desired_sells.append(
                        (
                            self._bybit_instrument.make_price(ask_place_rounded),
                            ask_cancel_rounded,
                            match_tol,
                        ),
                    )
            ...
            self._last_pricing_debug = {
                "pricing": {
                    ...
                    "ref_mid": _decimal_to_json_str(binance_mid),
                    "maker_top_bid": _decimal_to_json_str(best_bid_px),
                    "maker_top_ask": _decimal_to_json_str(best_ask_px),
                    "bybit_mid": _decimal_to_json_str(bybit_mid),
                    "binance_mid": _decimal_to_json_str(binance_mid),
                ...
                "skew": {
                    ...
                    "spot_plume_total": _decimal_to_json_str(skew_ctx["spot_qty"]),
        ...
        def _cancel_managed_quotes(self, reason: str, force: bool = False) -> None:
            if force or self._managed_orders():
        ...
        def _publish_market_bbo(
            ...
            *,
            ...
            base, quote = _normalize_contract_symbol(symbol)
            if not quote:
                quote = "USDT"
            ...
            symbol = f"{base}/{quote}"
            fv_coin = f"{base.lower()}/{quote.lower()}"
