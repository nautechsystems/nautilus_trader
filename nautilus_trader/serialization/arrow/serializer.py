...
from nautilus_trader.core.nautilus_pyo3 import ArrowDataTransformer
    ...
    def rust_defined_to_record_batch(  # noqa: C901 (too complex)
        ...
        match data_cls:
            case nautilus_pyo3.OrderBookDelta:
                batch_bytes = ArrowDataTransformer.pyo3_order_book_deltas_to_record_batch_bytes(
                    data,
                )
            case nautilus_pyo3.OrderBookDepth10:
                batch_bytes = ArrowDataTransformer.pyo3_order_book_depth10_to_record_batch_bytes(
                    data,
                )
            case nautilus_pyo3.QuoteTick:
                batch_bytes = ArrowDataTransformer.pyo3_quote_ticks_to_record_batch_bytes(data)
            case nautilus_pyo3.TradeTick:
                batch_bytes = ArrowDataTransformer.pyo3_trade_ticks_to_record_batch_bytes(data)
            case nautilus_pyo3.Bar:
                batch_bytes = ArrowDataTransformer.pyo3_bars_to_record_batch_bytes(data)
            case _:
                if data_cls == OrderBookDelta or data_cls == OrderBookDeltas:
                    ...
                    batch_bytes = ArrowDataTransformer.pyo3_quote_ticks_to_record_batch_bytes(
                        pyo3_quotes,
                    )
                elif data_cls == TradeTick:
                    pyo3_trades = TradeTick.to_pyo3_list(data)
                    batch_bytes = ArrowDataTransformer.pyo3_trade_ticks_to_record_batch_bytes(
                        pyo3_trades,
                    )
                elif data_cls == Bar:
                    pyo3_bars = Bar.to_pyo3_list(data)
                    batch_bytes = ArrowDataTransformer.pyo3_bars_to_record_batch_bytes(pyo3_bars)
