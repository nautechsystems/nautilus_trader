...
# -- REQUEST HANDLERS -----------------------------------------------------------------------------
    ...
    cpdef void _handle_date_range_request(
        ...
        if used_start_request > used_end_request or used_start_request > now:
            self._log.error(f"Cannot handle request: incompatible request dates for {request}")
            return

        if used_end_request < used_end_catalog:
            self._date_range_client_request(client, request)
            return
        ...
        # we count the total number of requests to define the request group that will wait for all requests to be completed
        ...
        new_request = request.with_dates(max_date(used_start_catalog, used_start_request), min_date(used_end_request, used_end_catalog))
        new_request.params["update_catalog_mode"] = self._convert_update_catalog_mode(
            new_request.params.get("update_catalog_mode", None),
            None
        )
