...
# -- RESPONSE HANDLERS ----------------------------------------------------------------------------
    ...
    cpdef dict _handle_quote_ticks(self, list ticks):
        ...
        if instrument_id not in self._quote_ticks_bar_types[instrument_id]:
            return result
    ...
    cpdef dict _handle_trade_ticks(self, list ticks):
        ...
        if instrument_id not in self._trade_ticks_bar_types[instrument_id]:
            return result
    ...
    cdef dict _aggregate_ticks(self, list ticks, list bar_types, bint is_quote_tick):
        ...
        for bar_type in bar_types:
            aggregated_bars = []
            aggregator = bar_types.get(bar_type)
            ...
            aggregator.init_batch_update(handler, ticks[0].ts_event)
            ...
            aggregator.finish_batch_update()
    ...
    cdef dict _handle_composite_bars(self, list bars):
        ...
        for composite_bar_type in self._composite_bar_types[bar_type]:
            ...
            handler = lambda bar: aggregated_bars.append(bar)
            aggregator.init_batch_update(handler, bars[0].ts_init)
