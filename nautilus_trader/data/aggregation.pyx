...
cdef class SpreadQuoteAggregator(Component):
    ...
    def __init__(
        ...
        self._handler = handler
        self._cache = cache
        self._log = Logger(name=f"{type(self).__name__}({spread_instrument_id})")
    ...
    cdef void _build_quote(self, TimeEvent event):
        ...
        for i, component_id in enumerate(self._component_ids):
            component_mid_prices = self._cache.price(component_id, PriceType.MID)
            component_quote = self._cache.quote_tick(component_id)

            if component_quote is None or component_mid_prices is None:
                self._log.debug(
            ...
            self._mid_prices[i] = component_mid_prices.as_double()
            self._vegas[i] = greeks_data.vega
            self._bid_ask_spreads[i] = component_quote.ask_price.as_double() - component_quote.bid_price.as_double()
        ...
        spread_mid_prices = (self._mid_prices * self._ratios).sum()
        raw_bid_price = spread_mid_prices - bid_ask_spread * 0.5
        raw_ask_price = spread_mid_prices + bid_ask_spread * 0.5
