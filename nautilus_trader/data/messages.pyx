...
cdef class SubscribeOrderBook(SubscribeData):
    ...
    def to_request(
        ...
        if self.data_type.type != OrderBookDepth10:
            raise ValueError(
                f"Cannot convert SubscribeOrderBook with data_type {self.data_type.type} to RequestOrderBookDepths. "
...
cdef class RequestOrderBookDepths(RequestData):
    """
    Represents a request for order book depths.
