...
# -- QUERIES ---------------------------------------------------------------------------------
    ...
    cpdef dict market_values(self, Venue venue):
        ...
        for position in positions_open:
            ...
            if xrate == 0:
                self._log.error(
                    f"Cannot calculate market value: "
                    f"insufficient data for {instrument.quote_currency}/{account.base_currency}."
                )
                return None  # Cannot calculate

            market_value = market_values.get(instrument.quote_currency, Decimal(0))
            market_value += Account.market_value_c(
                instrument,
                position.quantity,
                last,
            ) * xrate
            ...
            if account.base_currency is not None:
                market_values[account.base_currency] = market_value
            else:
    ...
    cpdef Money market_value(self, InstrumentId instrument_id):
        ...
        if account.base_currency is not None:
            return Money(market_value, account.base_currency)
        else:
...
# -- INTERNAL --------------------------------------------------------------------------------------
    ...
    cdef bint _update_initial_margin(self, Venue venue, list orders_working) except *:
        ...
        for order in passive_orders_working:
            ...
            # Calculate margin
            margin = Account.calculate_initial_margin_c(
                instrument,
                order.quantity,
                order.price,
            )
    ...
    cdef bint _update_maint_margin(self, Venue venue, list positions_open) except *:
        ...
        for position in positions_open:
            ...
            # Calculate margin
            margin = Account.calculate_maint_margin_c(
                instrument,
                position.side,
                position.quantity,
                last,
