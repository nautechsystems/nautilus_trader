...
cdef class AccountsManager:
    ...
    cdef AccountState update_orders(self, Account account, Instrument instrument, list orders_working, int64_t ts_event)
    cdef AccountState update_positions(self, MarginAccount account, Instrument instrument, list positions_open, int64_t ts_event)
    cdef AccountState _update_balance_locked(self, CashAccount account, Instrument instrument, list orders_working, int64_t ts_event)
    cdef AccountState _update_margin_init(self, MarginAccount account, Instrument instrument, list orders_working, int64_t ts_event)
