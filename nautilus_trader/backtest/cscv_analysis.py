...
class CSCV:
    ...
    def __init__(
        ...
        statics: PortfolioStatistic = SharpeRatio(),
    ):
        self.n_sub_matrices: int = n_sub_matrices
        self.statics: PortfolioStatistic = statics 
        self.returns: pd.DataFrame = returns 
        self.comb: List = [set(x) for x in combinations(range(self.n_sub_matrices), int(self.n_sub_matrices / 2))]
        ...
        return_bins = [self.returns[i*bin_size:(i+1) * bin_size] for i in range(self.n_sub_matrices)]
        for set_id, IS_set in enumerate(self.comb):
            ...
            IS_metrics = IS_returns.apply(self.statics.calculate_from_returns, axis = 0)
            OOS_metrics = OOS_returns.apply(self.statics.calculate_from_returns, axis = 0)
            self.Rs[set_id] = pd.concat([self.Rs[set_id],IS_metrics])
            self.R_bars[set_id] = pd.concat([self.R_bars[set_id],OOS_metrics]) 
            
    def estimate(self):
        """
        Evaluating backtest overfitting,performance_degradation and  the second order stochastic dominance.
        """
        ...
        self.logits = (1-((r_bar_rank_series)/(len(R_df.columns)+1))).map(lambda p: np.log(p/(1-p)))
        ...
        self.dom_df = pd.DataFrame(dict(optimized_IS=optimized, non_optimized_OOS=non_optimized), index=y)
    ...
    def plot_pbo(self):
        ...
        plt.hist(x=[l for l in self.logits if l > -10000], bins='auto',label= f"Prob Overfit = {self.pbo}")
    ...
    def plot_performance_degradation(self):
        ...
        plt.figure(figsize=(15, 4))
        plt.scatter(X, Y)
        plt.plot(x1, y1, color='red', ls='--', label=f'Regression slope: {str(np.round(results.params[1], 2))} ; '
