...
from cumulo.protocol.util import assert_one

logger = logging.getLogger(__name__)


class Ladder(CumuloBase):
    ...
    def sort_levels(cls, levels):
        return sorted(levels or [], key=attrgetter("price"))

    @validator("price_levels", always=True)
    def set_price_levels(cls, v, values):
        return {level.price: level for level in values.get("levels", [])}

    @validator("order_id_prices", always=True)
    def set_order_id_prices(cls, v, values):
        return {order.order_id: order.price for level in values.get("levels", []) for order in level.orders}

    @validator("side", always=True)
    def validate_side(cls, side, values):
        if values.get("levels"):
            side = one(set(lvl.side for lvl in values["levels"]))
        else:
            assert side, "If not passing `levels` to Ladder, must pass `side`"
        assert isinstance(side, OrderSide)
        return side

    @validator("reverse", always=True)
    def validate_reverse(cls, reverse, values):
        if reverse is None:
            return values["side"] == BID
        return reverse
