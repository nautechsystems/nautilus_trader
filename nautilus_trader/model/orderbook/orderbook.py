...
from cumulo.protocol.core import CumuloBase
from cumulo.protocol.enums import BID, ASK, FAK, OrderSide
...
from cumulo.protocol.util import assert_one, merge_dicts


class Orderbook(CumuloBase):
    default_on = "volume"
    ...
    @validator("bids", "asks", always=True)
    def ensure_ladder(cls, v: Optional[Ladder], field, values):
        if not isinstance(v, Ladder):
            return Ladder(
                side=OrderSide.from_name(name=field.name), exchange_order_ids=values.get("exchange_order_ids", False)
            )
        return v

    @validator("order_id_side", always=True)
    def set_order_id_side(cls, v: Dict, values):
        v.update({order.order_id: order.side for order in values["bids"].iter_orders()})
        v.update({order.order_id: order.side for order in values["asks"].iter_orders()})
        return v
    ...
    def insert(self, *_, order: Order, remove_trades=False):
        """
