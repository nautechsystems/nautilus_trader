...
// under standard precision fails with a price `PrecisionMismatch`. Unlike the local legacy
// fixtures there is no 64-bit variant to select (see `get_nautilus_test_data_file_path`).
