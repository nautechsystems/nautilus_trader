...
pub struct CustomDataVTable {
    ...
    pub schema_ipc: unsafe extern "C" fn() -> PluginResult<OwnedBytes>,
    ...
    pub encode_batch: unsafe extern "C" fn(
    ...
    pub decode_batch: unsafe extern "C" fn(
        ipc_bytes: Slice<'_, u8>,
        metadata: Slice<'_, MetadataEntry<'_>>,
    ) -> PluginResult<OwnedBytes>,

    /// Returns the event timestamp of the value behind the handle.
    pub ts_event: unsafe extern "C" fn(handle: *const CustomDataHandle) -> u64,

    /// Returns the init timestamp of the value behind the handle.
    pub ts_init: unsafe extern "C" fn(handle: *const CustomDataHandle) -> u64,

    /// Serializes a single value to a JSON payload (no envelope).
    pub to_json: unsafe extern "C" fn(handle: *const CustomDataHandle) -> PluginResult<OwnedBytes>,

    /// Clones the value behind the handle into a new owned handle.
    pub clone_handle:
        unsafe extern "C" fn(handle: *const CustomDataHandle) -> *mut CustomDataHandle,

    /// Drops the value behind the handle, freeing all of its resources.
    pub drop_handle: unsafe extern "C" fn(handle: *mut CustomDataHandle),
    ...
    T: PluginCustomData + PartialEq + Clone,
{
    const VTABLE: CustomDataVTable = CustomDataVTable {
        ...
        schema_ipc: schema_ipc_thunk::<T>,
        from_json: from_json_thunk::<T>,
        encode_batch: encode_batch_thunk::<T>,
        decode_batch: decode_batch_thunk::<T>,
        ts_event: ts_event_thunk::<T>,
        ts_init: ts_init_thunk::<T>,
        to_json: to_json_thunk::<T>,
        clone_handle: clone_handle_thunk::<T>,
        drop_handle: drop_handle_thunk::<T>,
        eq_handles: eq_handles_thunk::<T>,
    };
}
