...
use nautilus_okx::websocket::{messages::OKXWsMessage, parse::parse_book_msg_vec};
...
fn bench_book_snapshot(c: &mut Criterion) {
    c.bench_function("parse_book_snapshot", |b| {
        b.iter_batched(
            || from_str::<OKXWsMessage>(BOOK_SNAPSHOT).expect("snapshot event"),
            |event| match event {
                OKXWsMessage::BookData { data, action, .. } => {
...
fn bench_book_update(c: &mut Criterion) {
    c.bench_function("parse_book_update", |b| {
        b.iter_batched(
            || from_str::<OKXWsMessage>(BOOK_UPDATE).expect("update event"),
            |event| match event {
                OKXWsMessage::BookData { data, action, .. } => {
                    let instrument_id = InstrumentId::from("BTC-USDT.OKX");
                    let payload = parse_book_msg_vec(
