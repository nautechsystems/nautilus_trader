...
//! Live market data client implementation for the Architect adapter.
...
use crate::{
    common::{
        consts::ARCHITECT_VENUE,
        enums::{ArchitectCandleWidth, ArchitectMarketDataLevel},
        parse::map_bar_spec_to_candle_width,
    },
    config::ArchitectDataClientConfig,
    http::client::ArchitectHttpClient,
    websocket::{data::client::ArchitectMdWebSocketClient, messages::NautilusWsMessage},
};

/// Architect data client for live market data streaming and historical data requests.
///
...
pub struct ArchitectDataClient {
    ...
    active_book_subs: Arc<DashMap<InstrumentId, ArchitectMarketDataLevel>>,
    /// Active bar subscriptions (maps instrument to candle width).
    active_bar_subs: Arc<DashMap<InstrumentId, (BarType, ArchitectCandleWidth)>>,
}

impl ArchitectDataClient {
    /// Creates a new [`ArchitectDataClient`] instance.
    ///
    /// # Errors
    ///
...
#[async_trait(?Send)]
impl DataClient for ArchitectDataClient {
    fn client_id(&self) -> ClientId {
        self.client_id
    }

    fn venue(&self) -> Option<Venue> {
        Some(*ARCHITECT_VENUE)
    }
    ...
    async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        if self.config.has_api_credentials() {
            ...
            let api_secret = self.config.api_secret.as_ref().unwrap();
            let token = self
                .http_client
                .authenticate(api_key, api_secret, 86400) // 24 hour token
                .await
                .context("Failed to authenticate with Architect")?;
            log::debug!("Authenticated with Architect");
    ...
    fn subscribe_quotes(&mut self, cmd: &SubscribeQuotes) -> anyhow::Result<()> {
        ...
        get_runtime().block_on(async {
            self.ws_client
                .subscribe(symbol, ArchitectMarketDataLevel::Level1)
                .await
        })?;
    ...
    fn subscribe_book_deltas(&mut self, cmd: &SubscribeBookDeltas) -> anyhow::Result<()> {
        ...
        let level = ArchitectMarketDataLevel::Level2;
    ...
    fn request_instruments(&self, request: &RequestInstruments) -> anyhow::Result<()> {
        ...
        get_runtime().spawn(async move {
            match http.request_instruments(None, None).await {
                Ok(instruments) => {
                    log::info!("Fetched {} instruments from Architect", instruments.len());
    ...
    fn request_instrument(&self, request: &RequestInstrument) -> anyhow::Result<()> {
        ...
        get_runtime().spawn(async move {
            match http.request_instrument(&symbol, None, None).await {
                Ok(instrument) => {
                    log::debug!("Fetched instrument {symbol} from Architect");
