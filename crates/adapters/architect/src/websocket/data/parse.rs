...
//! Parsing functions to convert Architect WebSocket messages to Nautilus domain types.
...
/// Parses an Architect L1 book message into a [`QuoteTick`].
///
...
pub fn parse_book_l1_quote(
    ...
    .context("Failed to construct QuoteTick from Architect L1 book")
}

/// Parses a book level into price and quantity.
fn parse_book_level(
    level: &ArchitectBookLevel,
    price_precision: u8,
    size_precision: u8,
...
/// Parses an Architect L2 book message into [`OrderBookDeltas`].
///
...
pub fn parse_book_l2_deltas(
    ...
    for level in &book.b {
        ...
        .context("Failed to construct OrderBookDelta from Architect L2 bid level")?;
    ...
    for level in &book.a {
        ...
        .context("Failed to construct OrderBookDelta from Architect L2 ask level")?;
    ...
    OrderBookDeltas::new_checked(instrument_id, deltas)
        .context("Failed to assemble OrderBookDeltas from Architect L2 message")
}

/// Parses a L3 book level into price and quantity.
fn parse_book_level_l3(
    level: &ArchitectBookLevelL3,
    price_precision: u8,
    size_precision: u8,
) -> anyhow::Result<(Price, Quantity)> {
    let price = decimal_to_price_dp(level.p, price_precision, "book.level.price")?;
...
/// Parses an Architect L3 book message into [`OrderBookDeltas`].
///
...
pub fn parse_book_l3_deltas(
    ...
    for level in &book.b {
        ...
        for &order_qty in &level.o {
            ...
            .context("Failed to construct OrderBookDelta from Architect L3 bid order")?;
    ...
    for level in &book.a {
        ...
        for &order_qty in &level.o {
            ...
            .context("Failed to construct OrderBookDelta from Architect L3 ask order")?;
    ...
    OrderBookDeltas::new_checked(instrument_id, deltas)
        .context("Failed to assemble OrderBookDeltas from Architect L3 message")
}

/// Parses an Architect trade message into a [`TradeTick`].
///
/// # Errors
///
/// Returns an error if price or quantity parsing fails.
...
mod tests {
    ...
    fn create_instrument_with_precision(
        ...
        let instrument = CryptoPerpetual::new(
            InstrumentId::new(Symbol::new(symbol), *ARCHITECT_VENUE),
            Symbol::new(symbol),
            Currency::USD(),
            Currency::USD(),
            Currency::USD(),
    ...
    fn test_parse_book_l1_from_captured_data() {
        ...
        let book: ArchitectMdBookL1 = serde_json::from_str(json).unwrap();
    ...
    fn test_parse_book_l2_from_captured_data() {
        ...
        let book: ArchitectMdBookL2 = serde_json::from_str(json).unwrap();
    ...
    fn test_parse_book_l3_from_captured_data() {
        ...
        let book: ArchitectMdBookL3 = serde_json::from_str(json).unwrap();
