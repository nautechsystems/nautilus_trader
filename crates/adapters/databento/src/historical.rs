...
use crate::{
    ...
    symbology::{
        ...
        get_nautilus_instrument_id_for_record, infer_symbology_type,
...
impl DatabentoHistoricalClient {
    ...
    ///
    /// Returns an error if the API request or data processing fails.
    pub async fn get_range_order_book_depths(
        &self,
        params: RangeQueryParams,
        depth: Option<usize>,
        ...
        let mut process_record = |record: dbn::RecordRef| -> anyhow::Result<()> {
            // Get the venue for this record
            let venue = if let Some(msg) = record.get::<dbn::Mbp10Msg>() {
                let publisher_id = msg.hd.publisher_id;
                self.publisher_venue_map
                    .get(&publisher_id)
                    .copied()
                    .unwrap_or_else(|| Venue::from("UNKNOWN"))
            } else {
                Venue::from("UNKNOWN")
            };

            let instrument_id =
                get_nautilus_instrument_id_for_record(&record, &mut metadata_cache, venue)?;
