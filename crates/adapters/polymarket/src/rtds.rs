...
impl PolymarketRtdsFeed {
    ...
    fn snapshot_wire_subscriptions(&self) -> AHashMap<String, RtdsWireSubscription> {
        ...
        let mut has_crypto_subscription = false;

        for entry in &self.inner.subscriptions {
            if entry.wire.topic == RtdsTopic::CryptoPrices.as_str() {
                has_crypto_subscription = true;
            } else {
                snapshot.insert(entry.key().clone(), entry.wire.clone());
            }
        }

        if has_crypto_subscription {
            snapshot.insert(
                RtdsTopic::CryptoPrices.as_str().to_string(),
...
mod tests {
    ...
    async fn test_reconnected_control_replays_retained_subscriptions() {
        ...
        assert_eq!(
            subscriptions[0]["topic"].as_str(),
            Some(RtdsTopic::CryptoPrices.as_str()),
        );
        assert!(
            subscriptions[0]["filters"].is_null(),
            "crypto replay should use topic-level subscription without filters",
        );
    }
