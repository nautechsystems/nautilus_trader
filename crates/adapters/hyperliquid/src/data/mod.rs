...
impl DataClient for HyperliquidDataClient {
    ...
    fn subscribe(&mut self, cmd: SubscribeCustomData) -> anyhow::Result<()> {
        ...
        if data_type == "HyperliquidAllMids" {
            ...
            log::debug!("Subscribing to all mids");

            get_runtime().spawn(async move {
                if let Err(e) = ws.subscribe_all_mids().await {
                    log::error!("Failed to subscribe to all mids: {e:?}");
                }
            });
    ...
    fn unsubscribe(&mut self, cmd: &UnsubscribeCustomData) -> anyhow::Result<()> {
        ...
        if data_type == "HyperliquidAllMids" {
            ...
            log::debug!("Unsubscribing from all mids");

            get_runtime().spawn(async move {
                if let Err(e) = ws.unsubscribe_all_mids().await {
                    log::error!("Failed to unsubscribe from all mids: {e:?}");
                }
            });
