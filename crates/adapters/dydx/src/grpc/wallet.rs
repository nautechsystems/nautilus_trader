...
impl Account {
    ...
    pub fn sign(&self, doc: tx::SignDoc) -> Result<tx::Raw, anyhow::Error> {
        doc.sign(&self.key)
            .map_err(|e| err!("Failed to sign transaction: {e}"))
    }
...
impl Wallet {
    ...
    ///
    /// Returns an error if the account derivation fails.
    pub fn account_offline(&self, index: u32) -> anyhow::Result<Account> {
        self.derive_account(index, BECH32_PREFIX_DYDX)
    }

    fn derive_account(&self, index: u32, prefix: &str) -> anyhow::Result<Account> {
        // BIP-44 derivation path for Cosmos (coin type 118)
        // See https://github.com/satoshilabs/slips/blob/master/slip-0044.md
        let derivation_str = format!("m/44'/118'/0'/0/{index}");
        let derivation_path = DerivationPath::from_str(&derivation_str)?;
...
impl Account {
    ...
    ///
    /// Returns an error if signing fails.
    pub fn sign(&self, doc: tx::SignDoc) -> anyhow::Result<tx::Raw> {
        doc.sign(&self.key)
    ...
    ///
    /// Returns an error if the subaccount number is invalid.
    pub fn subaccount(&self, number: u32) -> anyhow::Result<Subaccount> {
        Ok(Subaccount {
            address: self.address.clone(),
            number,
        })
