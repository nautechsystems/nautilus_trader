...
pub(super) use ibapi::{
    ...
    client::Client,
    contracts::{Contract, Exchange, SecurityType},
    orders::{
...
impl ExecutionClient for InteractiveBrokersExecutionClient {
    ...
    async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        self.publish_combo_order_instruments(client.as_ref()).await;
    ...
    async fn generate_mass_status(
        ...
        let (mut order_reports, fill_reports, position_reports) = tokio::try_join!(
        ...
        parse::fill_missing_avg_px(&mut order_reports, &fill_reports);
...
impl InteractiveBrokersExecutionClient {
    ...
    async fn resolve_combo_fill_reports(
        ...
        for exec_data in combo_executions {
            ...
            let instrument_id = match combo_contracts
                ...
                .and_then(|contract| self.resolve_report_contract_instrument_id(contract))
            {
            ...
            if let Some(report) = self.build_historical_fill_report(
                cmd,
                &exec_data,
                instrument_id,
                0.0,
    ...
    async fn handle_cancel_order_async(
        ...
        if orders.lock()?.groups.contains_key(&cmd.client_order_id) {
            return Self::cancel_incarnation_group(
                ...
                request_timeout_secs,
                None,
                target_order.is_pending_cancel(),
            )
            .await;
        }
        ...
        // IB's terminal status can evict an order while its execution details still wait for
        // the commission report, so a cancel sent in that gap has no pending state to record
        if orders.lock()?.terminal_orders.contains_key(&ib_order_id) {
            tracing::debug!(
                "Cancel request for order {} was sent after IB reported it terminal",
                cmd.client_order_id
            );
            return Ok(());
        }

        // A strategy cancel already published PENDING_CANCEL, so only the tracking records it
        let result = if target_order.is_pending_cancel() {
            Self::track_pending_cancel(cmd.client_order_id, orders).map(|_| ())
        } else {
    ...
    pub(super) fn emit_order_pending_cancel(
        ...
        let Some((trader_id, strategy_id, instrument_id)) =
            Self::track_pending_cancel(client_order_id, orders)?
        else {
            return Ok(());
        };
    ...
    // Marks a tracked order as pending cancel, returning its identity when it was not already.
    pub(super) fn track_pending_cancel(
        client_order_id: ClientOrderId,
        orders: &OrderTracker,
    ) -> anyhow::Result<Option<(TraderId, StrategyId, InstrumentId)>> {
