...
impl InteractiveBrokersExecutionClient {
    ...
    pub(super) async fn handle_submit_order_async(
        ...
        if let Some(reason) = Self::unsupported_trailing_offset_reason(
    ...
    // IB trails by a price amount (`auxPrice`) or a percentage (`trailingPercent`)
    pub(super) fn unsupported_trailing_offset_reason(
        ...
        let detail = format!(
            "`TrailingOffsetType` {trailing_offset_type:?} is not supported (only PRICE and BASIS_POINTS are supported)"
        );
        Some(coded_denial_reason(
            DENIAL_TRAILING_OFFSET_TYPE_UNSUPPORTED,
            &detail,
        ))
    }
    ...
    pub(super) fn is_own_open_order(
        order: &ibapi::orders::Order,
        client_id: i32,
        ib_account: Ustr,
    ) -> bool {
        order.client_id == client_id && order.account == ib_account
    }
    ...
    pub(super) fn apply_modify_fields_to_ib_order(
        ...
        if let Some(trigger_price) = cmd.trigger_price {
            ...
            if matches!(ib_order.order_type.as_str(), "TRAIL" | "TRAIL LIMIT") {
                // IB rejects a trailing stop price change that keeps the trailing amount (10067)
                let trailing_offset = cmd
                    .params
                    .as_ref()
                    .and_then(|params| {
                        params
                            .get_f64(MODIFY_TRAILING_OFFSET_PARAM)
                            .or_else(|| params.get_str(MODIFY_TRAILING_OFFSET_PARAM)?.parse().ok())
                    })
                    .with_context(|| {
                        format!(
                            "IB requires a new trailing offset with a trailing stop trigger; \
                             set params[\"{MODIFY_TRAILING_OFFSET_PARAM}\"] in the order's offset units"
                        )
                    })?;

                // The order keeps its offset type: basis points convert to IB's percent
                if ib_order.trailing_percent.is_some() {
                    ib_order.trailing_percent = Some(trailing_offset / 100.0);
                } else {
        ...
        while let Some(order_result) = subscription.next().await {
            match order_result {
                Ok(Orders::OrderData(data)) => {
                    // Raw order IDs and references repeat across API clients and accounts, and
                    // IB lets only the placing client modify an order
                    if !Self::is_active_open_order(&data.order)
                        || !Self::is_own_open_order(&data.order, client.client_id(), ib_account)
                    {
                        continue;
                    }
...
mod tests {
    ...
    #[rstest::rstest]
    #[case::price_offset(None, Some(json!(2.5)), Some(2.5), None)]
    #[case::basis_point_offset(Some(0.25), Some(json!("30")), Some(0.5), Some(0.3))]
    fn modify_trailing_stop_sends_trigger_with_new_offset(
        #[case] trailing_percent: Option<f64>,
        #[case] trailing_offset: Option<serde_json::Value>,
        #[case] expected_aux: Option<f64>,
        #[case] expected_percent: Option<f64>,
    ) {
        ...
        cmd.params = trailing_offset.map(|offset| {
            ...
            params.insert(MODIFY_TRAILING_OFFSET_PARAM.to_string(), offset);
            params
        });
        ...
        assert_eq!(ib_order.aux_price, expected_aux);
        assert_eq!(ib_order.trailing_percent, expected_percent);
        assert_eq!(ib_order.trail_stop_price, Some(149.5));
    }

    #[rstest::rstest]
    fn modify_trailing_stop_without_offset_fails_before_sending() {
        let original = ibapi::orders::Order {
            order_type: "TRAIL".to_string(),
            aux_price: Some(0.5),
        ...
        assert_eq!(
            error.to_string(),
            "IB requires a new trailing offset with a trailing stop trigger; set params[\"trailing_offset\"] in the order's offset units"
        );
        assert_eq!(ib_order, original);
    }

    #[rstest::rstest]
    #[case::own_order(7, "DU001", true)]
    #[case::other_client(8, "DU001", false)]
    #[case::other_account(7, "DU002", false)]
    fn is_own_open_order_requires_client_and_account(
        #[case] client_id: i32,
        #[case] account: &str,
        #[case] expected: bool,
    ) {
        let order = ibapi::orders::Order {
            client_id,
        ...
        assert_eq!(
            InteractiveBrokersExecutionClient::is_own_open_order(&order, 7, Ustr::from("DU001")),
            expected
        );
    }
