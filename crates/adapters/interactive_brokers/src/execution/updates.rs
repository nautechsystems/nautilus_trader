...
impl InteractiveBrokersExecutionClient {
    ...
    pub(super) fn absent_order_terminal_event(
        ...
        let Some((status, perm_id)) = completed else {
        ...
        match ib_order_status {
            ...
            Some(IbOrderStatus::Inactive) => {
                ...
                if status.filled > 0.0 {
                    ...
                    // IB can report a working order as Inactive, for example after it
                    // refuses a modify, so an accepted order stays tracked as working
                    if !Self::emit_modify_rejected_if_pending(
                        &order,
                        status.order_id,
                        reason,
