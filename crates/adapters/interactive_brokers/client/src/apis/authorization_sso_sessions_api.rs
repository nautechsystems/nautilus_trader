...
pub async fn gw_api_v1_sso_browser_sessions_post(
    configuration: &configuration::Configuration,
    authorization: &str,
    create_browser_session_request: models::CreateBrowserSessionRequest,
) -> Result<models::CreateBrowserSessionResponse, Error<GwApiV1SsoBrowserSessionsPostError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CreateBrowserSessionResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CreateBrowserSessionResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GwApiV1SsoBrowserSessionsPostError> =
...
pub async fn gw_api_v1_sso_sessions_post(
    configuration: &configuration::Configuration,
    authorization: &str,
    create_session_request: models::CreateSessionRequest,
) -> Result<models::CreateSessionResponse, Error<GwApiV1SsoSessionsPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_authorization = authorization;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CreateSessionResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CreateSessionResponse`")))),
        }
    } else {
        let content = resp.text().await?;
