...
pub async fn gw_api_v1_echo_https_get(configuration: &configuration::Configuration, ) -> Result<models::EchoResponse, Error<GwApiV1EchoHttpsGetError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::EchoResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::EchoResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn gw_api_v1_echo_signed_jwt_post(configuration: &configuration::Configuration, signed_jwt_echo_request: models::SignedJwtEchoRequest) -> Result<models::EchoResponse, Error<GwApiV1EchoSignedJwtPostError>> {
