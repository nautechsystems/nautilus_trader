...
pub enum SubmittedOrderExhaustionPolicy {
    /// Resolves the order locally using the existing timeout and missing-order policies.
    #[default]
    ResolveLocally,
    /// Retains the unresolved submission without further automatic per-order queries.
    RetainUnresolved,
}
...
///
/// This diagnostic is not an order event and does not change order status. It is
/// published once on the submission recovery exhaustion topic. The native manager
/// retains the record until applied evidence establishes the submission outcome.
