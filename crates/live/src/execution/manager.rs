...
impl ExecutionManager {
    ...
    /// Records a position-check fill the engine did not apply, keeping its first failure time.
    pub fn record_unapplied_fill_report(&mut self, report: &FillReport) {
        ...
        self.fills_unapplied.prune_older_than(Duration::from_mins(
            self.config.position_check_lookback_mins.max(1),
        ));
    }

    /// Returns whether a fill has stayed unapplied past the position check threshold.
    ///
    /// Such a fill no longer holds back synthetic position reconciliation; a fill still inside
    /// the threshold may be waiting on an asynchronous step, such as an order status query.
    #[must_use]
