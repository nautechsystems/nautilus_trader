...
impl<S: order_accepted_spec_builder::IsComplete> OrderAcceptedSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderAccepted`] through its production constructor.
    #[must_use]
...
mod tests {
    ...
    fn defaults_are_sensible() {
        ...
        let event = OrderAcceptedSpec::builder().build();
        ...
        assert_eq!(event.venue_order_id, VenueOrderId::test_default());
    ...
    fn event_ids_are_unique_within_a_run() {
        reset_test_uuid_rng();
        let a = OrderAcceptedSpec::builder().build();
        let b = OrderAcceptedSpec::builder().build();
        let c = OrderAcceptedSpec::builder().build();
    ...
    fn event_id_sequence_is_reproducible() {
        ...
        let first_run: Vec<_> = (0..3)
            .map(|_| OrderAcceptedSpec::builder().build().event_id)
            .collect();
