...
impl<S: order_triggered_spec_builder::IsComplete> OrderTriggeredSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderTriggered`] through its production constructor.
    #[must_use]
...
mod tests {
    ...
    fn defaults_are_sensible() {
        ...
        let event = OrderTriggeredSpec::builder().build();
    ...
    fn overrides_apply_through_constructor() {
        let event = OrderTriggeredSpec::builder()
    ...
    fn event_ids_are_unique_within_a_run() {
        reset_test_uuid_rng();
        let a = OrderTriggeredSpec::builder().build();
        let b = OrderTriggeredSpec::builder().build();
        let c = OrderTriggeredSpec::builder().build();
    ...
    fn event_id_sequence_is_reproducible() {
        ...
        let first_run: Vec<_> = (0..3)
            .map(|_| OrderTriggeredSpec::builder().build().event_id)
            .collect();
