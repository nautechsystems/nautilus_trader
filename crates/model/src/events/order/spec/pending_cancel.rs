...
impl<S: order_pending_cancel_spec_builder::IsComplete> OrderPendingCancelSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderPendingCancel`] through its production constructor.
    #[must_use]
...
mod tests {
    ...
    fn defaults_are_sensible() {
        ...
        let event = OrderPendingCancelSpec::builder().build();
    ...
    fn event_ids_are_unique_within_a_run() {
        reset_test_uuid_rng();
        let a = OrderPendingCancelSpec::builder().build();
        let b = OrderPendingCancelSpec::builder().build();
        let c = OrderPendingCancelSpec::builder().build();
    ...
    fn event_id_sequence_is_reproducible() {
        ...
        let first_run: Vec<_> = (0..3)
            .map(|_| OrderPendingCancelSpec::builder().build().event_id)
            .collect();
