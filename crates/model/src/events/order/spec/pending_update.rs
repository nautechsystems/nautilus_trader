...
/// Test-only fluent spec for [`OrderPendingUpdate`].
///
/// All fields carry sensible defaults so callers only set what differs.
/// `build()` constructs the event through [`OrderPendingUpdate::new`] so any future invariants
...
impl<S: order_pending_update_spec_builder::IsComplete> OrderPendingUpdateSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderPendingUpdate`] through its production constructor.
    #[must_use]
