...
impl<S: order_emulated_spec_builder::IsComplete> OrderEmulatedSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderEmulated`] through its production constructor.
    #[must_use]
    pub fn build(self) -> OrderEmulated {
...
mod tests {
    ...
    fn defaults_are_sensible() {
        ...
        let event = OrderEmulatedSpec::builder().build();
    ...
    fn overrides_apply_through_constructor() {
        let event = OrderEmulatedSpec::builder()
    ...
    fn event_ids_are_unique_within_a_run() {
        reset_test_uuid_rng();
        let a = OrderEmulatedSpec::builder().build();
        let b = OrderEmulatedSpec::builder().build();
        let c = OrderEmulatedSpec::builder().build();
    ...
    fn event_id_sequence_is_reproducible() {
        ...
        let first_run: Vec<_> = (0..3)
            .map(|_| OrderEmulatedSpec::builder().build().event_id)
            .collect();
