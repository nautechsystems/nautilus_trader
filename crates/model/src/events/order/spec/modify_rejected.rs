...
impl<S: order_modify_rejected_spec_builder::IsComplete> OrderModifyRejectedSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderModifyRejected`] through its production constructor.
    #[must_use]
    pub fn build(self) -> OrderModifyRejected {
...
mod tests {
    ...
    fn defaults_are_sensible() {
        ...
        let event = OrderModifyRejectedSpec::builder().build();
    ...
    fn event_ids_are_unique_within_a_run() {
        reset_test_uuid_rng();
        let a = OrderModifyRejectedSpec::builder().build();
        let b = OrderModifyRejectedSpec::builder().build();
        let c = OrderModifyRejectedSpec::builder().build();
    ...
    fn event_id_sequence_is_reproducible() {
        ...
        let first_run: Vec<_> = (0..3)
            .map(|_| OrderModifyRejectedSpec::builder().build().event_id)
            .collect();
