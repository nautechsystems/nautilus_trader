...
impl<S: order_cancel_rejected_spec_builder::IsComplete> OrderCancelRejectedSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderCancelRejected`] through its production constructor.
    #[must_use]
    pub fn build(self) -> OrderCancelRejected {
...
mod tests {
    ...
    fn defaults_are_sensible() {
        ...
        let event = OrderCancelRejectedSpec::builder().build();
    ...
    fn overrides_apply_through_constructor() {
        ...
        assert_eq!(event.reason, Ustr::from("ORDER_DOES_NOT_EXIST"));
    ...
    fn event_ids_are_unique_within_a_run() {
        reset_test_uuid_rng();
        let a = OrderCancelRejectedSpec::builder().build();
        let b = OrderCancelRejectedSpec::builder().build();
        let c = OrderCancelRejectedSpec::builder().build();
    ...
    fn event_id_sequence_is_reproducible() {
        ...
        let first_run: Vec<_> = (0..3)
            .map(|_| OrderCancelRejectedSpec::builder().build().event_id)
            .collect();
