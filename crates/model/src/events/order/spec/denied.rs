...
/// Test-only fluent spec for [`OrderDenied`].
///
/// All fields carry sensible defaults so callers only set what differs.
/// `build()` constructs the event through [`OrderDenied::new`] so any future invariants
...
impl<S: order_denied_spec_builder::IsComplete> OrderDeniedSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderDenied`] through its production constructor.
    #[must_use]
