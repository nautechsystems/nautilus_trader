...
impl TrailingStopMarketOrder {
    ...
    fn py_account_id(&self) -> Option<crate::identifiers::AccountId> {
        self.account_id
    }
    ...
    fn py_from_dict(values: &Bound<'_, PyDict>) -> PyResult<Self> {
        ...
        let trailing_offset = get_required_parsed(values, "trailing_offset", |s| {
            s.parse::<Decimal>().map_err(|e| e.to_string())
        })?;
        let trailing_offset_type = get_required_parsed(values, "trailing_offset_type", |s| {
            s.parse::<TrailingOffsetType>().map_err(|e| e.to_string())
        })?;
    ...
    fn py_to_dict(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
        ...
        self.activation_price.map_or_else(
            || dict.set_item("activation_price", py.None()),
            |x| dict.set_item("activation_price", x.to_string()),
        )?;
