...
impl CryptoFuturesSpread {
    ...
    #[new]
    #[pyo3(signature = (instrument_id, raw_symbol, underlying, quote_currency, settlement_currency, is_inverse, strategy_type, activation_ns, expiration_ns, price_precision, size_precision, price_increment, size_increment,ts_event, ts_init, multiplier=None, lot_size=None, max_quantity=None, min_quantity=None, max_notional=None, min_notional=None, max_price=None, min_price=None, margin_init=None, margin_maint=None, maker_fee=None, taker_fee=None, tick_scheme=None, info=None))]
    fn py_new(
    ...
    fn py_strategy_type(&self) -> String {
    ...
    fn py_to_dict(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
        ...
        dict.set_item("type", stringify!(CryptoFuturesSpread))?;
...
mod tests {
    ...
    use crate::instruments::{CryptoFuturesSpread, stubs::*};

    #[rstest]
    fn test_dict_round_trip(crypto_futures_spread_btc_deribit: CryptoFuturesSpread) {
        Python::initialize();
        Python::attach(|py| {
            let original = crypto_futures_spread_btc_deribit;
            let values = original.py_to_dict(py).unwrap();
            let values: Py<PyDict> = values.extract(py).unwrap();
            let restored = CryptoFuturesSpread::py_from_dict(py, values).unwrap();
