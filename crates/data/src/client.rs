...
use nautilus_common::messages::data::{
    ...
    RequestOrderBookDepths, RequestQuotes, RequestTrades, SubscribeBars, SubscribeBookDeltas,
...
pub trait DataClient: Any + Sync + Send {
    ...
    fn request_order_book_depths(&self, request: &RequestOrderBookDepths) -> anyhow::Result<()> {
        log_not_implemented(&request);
        Ok(())
    }
}
...
impl DataClientAdapter {
    ...
    pub fn request_order_book_depths(&self, req: &RequestOrderBookDepths) -> anyhow::Result<()> {
        self.client.request_order_book_depths(req)
    }
}
