...
const QUEUE_ORDER_COUNTS: &[usize] = &[1, 32, 256];
const LIQUIDITY_LEVEL_COUNTS: &[usize] = &[1, 10, 100];
...
fn bench_queue_position(c: &mut Criterion) {
    ...
    let l3_updates = generate_l3_queue_updates(instrument_id, rest_price, MARKET_DATA_COUNT);
    let quotes = generate_quotes(instrument_id, MARKET_DATA_COUNT);
    let mut group = c.benchmark_group("matching_engine/queue_position");

    group.throughput(Throughput::Elements(MARKET_DATA_COUNT as u64));

    for &order_count in QUEUE_ORDER_COUNTS {
        ...
        // Trades away from the tracked price expose unrelated-order scan cost
        group.bench_function(BenchmarkId::new("trade_l2_miss", order_count), |b| {
            b.iter_custom(|iters| {
                run_iterations(
                    iters,
...
fn bench_liquidity_consumption(c: &mut Criterion) {
    let instrument_id = crypto_perpetual_ethusdt().id();
    let market_orders = generate_orders(instrument_id, OrderType::Market, None, ORDER_COUNT);
    let mut group = c.benchmark_group("matching_engine/liquidity_consumption");

    group.throughput(Throughput::Elements(ORDER_COUNT as u64));

    for &level_count in LIQUIDITY_LEVEL_COUNTS {
            ...
            group.bench_function(
                ...
                |b| {
                    b.iter_custom(|iters| {
                        run_iterations(
                            iters,
                            || build_liquidity_order_scenario(&market_orders, level_count, enabled),
                            |scenario| {
    ...
    for &level_count in LIQUIDITY_LEVEL_COUNTS {
        let worst_price = price_from_cents(150_000 + level_count as i64 - 1);
        let trades = generate_sweep_trades(instrument_id, worst_price, MARKET_DATA_COUNT);
        group.bench_function(BenchmarkId::new("trade_seed", level_count), |b| {
            b.iter_custom(|iters| {
                run_iterations(
                    iters,
                    || build_liquidity_engine(level_count, MARKET_DATA_COUNT),
                    |state| {
                        for trade in &trades {
                            state.engine.process_trade_tick(black_box(trade));
                        }
                    ...
                    |state| {
                        assert_eq!(state.engine.get_core().last, Some(worst_price));
                        assert_events(state, EventCounts::default());
                    },
                )
            });
...
fn build_queue_engine(
    ...
    if let Some(level) = level {
        state
            ...
            .expect("seed level should be processed");
    }
...
fn build_liquidity_order_scenario(
    orders: &[OrderAny],
    level_count: usize,
    liquidity_consumption: bool,
) -> OrderScenario {
    let state = build_liquidity_engine_with_config(level_count, ORDER_COUNT, liquidity_consumption);
...
fn build_liquidity_engine(level_count: usize, total_size: usize) -> BenchEngine {
    build_liquidity_engine_with_config(level_count, total_size, true)
}

fn build_liquidity_engine_with_config(
    level_count: usize,
    total_size: usize,
    liquidity_consumption: bool,
) -> BenchEngine {
    ...
    let mut state = build_engine_with_config(BookType::L2_MBP, config);
    let bid = book_delta(
        instrument_id,
        BookAction::Add,
        OrderSide::Buy,
    ...
    state
        .engine
        .process_order_book_delta(&bid)
        .expect("seed bid should be processed");

    for delta in generate_liquidity_levels(instrument_id, level_count, total_size) {
        state
            .engine
...
fn generate_sweep_trades(
    ...
    (0..count)
        .map(|i| {
            let ts = UnixNanos::from(BASE_TS_NS + count as u64 + i as u64);
            TradeTick::new(
                instrument_id,
                price,
                Quantity::from("1.000"),
                AggressorSide::Buy,
                TradeId::from(format!("T-SWEEP-{i}").as_str()),
                ts,
                ts,
            )
        })
...
fn generate_liquidity_levels(
    instrument_id: InstrumentId,
    level_count: usize,
    total_size: usize,
) -> Vec<OrderBookDelta> {
    let level_size = total_size.div_ceil(level_count);
    let size = Quantity::from(format!("{level_size}.000").as_str());
...
fn book_delta(
    ...
    OrderBookDeltaTestBuilder::new(instrument_id)
        .book_action(action)
        .book_order(BookOrder::new(side, price, size, order_id))
        .sequence(sequence)
        .ts_event(ts)
        .ts_init(ts)
        .build()
