...
- Examples:
  ...
  - `plumeusdt_bitget_perp_makerv3.toml`
  - `plumeusdt_bitget_spot_makerv3.toml`
...
- `[node.venues.BITGET].api_passphrase_env` is required for Bitget private execution/data auth.
...
- Use `docs/runbooks/tokenmm-risk-validation.md` for the post-restart
  startup reconciliation and local/global risk review gates.
...
- Node files should reference env var names such as `BYBIT_API_KEY`, `BINANCE_API_KEY`, `BITGET_API_KEY`, and `OKX_API_KEY`, not inline secrets.
